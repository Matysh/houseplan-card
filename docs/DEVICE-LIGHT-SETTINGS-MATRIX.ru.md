...
|---|---|
| «Значок + динамическая подложка» | Подложка устройства показывает его resolved working state; Glow определяется матрицей выше |
| «Значок + активность» | Дополнительно показывает короткую активность; источник света не меняется |
| «Значение вместо иконки» | Меняет только содержимое маркера; источник света не меняется |
