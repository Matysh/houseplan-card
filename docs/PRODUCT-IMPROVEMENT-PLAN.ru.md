...
| Проверки | Инвентаризация: 497 Node unit, 105 pure backend, 50 HA-harness и 118 browser smoke | Первый просмотренный Linux baseline из 20 golden-сценариев принят и включён в блокирующий CI gate; performance budgets ещё не утверждены |
...
| HP-QA-01 | P1 | Baseline принят, CI gate активен; матрица расширяется вместе с поверхностью продукта | Визуальная regression-матрица геометрии, света и адаптивных диалогов |
...
является единственным путём замены эталонов. Первый Linux baseline из 20
сценариев просмотрен и принят на gate `v1.60.3-beta.1`; после появления PNG
отдельный CI job автоматически работает в блокирующем режиме.
