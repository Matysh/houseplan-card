...
| Item | State |
|---|---|
| Version | **v1.68.0-beta.1 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.67.0-rc.3 is published. Current `dev` stabilises persisted wall-segment identity and atomic model-v8 upgrades (#282), keeps active wall-chain axes visible (#307), corrects acute and multi-wall junction envelopes (#309/#310), and includes code-only stylesheet/process maintenance. v1.68.0-beta.1 packages the reviewed S8 product work and remains gated by exact-SHA Validate before publication. |
