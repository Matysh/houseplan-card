...
| Current local cycle | v1.68.1 is the current stable release. Active branch `issue/264-resize-controller` is a refactor-only cycle: move Resize interaction ownership out of the root card, share the wall-record preservation invariant with the CLI, and keep user-visible behaviour unchanged. Targeted controller/invariant coverage, production Resize smokes, benchmarks and golden verification are required before review. |
