...
| Item | State |
|---|---|
| Version | **v1.64.0** everywhere (manifest, const.py, package.json, CARD_VERSION) — stable promotion candidate after published v1.64.0-beta.2 |
| Current local cycle | v1.64.0 promotes the published beta line without product-code changes. It contains the read-only cold-start fix (#131), persistent virtual-light toggles (#107), four-phase Sun background (#146), visible plan snap topology (#137), shared-wall room autoclose (#138), seamless wall junctions (#141) and dialog-footer polish (#140), plus merged test, CI, process and documentation maintenance. The independent #124 exact-SHA view-toggle performance debt remains open under the owner's accepted exception. |
