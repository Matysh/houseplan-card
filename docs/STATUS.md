...
| Item | State |
|---|---|
| Version | **v1.61.0-beta.6** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate with visibility-based light transport, stable room hover and shared responsive dialog chrome |
| Current local cycle | v1.61.0-beta.6 rebuilds light transport on ONE rule accepted by the owner after manual testing (#71): **a lamp lights the floor it can see**. `_lightBarriers` collects the drawn wall bodies with their real thickness, independent bodies and the outlines of edges without thickness, cuts out passages (a window and a door with no floor behind it stay solid) and virtual boundaries, then splits every barrier where barriers cross so the sweep never skips a junction corner; `visibilityPolygon` (`src/light-visibility.ts`) returns the region and one clipped circle paints it. The separate spill path, the doorway sector, the aperture tunnel, the open-zone graph and the shadow mask are gone, and with them the class of bugs where two layers disagreed. Pools now fade over the whole radius (`GLOW_FALLOFF`) instead of holding a plateau to 70%, and the layer gets one screen-pixel-sized blur as a penumbra. Smoke asserts pixels: aperture lit, floor behind a door lit, nothing behind a column (≤3) while the floor beside it is lit (≥8), a lit→unlit border at most 4 px wide, one painted child per spot, an outside door that changes the lit region not at all. Room hover was also moved off compositor filters to remove a one-frame Glow brightness flash (#72), and shared dialogs now grow for multiline titles and wrap destructive/commit action groups safely (#77/#78). 60 sources render ~3.5× faster than the layered model. The model, its
exceptions and its gotchas are documented in `docs/LIGHT.md`. |
...
| CI | Prerelease publication requires a green exact-SHA Validate: frontend/backend, smoke, golden, HACS/Hassfest and a short absolute-ceiling performance smoke. Obsolete same-ref Validate runs are cancelled. Full seven-sample base/candidate performance moved to `performance.yml` (`main` push, weekly, manual); stable release assets fail closed unless both Validate and Full Performance are green for the exact tagged SHA. |
