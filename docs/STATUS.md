...
| Item | State |
|---|---|
| Version | **v1.68.0** everywhere (manifest, const.py, package.json, CARD_VERSION) — stable promotion of the published v1.68.0-beta.4 product line |
| Current local cycle | v1.68.0 promotes the exact product behaviour already published through beta.4: stable wall-segment identity and atomic model-v9 migration, unified zero-thickness walls, repaired junction rendering and reliable plan editing. The complete local suite and exact-tree Validate are green. Stable-gate diagnostics exposed infrastructure-only defects: legacy v1.66.0 lacked the current bundle materializer, five full 7-sample profile pairs could not fit a 60-minute serial job, and the comparative launcher initially validated an old served bundle against the candidate tree. Old baselines now use an equivalent fresh-bundle fallback, each target is freshness-checked against its own exact tree, and independent profile pairs run in parallel while retaining same-runner sequential base/candidate comparison. These repairs change no shipped bundle or product behaviour; their exact-SHA Validate and Full Performance remain the publication gates. |
