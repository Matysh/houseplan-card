...
| Item | State |
|---|---|
| Version | **v1.71.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.71.0-beta.3 adds explicit map-to-space routing for multi-floor robot vacuums while keeping the dock in its own space (#162), rolls General settings back to the last server-confirmed state after a rejected save (#439), and hardens custom Background assets, pointer modality and audit witnesses (#440). Publish only after exact-SHA Validate is green. |
