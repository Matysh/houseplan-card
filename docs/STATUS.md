...
| Item | State |
|---|---|
| Version | **v1.69.0-beta.5 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.68.1 is the current stable release. v1.69.0-beta.5 packages the reviewed `S8-merged` work since beta.4: tighter whole-house framing and cached Glow in `houseplan-space-card` (#373, #375), persistent Background default style (#377), the beta.4 audit polish (#376), and an explicit source selector for the Value + state face (#378). Publication remains gated by exact-SHA Validate. |
