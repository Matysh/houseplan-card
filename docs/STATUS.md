...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.3 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — third prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.2 is published. Current `dev` includes orphan-reference repair (#244), idempotent Optimize storage/reload handling (#248), and centerline-only opening-symbol placement regardless of `flip_v` (#250). v1.67.0-beta.3 packages these reviewed S8 changes and remains gated by the full exact-SHA Validate before publication. |
