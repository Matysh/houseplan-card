...
| Item | State |
|---|---|
| Version | **v1.77.0-beta.1** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.77.0-beta.1 candidate** — prepared from the exact integrated `dev` tree; `main` stays on v1.76.0. One user-visible fix: furniture pieces with equal declared sizes now have equal footprints on the plan and the “Stairs”/“TV” menu icons stay inside their bounds (#584). Internal in the line: the golden manifest records the capture platform rather than the acceptance one (#571); the CI proof separates the product tree from the accepted golden overlay and release consumers recompute it, so a baseline-only commit after a golden-red candidate reuses smoke, performance smoke, parity and backend and skips caught witnesses (#573); the candidate-only strict screenshot-freshness mode actually engages (#586); the stable Full Performance base is the previous stable tag instead of the `main` tip (#587). In implementation: #585 replaces the hidden 2.5D overlay's lossy 4 px lattice with iterative critical-boundary events, restores the dense Room 2.5 marker pair and returns both full isometric profiles to their ordinary performance allowances; the reviewed Linux golden (`86dffbc7`, run `35470468142`) and exact-SHA Full Performance (`35477092908`) evidence are green. |
...
| Furniture | #159 replaces the flat ~30-item picker with a two-level category/variant palette; #593 raises it to 60 top-view symbols, all designer artwork. The reviewed 93-SVG MIT source pack is vendored under `assets/furniture/houseplan-0.4.0`; plan art is a lazy chunk (#474), front-view category art stays in the lazy editor graph, and existing saved geometry and default dimensions are unchanged. |
