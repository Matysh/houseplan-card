...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.8** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.8 aligns the summary panel, split header control and wide settings dialog with the designer prototype, adds smooth hiding, removes redundant size controls and links mobile visibility to local panel visibility (#505). Backup imports report committed results reliably, and removed or re-targeted robot map routes no longer restore deleted runs after restart (#495). Upload quota accounting, SVG reference-depth limits and support-package palette privacy are hardened (#498). All three issues passed independent review and are merged into `dev`. Publication requires fresh reviewed documentation screenshots and green exact-SHA Validate; `main` remains at v1.72.0. |
