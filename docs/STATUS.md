...
| Version | **1.78.0-beta.3** in all 7 version sources (`scripts/release-contract.mjs`) |
| Latest stable tag | `v1.77.0` |
| Latest prerelease tag | `v1.78.0-beta.2` |
| Tests | Node unit 3051 · pure backend 389 · HA-harness backend 300 · browser smokes 275 (`npm run inventory`) |
<!-- status-snapshot:end -->
...
| Item | State |
|---|---|
| Current local cycle | **Beta v1.78.0-beta.3 candidate** — prepared from the exact integrated `dev` tree. It makes 2.5D a public opt-in setting (#649), compacts the phone header and stabilises the main toolbar (#616, #647), adds native Sections height resize (#648), tightens Home Assistant read-only access (#626), and includes the associated repository, release-review and mutation-gate maintenance (#628, #638, #650). `main` remains on stable v1.77.0. |
