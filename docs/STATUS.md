...
| Item | State |
|---|---|
| Version | **v1.69.0-beta.4 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.68.1 is the current stable release. v1.69.0-beta.4 packages the reviewed `S8-merged` work since beta.3: opt-in Glow and compact empty-title framing in `houseplan-space-card` (#374, #372), French localisation (#371), safer Background editing and large-backdrop handling (#39, #360–#363), bounded moving-opening Glow updates (#366), furniture stroke scaling (#361), and the audit fixes in #369. Publication remains gated by exact-SHA Validate. |
