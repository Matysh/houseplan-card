...
| Item | State |
|---|---|
| Version | **v1.60.2-beta.2** everywhere (manifest, const.py, package.json, CARD_VERSION) — pre-release with corrected composite-device lifecycle state, responsive device information, a single no-area room save path and a wider Erase hit target. The tag points at `dev`; `main` remains on v1.60.0 |
