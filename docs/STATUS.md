...
| Item | State |
|---|---|
| Version | **v1.68.0-beta.3 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.68.0-beta.2 is published. Current `dev` advances the persisted wall model to v9 (#306): the separate Boundary/virtual-wall UX is removed, all `cm:0` walls share one canonical representation, and each space selects dashed/light-transmitting or solid/light-blocking zero-wall semantics. Legacy `open_spans/open_to` remain read-compatible until the next structural transaction or Optimize. v1.68.0-beta.3 packages this reviewed S8 product work and remains gated by exact-SHA Validate before publication. |
