...
| Item | State |
|---|---|
| Version | **v1.61.0-beta.5** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate with tri-state light-source roles, per-marker Glow colour/brightness, perceptual intensity and a faster release validation path |
| Current local cycle | v1.61.0-beta.5 combines #65–#67: Auto/Always/Never per-marker light roles, strict colour/brightness overrides and one perceptual Glow-alpha resolver. #69 splits release performance policy between the candidate-only 60-source smoke used by ordinary exact-SHA Validate and the full same-runner comparison required for stable releases; #70 makes Telegram announcements stable-only. |
...
- **Independent Glow overlay** (#55/#19, refined locally by #61/#65–#67): dark-room
  ...
  with brightness. One perceptual alpha resolver feeds both pools and doorway
  spill; per-source radius, door sectors and transitive open zones remain.
