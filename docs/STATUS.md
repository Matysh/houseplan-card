...
| Current local cycle | Development after v1.73.0-beta.6 adds the configurable read-only summary overlay (#437): shared versioned blocks and sources, per-user/per-card show and View sizes, native mobile/fit hiding, authoritative device and clean-floor totals, and a lazy settings form. The feature passed code review and is merged into `dev` (S8-merged, awaiting the next beta); publish from `dev` only after exact-SHA Validate is green, while `main` remains at v1.72.0. |
