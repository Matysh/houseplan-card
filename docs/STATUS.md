...
| Current local cycle | v1.67.0-beta.7 is published. The next beta cycle contains exact explicit-Optimize reconciliation of a redundant partition over one solid shared room wall (#276) and fixed-topology Resize that moves only safe horizontal/vertical walls, affects at most two exact endpoint-matched rooms and clamps before openings/corners/topology changes (#277); wall-union failure isolation (#278) follows before publication. |
