...
| Item | State |
|---|---|
| Version | **v1.74.0** everywhere (manifest, const.py, package.json, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Stable v1.74.0** — `main` is fast-forwarded to the tested `dev` SHA and the GitHub Release is public. The line aggregates from the stable v1.73.0: the plan is noticeably smoother and in Firefox dramatically so — a pan frame now moves what is already drawn by a composited transform with the scene re-rasterizing on a budget (#531), and a marker shadow sized in container units no longer restarts a non-composited animation on every marker at once (#524); the printed plan is about a third larger because the in-plan dimension labels shrank and the separate column of external dimensions is gone (#530); a plan left over from the era of rooms without walls can be edited again, the leftover cleaned up on the first save (#529, reported in #527); plus alignment guides that follow the gesture (#521), a summary panel that neither freezes the first frame nor claims an unavailable source (#509) and whose settings dialog scrolls again (#508), no replayed door leaf on a space switch (#525), and 12.8 KB less on a cold start (#526). Internal in the line: #528, #518, #517, #512, #511, #510. The speed verdict for #531 is deliberately NOT claimed by the sandbox — headless Firefox and Chromium hold 60 fps on both the old and the new path; it is owed by a Firefox profile from the owner's machine on the same pan. The first release candidate was held back by a flaky witness, not by a product defect: `demo/smoke_room_resize.mjs` fixed its screen coordinates once, up front, while the card converts them back at event time, so a late layout made the gesture commit the wrong amount — red on the runner, green locally. The witness now converts inside the same frame as the dispatch, checks that every event was delivered and pins the mapping itself (#533). |
