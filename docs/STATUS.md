...
| Item | State |
|---|---|
| Version | **v1.71.0-beta.4** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.71.0-beta.4 fixes adding and removing multi-floor robot routes (#441, #443), atomically rolls rejected device and robot-calibration writes back to server-confirmed state (#442), snaps furniture to the physical wall face (#445), and improves the HACS storefront copy (#444). Publish only after exact-SHA Validate is green. |
