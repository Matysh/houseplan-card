...
| Item | State |
|---|---|
| Version | **v1.74.0-beta.2** everywhere (manifest, const.py, package.json, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | v1.74.0-beta.2 continues the line after the stable v1.73.0. User-visible in this beta: a plan left over from the era of rooms without walls can be edited again — the leftover is cleaned up on the first save instead of refusing every change, sending the user to an Optimize that cannot help and blocking the export as well (#529, reported by a user in #527); the plan is smooth again in Firefox, where a marker shadow sized in container units restarted a non-composited animation on every marker at once and cost 9.4 frames per second (#524); switching between spaces no longer replays a neighbouring door's leaf movement or a marker's shadow (#525); and the card downloads 12.8 KB less on a cold start because the stylesheet minifier had never actually run (#526). Internal: the space-switch witness now judges node identity rather than a transition that #524 removed (#528), and the review gate charges a round only for what the round changed — witnesses are judged by their anchor's neighbourhood, an empty shard plan costs no environment, and the matrix is six shards (#518). Everything from v1.74.0-beta.1 stays in the line. Publication requires green Validate on the exact candidate SHA; `main` stays at v1.73.0. |
