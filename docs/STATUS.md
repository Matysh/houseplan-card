...
| Item | State |
|---|---|
| Version | **v1.61.0-beta.8** everywhere (manifest, const.py, package.json, CARD_VERSION) — final prerelease candidate for the v1.61.0 stability release |
| Current local cycle | v1.61.0-beta.8 completes the #73 review corrections. Full and compact cards use one bounded continuity barrier and immutable render-device snapshot, so a reconnect, delayed protected backdrop or ordinary HA tick cannot mix device, room, Glow, sun, opening, vacuum and live-text data from different frames or leave a complete plan trapped below an opaque recovery layer. Structural fingerprints ignore ordinary state ticks; reconnect semantics require a confirmed connection loss; signed-asset and shared-config races are bounded and retryable. The deterministic continuity smoke now exercises the production visibility listener and a fixture containing thick walls, openings, Glow, sun, decor, protected content and devices. `docs/LIGHT.md` remains canonical. |
