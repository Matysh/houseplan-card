...
| Current local cycle | v1.68.0 promotes the exact behaviour already published through beta.4: stable wall-segment identity and atomic model-v9 migration, unified zero-thickness walls, repaired junction rendering and reliable plan editing. The stable promotion contains only version fields, generated bundle snapshots and release metadata. The complete local suite and the first exact-tree Validate are green; after the no-content `main` history return, the final exact-SHA Validate and Full Performance workflows remain the publication gates. |
