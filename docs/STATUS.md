...
| Current local cycle | v1.68.0 promotes the exact product behaviour already published through beta.4: stable wall-segment identity and atomic model-v9 migration, unified zero-thickness walls, repaired junction rendering and reliable plan editing. The complete local suite and exact-tree Validate are green. The first `main` Full Performance attempt exposed only a release-infrastructure compatibility gap with the older v1.66.0 baseline; the backward-compatible baseline materialization fix changes no shipped bundle or product behaviour, and the repaired exact-SHA Validate plus Full Performance remain the publication gates. |
