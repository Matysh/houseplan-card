...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.9 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — ninth prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.8 is published. Current `dev` preserves masonry at near-orthogonal T-junctions without straightening stored axes (#279), independently validates Optimize opening rehosts at the backend boundary (#280), and safely reconciles exact outer-wall partitions so Resize exposes only handles with a real non-zero grid step (#281). v1.67.0-beta.9 packages these owner-approved S8 fixes and remains gated by the full exact-SHA Validate before publication. |
