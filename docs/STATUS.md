...
| Current local cycle | v1.68.0 promotes the exact behaviour already published through beta.4: stable wall-segment identity and atomic model-v9 migration, unified zero-thickness walls, repaired junction rendering and reliable plan editing. The stable commit contains only version fields, generated bundle snapshots and release metadata; publication is gated by the complete local suite plus green exact-SHA Validate and Full Performance workflows. |
