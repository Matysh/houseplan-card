...
| Item | State |
|---|---|
| Version | **v1.65.0-beta.7** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate for the current `S8-merged` queue |
| Current local cycle | v1.65.0-beta.7 candidate carries #210 and #211. Card instances may be pinned to one fixed space without sharing navigation, and device markers now match the source design package instead of the oversized beta.6 projection. Publication waits for a green exact-SHA Validate, reviewed Linux golden baselines for the intentional marker correction, and the fail-closed prerelease preflight. v1.65.0-beta.6 is published and stable v1.64.0 remains unchanged. |
