...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.9** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.9 fixes the startup regression that blocked the stable promotion of v1.73.0: a card instance on a page whose summary chunk is already loaded now owns its summary runtime before the first render, so the first header measurement is complete and no late `stage-resize` continuity cascade lands on the first HA tick (#506). Code factory is cached per page, runtime state stays per instance; the summary chunk remains lazy. Stable v1.73.0 follows only after green Validate and Full Performance (all nine profiles against the v1.72.0 product) on the exact release SHA; `main` remains at v1.72.0. |
