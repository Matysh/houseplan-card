...
| Item | State |
|---|---|
| Version | **v1.77.0-beta.5** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.77.0-beta.5 candidate** — prepared from the exact integrated `dev` tree; `main` stays on v1.76.0. User-visible: corrected furniture catalog artwork and Exercise placement (#606), and further dialog polish for the discard prompt, switches, room settings, colour controls and device fields (#603, #605). Internal: nightly mutation-gate shards now run as six bounded jobs and reject incomplete shard evidence (#604). |
