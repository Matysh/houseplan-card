...
| Item | State |
|---|---|
| Version | **v1.72.0-beta.2** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.72.0-beta.2 adds double-click/double-tap Fit all on free plan background (#449), accepts real Zigbee2MQTT camelCase raw network maps (#450), corrects the Russian Fit all guide label (#452), and hardens review-round accounting (#454). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.71.0. |
