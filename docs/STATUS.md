...
| Item | State |
|---|---|
| Version | **v1.70.0-beta.4** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.70.0-beta.4 is the prerelease candidate for the final Help & feedback polish: matching header-button sizing and the standard message-input surface. `main` remains on the stable line; publication is gated by the exact-SHA Validate and the local prerelease contract, after which this tested beta line is promoted to v1.70.0 without new product behaviour. |
