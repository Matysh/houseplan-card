...
| Item | State |
|---|---|
| Version | **v1.65.0-beta.5** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate for the current `S8-merged` queue |
| Current local cycle | v1.65.0-beta.5 candidate carries #176, #186, #195, #200, #201, #203, #204 and #205; issue branch #198 additionally makes explicit Optimize remove only a proven isolated sub-half-step wall-thickness island, while ordinary render/edit normalisation remains lossless and Preview/Apply/server Undo remain unchanged. One same-map vacuum trail survives available stops up to 30 minutes; new-space File/Draw border/name defaults are visible and a user-touched pair survives source switches. Independent-wall openings preserve their physical jamb margin, room labels keep View/Plan geometry and obey the name-visibility switch, atomic wall stretches inherit the intended thickness, the editor close target is easier to hit, and unreachable legacy Partition tool-state is removed. Publication waits for a green exact-SHA Validate and fail-closed prerelease preflight; v1.65.0-beta.4 is published and stable v1.64.0 remains unchanged. |
