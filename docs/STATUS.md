...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.5** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.5 adds the primary `/houseplan` sidebar panel around the same full-card model, with fail-soft owned backend registration, a separate bounded panel entry, read-only empty-state safety and full-width Sections default for the optional dashboard card (#486). PDF export also preserves a complete reconstructable exterior-dimension chain around rectangular facade steps (#484). The card graph and saved plan/config/layout models remain unchanged. Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
