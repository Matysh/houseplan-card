...
| Item | State |
|---|---|
| Version | **v1.59.0-beta.6** everywhere (manifest, const.py, package.json, CARD_VERSION) — **pre-release**, tag `v1.59.0-beta.6` on **`dev`**, GitHub Release with `prerelease=true`; `main` is NOT touched and **nothing is copied to the home instance by hand** — HACS delivers it on the beta channel. On top of beta.5: **partial open spans** (`space.open_spans`, two-click openwall, openings forbidden on virtual); **wall-centric Delete** (close virtual → merge/delete room with confirm). Previous pre-release: v1.59.0-beta.5; previous stable: v1.58.0 |
