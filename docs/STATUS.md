...
| Item | State |
|---|---|
| Version | **v1.59.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION) — **pre-release**, tag `v1.59.0-beta.3` on **`dev`**, GitHub Release with `prerelease=true`; `main` is NOT touched and **nothing is copied to the home instance by hand** — HACS delivers it on the beta channel. Contents on top of beta.2: **furniture library** (docs/FURNITURE.md — ~30 top-view symbols at real size, wall magnet, independent width/depth); **hide decor / hide openings** space toggles (editors still show their own layer); **card-mod styling hooks** (docs/STYLING-HOOKS.md — stable `data-hp`/`data-id`/…); HA `formatEntityState` value formatting; bead-sized corner handles; backdrop editor opens on its tool; virtual walls follow the room-border switch; audit P0/P3 follow-ups (write policy, README, validation). **Wall thickness** approved as spec only (docs/WALL-THICKNESS.md), not coded. Previous pre-release: v1.59.0-beta.2; previous stable: v1.58.0 |
