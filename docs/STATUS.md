...
| Current local cycle | On top of v1.73.0-beta.4, #484 preserves a complete reconstructable PDF exterior-dimension chain around rectangular facade steps. #486 adds the primary `/houseplan` sidebar panel around the same full-card model, with fail-soft owned backend registration, a separate bounded panel entry, read-only empty-state safety and full-width Sections default for the optional dashboard card. The card graph and saved plan/config/layout models remain unchanged. Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
