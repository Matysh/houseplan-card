...
| Item | State |
|---|---|
| Version | **v1.67.0-rc.3 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.67.0-rc.2 is published. Current `dev` closes multi-wall junctions without wedges (#302), keeps the complete wall-axis/endpoint overlay identical across every Plan tool (#304), adds opening-entity search (#301), preserves import-reference lineage across repeated imports (#265), and renders wall-thickness hover at the real physical width (#303). v1.67.0-rc.3 packages these reviewed S8 changes and remains gated by the exact-SHA Validate before publication. |
