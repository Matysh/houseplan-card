...
| Item | State |
|---|---|
| Version | **v1.74.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | v1.74.0-beta.3 continues the line after the stable v1.73.0. User-visible in this beta: dragging the plan is smooth again — a gesture frame now moves what is already drawn by a composited transform and the scene re-rasterizes on a budget (100 ms, or a 15 % shift) instead of on every frame, which on the owner’s 144 Hz Firefox cost 200 ms per frame with the refresh driver skipping 124-144 ticks a second waiting for paint (#531); and the printed plan is about a third larger, because the in-plan dimension labels shrank by a quarter and the separate column of external dimensions is gone, so the drawing keeps a whole scale step (#530). Everything from v1.74.0-beta.2 stays in the line: #529, #524, #525, #526, plus internal #528 and #518. The speed verdict for #531 is deliberately NOT claimed by the sandbox — headless Firefox and Chromium hold 60 fps on both the old and the new path; it is owed by a Firefox profile from the owner’s machine on the same pan. Publication requires green Validate on the exact candidate SHA; `main` stays at v1.73.0. |
