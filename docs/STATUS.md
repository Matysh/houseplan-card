...
| Item | State |
|---|---|
| Version | **v1.73.0** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | Stable promotion of the nine published v1.73.0 betas, through v1.73.0-beta.9. Highlights: the native HA sidebar page (#486), configurable designer-aligned summary panel (#437/#505), PDF export (#53), presence-radar Stage 1 (#485), room comfort limits (#487), wall-chain simplification (#478/#477) and reliability improvements. beta.9 removed the startup regression that Full Performance caught on the first promotion attempt (#506): a warm summary runtime now attaches before the first render. No new product behaviour is added during promotion. Publication requires the complete local gate, green Validate and Full Performance against the v1.72.0 product on the exact release SHA; stable assets also require the compositor-continuity check. |
