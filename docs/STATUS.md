...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.6** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | Development after v1.73.0-beta.6 adds the configurable read-only summary overlay (#437): shared versioned blocks and sources, per-user/per-card show and View sizes, native mobile/fit hiding, authoritative device and clean-floor totals, and a lazy settings form. The feature is still on its issue branch pending code review; publish from `dev` only after exact-SHA Validate is green, while `main` remains at v1.72.0. |
...
- **Configurable summary overlay** (#437, current issue branch): a two-part
