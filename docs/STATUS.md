...
| Item | State |
|---|---|
| Version | **v1.69.0-beta.2 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.68.1 is the current stable release. v1.69.0-beta.2 packages the reviewed `S8-merged` work since beta.1: the expanded furniture library (#159), room-aware climate values (#317), physical-controller state parity (#318), resilient lazy editor delivery (#353), revision-safe configuration and layout writes (#340, #356), plus supporting fixes and test infrastructure. Publication remains gated by exact-SHA Validate. |
