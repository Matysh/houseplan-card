...
| Item | State |
|---|---|
| Version | **v1.70.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.70.0-beta.3 is the prerelease candidate for the private Help & feedback flow, accessible confirmation-shell fixes, safe rollback of failed HA Area relocation, and their browser/golden guard hardening. `main` remains on the stable line; publication is gated by the exact-SHA Validate and the local prerelease contract. |
