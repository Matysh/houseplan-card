...
| Item | State |
|---|---|
| Version | **v1.72.0-beta.1** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.72.0-beta.1 adds contextual per-device Zigbee topology diagnostics (#54), click/tap room fitting with 10% margins (#152), exterior wall-face furniture snapping and exact one-cell keyboard nudging for Background objects (#447). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.71.0. |
