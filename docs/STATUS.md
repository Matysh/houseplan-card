...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.3 adds a clean one-page architectural PDF export for the current space, with optional furniture, decor and backdrop (#53). The #482 update makes that export print-ready: compact dialog, horizontal/vertical locally deduplicated dimensions, hatched grey walls, complete-scene fitting, vector compass and no symbol legend. The changed-mutants gate keeps a journal of proven witnesses and avoids repeatedly judging the complete accumulated `dev` range (#481). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
