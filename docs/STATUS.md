...
| Item | State |
|---|---|
| Version | **v1.55.1** everywhere (manifest, const.py, package.json, CARD_VERSION); deployed to the home instance |
...
| Home instance | ha.jbstudio.pro (SSH port **22222**, key `ha_jb`; HA config root is `/mnt/data/supervisor/homeassistant` — `/config` does NOT exist in this SSH environment), deployed **v1.55.1** via direct copy (HACS custom repo also installed) |
