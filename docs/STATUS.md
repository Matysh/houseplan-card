...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.5 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — fifth prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.4 is published. Current `dev` includes stable wall-thickness keys across storage round-trips (#258), bounded masonry at mixed-thickness T-junctions (#261), and child-entity re-add after deleting its complete HA device (#262). v1.67.0-beta.5 packages these reviewed S8 changes and remains gated by the full exact-SHA Validate before publication. |
