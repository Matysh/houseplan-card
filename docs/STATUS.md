...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.1** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.1 keeps active Zigbee topology readable above room names and unrelated markers while preserving linked-device markers on top (#464). The same candidate carries hidden isometric Stage 3 (#160) without public enablement or changelog entry. Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
| Hidden Alpha Stage | #89 Stage 1 ships in v1.63.0-beta.1, #122 Stage 2 in v1.64.0 and #160 Stage 3 in v1.73.0-beta.1. The same hidden `iso` view now uses the fixed 4° camera, raised/tethered device-room-lock overlays, deeper openings and bounded theme materials. Since #448 the experiment is enabled only through the single indefinite browser-local `hp_alpha` gate; it is not expiring and has no per-stage key. Flat remains default; editors, `houseplan-space-card`, floor effects, stored coordinates and HA actions remain unchanged. Stage 3 stays internal and is absent from public changelog/user documentation. |
