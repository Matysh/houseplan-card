...
| Item | State |
|---|---|
| Version | **v1.72.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.72.0-beta.3 keeps the heavy scene stable during hover, pan, pinch and editor drags, filters unrelated Home Assistant updates, and preserves exact live resize validation on large plans (#451). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.71.0. |
