...
| Item | State |
|---|---|
| Version | **v1.65.0-beta.9** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate for the current `S8-merged` queue |
| Current local cycle | v1.65.0-beta.9 candidate carries #213. Device marker cores stay concentric at fractional scales, MDI glyphs are larger, the complete value capsule owns hover and the configured action, opening lock badges follow the accepted artwork, and marker LQI again uses the continuous colour scale. Publication waits for a green exact-SHA Validate, reviewed Linux golden baselines for the intentional marker and lock-badge changes, and the fail-closed prerelease preflight. v1.65.0-beta.8 is published and stable v1.64.0 remains unchanged. |
