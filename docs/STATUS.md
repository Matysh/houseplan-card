...
| Current local cycle | v1.68.1 is the current stable release. Active branch `issue/29-device-inbox-lifecycle` replaces the Device editor's separate Add/hidden paths with one searchable lifecycle catalog. The catalog is a read-only projection of existing markers, tombstones and HA registry state; no persisted model or discovery semantics change. |
