...
| Item | State |
|---|---|
| Version | **v1.70.0** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.70.0 is the promotion-only stable candidate for the published beta.1–beta.4 line: smoother camera and furniture editing, safer and clearer device workflows, localized help/errors/confirmations, and the private Help & feedback flow. No new product behaviour is added after beta.4; the byte-identical main-only #413/#416 workflow mirrors have been reconciled into `dev`, and publication remains gated by the complete local suite plus exact-SHA Validate and Full Performance. |
