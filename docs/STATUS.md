...
| Item | State |
|---|---|
| Version | **v1.65.0-beta.6** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate for the current `S8-merged` queue |
| Current local cycle | v1.65.0-beta.6 candidate carries #179, #198, #204 and #205. Device markers use the designer-supplied light/dark shell and complete state system; Optimize removes only a proven isolated sub-half-step wall-thickness island; new-space File/Draw border/name defaults are visible and preserve a user-touched pair; and one same-map vacuum trail survives available stops up to 30 minutes. Publication waits for a green exact-SHA Validate, reviewed Linux golden baselines where the intentional icon redesign changes pixels, and the fail-closed prerelease preflight. v1.65.0-beta.5 is published and stable v1.64.0 remains unchanged. |
