...
| Item | State |
|---|---|
| Version | **v1.69.0-beta.1 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.68.1 is the current stable release. v1.69.0-beta.1 packages the reviewed `S8-merged` work since it: the searchable Device lifecycle catalog (#29), resilient and faster wall-junction limits (#329–#331), lazy editor runtime (#337), scalable i18n registry (#62) and the complete German locale (#348), together with supporting refactors and release/test infrastructure. Publication remains gated by exact-SHA Validate. |
