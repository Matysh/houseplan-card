...
| Item | State |
|---|---|
| Version | **v1.64.0-beta.3** everywhere (manifest, const.py, package.json, CARD_VERSION) — performance-repair candidate after the stable gate exposed #156 |
| Current local cycle | v1.64.0-beta.3 preserves the published beta feature line and removes the Full Performance regressions from #156: runtime consumers no longer build an unused physical-wall union on every floor switch, and initial Plan snap hover updates only its active marker. The owner explicitly authorized implementation and self-review in one Codex session while Claude/status automation was unavailable. After a green exact-SHA beta, stable v1.64.0 is promoted again without product-code changes. The independent #124 view-toggle debt remains open under the owner's accepted exception. |
