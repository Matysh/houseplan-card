...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.4 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — fourth prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.3 is published. Current `dev` includes bounded multi-wall junctions (#249), controller availability independent from unavailable targets (#251), proven-orphan cleanup in Optimize (#252), lossless partial wall Resize (#253), and projected layout diagnostics for `houseplan/config/get` (#256). v1.67.0-beta.4 packages these reviewed S8 changes and remains gated by the full exact-SHA Validate before publication. |
