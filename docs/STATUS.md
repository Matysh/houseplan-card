...
| Item | State |
|---|---|
| Version | **v1.76.0** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Stable v1.76.0** — `main` is fast-forwarded to the tested `dev` SHA and the GitHub Release is public. The line aggregates from the stable v1.75.0: pinch-zoom in the Home Assistant app no longer tears the picture apart — large “Follows the Sun” plans lost their fixed white gaps (#582), the plan no longer moves between compositor paths mid gesture (#579), and a pinch that starts on a device marker no longer runs that device (#578, #563); sunlight can now start at the outer window corners as well as the inner ones (#577); “As the space” returns the room colour and heals rooms stuck with a foreign one (#581); closely placed markers stop stealing one another's tap and keyboard focus gets the same tooltip as hover (#564, #565); a plan edited in another browser no longer rolls back (#543) and dragging leaves no empty strip at the edge (#544). Smaller: #561, #554, #545, #580. Internal in the line: #583 (2.5D refinement plus the overlay-placement speed fix), #547, #556, #560, #566, #567, #568, #569. Recorded rather than silenced: the screenshot-freshness index is stale since `699ab471` and the Validate gate that was supposed to catch it only warns — the mode selection compares against a multi-line CLI output (#586). The eleven documentation frames cannot change from that commit: none of them renders the hidden 2.5D view, and the fingerprint covers all of `src/**`. |
