...
| Item | State |
|---|---|
| Version | **v1.74.0-beta.1** everywhere (manifest, const.py, package.json, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | v1.74.0-beta.1 opens the line after the stable v1.73.0. User-visible: the summary panel no longer freezes the first frame on a large plan and no longer shows "Source unavailable" while its values are still being computed (#509), its settings dialog scrolls again with the wheel and with a swipe (#508), and the alignment guides follow the gesture again in all three editors — the marker, the shape and the drawing cursor (#521, a regression that shipped in v1.72.0 and v1.73.0). Everything else in this beta is internal: one owner for config/layout identity and the adoption sequence (#500) plus the cold-start regression it left behind — the authoritative adoption now keeps its own task and the first stable frame of a 60-room house is back inside its budget (#520) — and the process/CI work: review waits for a green Validate with diff mutants on the exact material (#510), merge-candidate publishes and validates the merge candidate (#492), the release gate judges the latest non-cancelled run (#511), the version text left the golden frames behind a seam with a local `docs:accept --identical` path (#512), the full mutation registry moved to a nightly schedule (#513), specs live in the issue body (#517), and the Windows gate is green with CI-pinned toolchain (#496). Publication requires green Validate on the exact candidate SHA; `main` stays at v1.73.0. |
