...
| Item | State |
|---|---|
| Version | **v1.62.0-beta.1** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate with portable backups, reusable help and passive light-source controls |
| Current local cycle | The v1.62.0-beta.1 candidate contains issues #50, #68, #81, #84, #87 and #88. #84/#88 add passive forced light sources, validated `marker:*` controller links, cross-space ownership, explicit leading entities for multi-channel devices and transfer-safe link remapping; the complete truth table is `docs/DEVICE-LIGHT-SETTINGS-MATRIX.ru.md`, while `docs/LIGHT.md` remains canonical for light geometry and source identity. #50 preserves the actual export model version, globally bounds preview memory, rechecks plan files at apply and retries/rolls back paired Store failures without surprise recovery; #68 shares one fallback-surface controller, keeps hover focus stable and includes portal controls in the dialog focus trap. HA endpoint mutation guards cover detach, remap, metadata, event order and pair failure; Windows can compile them but the HA harness remains CI/Linux-only. Opening tunnels use one non-overlapping room-coloured contour per connected physical span with a direct max-depth union mutation test. |
