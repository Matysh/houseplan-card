...
| Current local cycle | v1.73.0-beta.1 keeps active Zigbee topology readable above room names and unrelated markers while preserving linked-device markers on top (#464). Unreleased work removes painted rectangular plates around raised device, room and lock overlays while retaining their invisible collision/fit footprint (#471), and makes completed editor writers converge with Optimize: bounded wall-chain finish, room-reference transactions and continuous furniture/image transforms (#477, building on #478). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
