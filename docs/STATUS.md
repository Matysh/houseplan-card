...
| Current local cycle | v1.69.0 is the stable promotion of the published beta.1–beta.5 line: German and French localization, editors outside the initial View graph, 56 furniture types and the expanded Background workflow, live-opening Glow plus opt-in Glow/house framing in `houseplan-space-card`, and an explicit source selector for Value + state. The stable promotion contains no new product behaviour and remains gated by the complete local suite plus exact-SHA Validate and Full Performance. |
