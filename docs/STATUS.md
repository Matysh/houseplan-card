...
| Item | State |
|---|---|
| Version | **v1.76.0-beta.5** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.76.0-beta.5 candidate** — prepared from the exact integrated `dev` tree; `main` remains on v1.75.0. It fixes fixed white gaps during pinch-zoom on large “Follows the Sun” plans in the HA app (#582), outer-corner sun rays shining through thick walls (#580) and a room keeping its own colour after “As the space” — orphan room colours now heal on read (#581); the hidden 2.5D presentation is refined internally (#583). First beta whose bookkeeping closes issues from the immutable `RELEASE-MEMBERSHIP.json` of the candidate (#547). The first candidate was held by its own performance gate: the #583 refinement made the 2.5D overlay collision resolver scan a 1 px disc of offsets and ask every wall about every candidate — resize 344 → 3635 ms, pan/zoom 37 → 1244 ms against the tree before it. Measurement, not guesswork, named the cost (98 % of positions rejected by walls); the search now walks a 4 px lattice and refines to 1 px around the winner, group pass 9584 → 839 ms with identical resolution quality, and this commit re-marks the candidate so the heavy gates re-run on the fixed SHA (§11.4). |
