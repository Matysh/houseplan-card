...
| Current local cycle | After v1.61.0-beta.6, #73 visual continuity is implemented locally alongside the deterministic #79/#80 fixes and the beta.6 review corrections. Full and compact cards share one tokenised controller, keep the last coherent frame through resume/reconnect/remount/resize, adopt config/layout by revision plus content fingerprint, share loaded signed assets and expose a delayed localized recovery fallback only when no complete frame exists. Deterministic unit/frame-sequence coverage is added but not run yet under the owner's local-edit rule. Room hover remains split into a neutral clean-floor wash below Glow/sun/walls and a late outline above walls; the device CSS cascade restores common hover and neutral elevation shadow. Glow now fails dark on clipping errors and sources placed on barriers, normalizes the angular seam, fingerprints complete barrier geometry per space, freezes feather parameters through viewport gestures and has a semantic golden assertion that the receiving room is actually lit. Opening/space footers use the shared responsive action groups. `docs/LIGHT.md` remains canonical. |
