...
| Item | State |
|---|---|
| Version | **v1.66.0-beta.1 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — first prerelease of the v1.66 line |
| Current local cycle | v1.66.0-beta.1 prepares three reviewed fixes after v1.65.0: capsule-shaped Text marker outlines, resilient Glow floor clipping and the unified red/green lock plus theme-aware orange-core palette. Publication remains gated by the full exact-SHA Validate, including reviewed Linux golden baselines, smoke and performance-smoke. |
