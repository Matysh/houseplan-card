...
| Item | State |
|---|---|
| Version | **v1.72.0-beta.5** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.72.0-beta.5 makes wall-chain drawing on large plans faster (#461), adds observable frontend/integration version recovery (#462), and keeps native/fallback dialogs modal and centred after reconnects (#463). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.71.0. |
