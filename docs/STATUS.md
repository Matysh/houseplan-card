...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.7** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.7 adds the configurable read-only summary overlay (#437), keeps its live values and settings responsive on large installs (#490/#493), gives Optimize/Undo/import/delete one crash-resumable config/layout commit protocol (#491), and publishes stable `data-hp-*` UI hooks for external tests and advanced card-mod rules (#489). Supporting release/process infrastructure is hardened by #492/#499. Issue #485 Stage 1 adds optional presence-radar source binding, physical installation/calibration and a bounded room-clipped live layer on its issue branch; it remains unshipped until the normal S7 review and dev pipeline complete. The card graph and stored plan geometry remain compatible; publish from `dev` only after exact-SHA Validate is green, while `main` remains at v1.72.0. |
...
- **Presence radars (#485 Stage 1, issue branch):** an optional marker-owned,
