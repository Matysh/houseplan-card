...
| Item | State |
|---|---|
| Version | **v1.76.0-beta.1** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.76.0-beta.1 candidate** — the first beta after stable v1.75.0 is prepared from `dev`; `main` remains on v1.75.0. User-visible fixes keep the newest accepted plan and its instant-start cache together during overlapping reloads (#543), keep the already rendered scene covering the card throughout pan and pinch gestures (#544), restore the complete radar source inventory in preview and permission checks (#545), avoid double-counting a staged attachment against the disk reserve (#554), and give Masonry cards stable summary-panel preference identities across reloads (#561). Internal work in the candidate unifies exact-SHA CI proof and its inputs (#541, #542, #548), hardens mutation and nightly execution (#549, #550), splits mutation declarations/planning/evidence/execution while caching guard inputs per invocation (#558), makes review waits, stages, reconciliation and process documentation deterministic (#546, #551, #553, #555, #562), binds beta bookkeeping to immutable candidate membership (#547), and pins reproducible local Windows/WSL toolchains (#557). |
