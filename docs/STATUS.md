...
| Item | State |
|---|---|
| Version | **v1.77.0-beta.3** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.77.0-beta.3 candidate** — prepared from the exact integrated `dev` tree; `main` stays on v1.76.0. User-visible: the Space, General settings, Room settings and Device on the plan dialogs now match the designer reference instead of wrapping the old form in new cards: shared controls, one scrollbar, explicit validation, dirty-state Save and discard confirmation (#600). |
