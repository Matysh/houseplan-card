...
| Item | State |
|---|---|
| Version | **v1.70.0-beta.5** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.70.0-beta.5 is the gate-repair prerelease candidate for the beta.1–beta.4 line. It keeps #126's first authoritative HA Area provenance write out of the geometry epoch, returning the 20-room cold-load benchmark to the v1.69.0 level; the exact cache ceiling is now 100. After exact-SHA Validate this beta is published, then promoted to v1.70.0 only after Full Performance is green on the exact `main` SHA. |
