...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.6 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — sixth prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.5 is published. Current `dev` includes finite short rays at degree-3+ wall nodes (#271), hole-free bounded multi-wall bevels (#272), topology-safe thickness-island cleanup in Optimize (#273), and matching wireless-controller presentation on the plan and in preview (#274). v1.67.0-beta.6 packages these reviewed S8 changes and remains gated by the full exact-SHA Validate before publication. |
