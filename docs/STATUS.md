...
| Current local cycle | v1.68.1 is the current stable release. Active branch `issue/267-device-presentation-table` is a refactor-only cycle: the marker face now has one pure ordered policy plus a canonical executable decision matrix, stable internal trace and mutation evidence. User-visible behaviour, config and renderer DOM/CSS remain unchanged. |
