...
| Item | State |
|---|---|
| Version | **v1.65.0** everywhere (manifest, const.py, package.json, CARD_VERSION) — stable promotion of the published v1.65.0-beta.9 product line |
| Current local cycle | v1.65.0 promotes the exact product behaviour already published through beta.9: the unified Plan wall/opening workflow, more precise Home Assistant bindings and device states, plan-only export and fixed-floor cards, plus the complete light/dark device-marker redesign and interaction polish. The stable commit contains only version fields, generated snapshots and release metadata; stable publication is gated by the full local suite and green exact-SHA Validate and Full Performance workflows. |
