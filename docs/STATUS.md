...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.7** everywhere (manifest, const.py, package.json, CARD_VERSION) — seventh prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.7 is published. The v1.67.0-beta.8 candidate contains exact explicit-Optimize reconciliation of a redundant partition over one solid shared room wall (#276), fixed-topology Resize that moves only safe horizontal/vertical walls and affects at most two exact endpoint-matched rooms (#277), and component-aware wall-union isolation plus a strict transaction barrier for all physical geometry edits (#278). Publication follows green #278 code review and exact-SHA prerelease gates. |
