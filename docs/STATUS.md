...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.7 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — seventh prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.6 is published. Current `dev` preserves complete finite wall strips at perpendicular T/X multi-wall junctions, including neighbouring nodes with overlapping processing regions (#275). v1.67.0-beta.7 packages this reviewed S8 fix and remains gated by the full exact-SHA Validate before publication. |
