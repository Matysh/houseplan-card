...
| Item | State |
|---|---|
| Version | **v1.77.0** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Stable v1.77.0 candidate** — promotion of the published beta line from v1.76.0. The line delivers a 60-symbol furniture library and dimension parity (#593, #584), redesigned settings dialogs (#594, #598, #600), and a value plus static icon mode (#588). Beta-only polish (#602, #603, #605, #606) is included without presenting it as a new stable feature. Internal CI mutation-gate shards are bounded and evidence-checked (#604). The exact SHA still requires Validate, Full Performance and stable release gates before publication. |
