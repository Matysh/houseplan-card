...
| Current local cycle | v1.73.0-beta.7 adds the configurable read-only summary overlay (#437), keeps its live values and settings responsive on large installs (#490/#493), gives Optimize/Undo/import/delete one crash-resumable config/layout commit protocol (#491), and publishes stable `data-hp-*` UI hooks for external tests and advanced card-mod rules (#489). Presence-radar Stage 1 (#485) adds optional source binding, physical installation/calibration and a bounded room-clipped live layer. Supporting release/process infrastructure is hardened by #492/#499. The card graph and stored plan geometry remain compatible; publish from `dev` only after exact-SHA Validate is green, while `main` remains at v1.72.0. |
