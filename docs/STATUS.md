...
| Item | State |
|---|---|
| Version | **v1.67.0-beta.2 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) — second prerelease of the v1.67 line |
| Current local cycle | v1.67.0-beta.2 is published. Current `dev` includes orphan-reference repair (#244) and idempotent Optimize storage/reload handling (#248); the #250 issue branch additionally makes every opening symbol remain on the wall centreline regardless of `flip_v`. The next prerelease remains gated by review and the full exact-SHA Validate. |
