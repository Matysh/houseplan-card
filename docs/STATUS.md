...
| Item | State |
|---|---|
| Version | **v1.65.0-beta.8** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate for the current `S8-merged` queue |
| Current local cycle | v1.65.0-beta.8 candidate carries #212. Device markers are 10% more compact without moving their saved centres or shrinking hit targets, accepted actions have brief press feedback, and touch/pen hover no longer sticks across the plan and shared controls. Publication waits for a green exact-SHA Validate, reviewed Linux golden baselines for the intentional marker resize, and the fail-closed prerelease preflight. v1.65.0-beta.7 is published and stable v1.64.0 remains unchanged. |
