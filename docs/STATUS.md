...
| Item | State |
|---|---|
| Version | **v1.64.0** everywhere (manifest, const.py, package.json, CARD_VERSION) — stable promotion after the published performance-repair beta.3 |
| Current local cycle | v1.64.0-beta.3 is published and #156 is closed after exact-SHA Validate, release and HACS-discovery success. Stable v1.64.0 promotes those identical product changes without product-code edits: runtime consumers no longer build an unused physical-wall union on every floor switch, and initial Plan snap hover updates only its active marker. The owner explicitly authorized #156 implementation and self-review in one Codex session while Claude/status automation was unavailable. The independent #124 view-toggle debt remains open under the owner's accepted exception. |
