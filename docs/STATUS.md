...
| Current local cycle | Stable promotion of the eight published v1.73.0 betas, through v1.73.0-beta.8. Highlights: the native HA sidebar page (#486), configurable designer-aligned summary panel (#437/#505), PDF export (#53), presence-radar Stage 1 (#485), room comfort limits (#487), wall-chain simplification (#478/#477) and reliability improvements. No new product behaviour is added during promotion. The main-only workflow mirrors were merged without changing the beta.8 tree. Publication requires the complete local gate, reviewed golden frames where version text changes, and green Validate plus Full Performance on the exact release SHA; stable assets also require the compositor-continuity check. |
