...
| Current local cycle | v1.73.0-beta.1 keeps active Zigbee topology readable above room names and unrelated markers while preserving linked-device markers on top (#464). The following hidden-alpha correction removes painted rectangular plates around raised device, room and lock overlays while retaining their invisible collision/fit footprint (#471). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
