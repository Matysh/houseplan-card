...
| Current local cycle | v1.68.0 promotes the exact product behaviour already published through beta.4: stable wall-segment identity and atomic model-v9 migration, unified zero-thickness walls, repaired junction rendering and reliable plan editing. The complete local suite and exact-tree Validate are green. Stable-gate diagnostics exposed two infrastructure-only defects: legacy v1.66.0 lacked the current bundle materializer, and five full 7-sample profile pairs could not fit a 60-minute serial job. Old baselines now use an equivalent fresh-bundle fallback, while independent profile pairs run in parallel and retain same-runner sequential base/candidate comparison. These repairs change no shipped bundle or product behaviour; their exact-SHA Validate and Full Performance remain the publication gates. |
