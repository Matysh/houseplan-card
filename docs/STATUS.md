...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.4** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | On top of v1.73.0-beta.4, the pending #484 fix preserves a complete reconstructable external dimension chain around rectangular facade steps while retaining strict post-exit wall-collision rejection. PDF unit, browser-download, mutation and dedicated Linux golden/semantic witnesses cover the regression. The interaction performance smoke aggregate keeps hosted-runner headroom while retaining the unchanged per-series, Long Task, heap, cache and structural gates (#483). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
