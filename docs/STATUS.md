...
| Item | State |
|---|---|
| Version | **v1.62.0-beta.7** everywhere (manifest, const.py, package.json, CARD_VERSION) — prerelease candidate |
| Current local cycle | v1.62.0-beta.7 keeps the device editor's persisted Display, Room and leading-light selections aligned with their first rendered native selects, adds a source-level guard against unsafe select bindings and extends fresh-dialog browser coverage. Issues #89 and #103 remain specification/backlog-only. |
