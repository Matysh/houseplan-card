...
| Item | State |
|---|---|
| Version | **v1.76.0-beta.2** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.76.0-beta.2 candidate** — prepared from the exact integrated `dev` tree; `main` remains on v1.75.0. User-visible fixes suppress delayed device activation after pinch (#563), arbitrate overlapping marker hit areas in favour of the painted marker under the pointer (#564), and expose current-space and device-tooltip state consistently to keyboard and assistive-technology users without changing room-label colours or touch/pen behaviour (#565). Internal work adds a field geometry/household regression corpus (#560), separates mutation registry, selection, evidence and execution (#558), hardens CI trust boundaries (#556), preserves semantically identical radar calibration sources (#567), aligns stale-layout invariant attribution with product ownership (#566), attributes mutation setup failures to the introducing tree (#568), and restores nine dead mutation witnesses (#569). |
| Hidden Alpha Stage | #89 Stage 1 ships in v1.63.0-beta.1, #122 Stage 2 in v1.64.0 and #160 Stage 3 in v1.73.0-beta.1; #570 adds the Stage 4 designer handoff. The same hidden `iso` view now uses a fixed 0°/20° camera and scale-aware wall height 84, low screen-facing device/room/lock overlays without tethers or ground dots, neutral full-height window frames with blue glass, and the existing bounded theme materials. Since #448 the experiment is enabled only through the single indefinite browser-local `hp_alpha` gate; it is not expiring and has no per-stage key. Flat remains default; editors, `houseplan-space-card`, floor effects, stored coordinates and HA actions remain unchanged. Stage 4 stays internal and is absent from public changelog/user documentation. |
