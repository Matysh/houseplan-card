...
| Current local cycle | v1.70.0 is the promotion-only stable candidate for the published beta.1–beta.5 line. Beta.5 fixed #126's first authoritative HA Area provenance write so it no longer advances the geometry epoch, returned the 20-room cold-load benchmark to the v1.69.0 level, and tightened the exact cache ceiling to 100. Publish stable only after exact-SHA Validate and Full Performance are green on `main`. |
