...
| Current local cycle | v1.73.0-beta.4 makes PDF export print-ready: compact dialog, horizontal/vertical locally deduplicated dimensions, hatched grey walls, complete-scene fitting, vector compass and no symbol legend (#482). The interaction performance smoke aggregate now has hosted-runner headroom while retaining the unchanged per-series, Long Task, heap, cache and structural gates (#483). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
