...
| Item | State |
|---|---|
| Version | **v1.73.0-beta.2** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.73.0-beta.2 makes every accepted wall-chain segment an ordinary saved wall, migrates legacy drafts, and brings completed editor writes to the same fixed point as Optimize (#478, #477). Color pickers have an explicit confirmation button (#476); hidden isometric overlay plates are no longer painted and the initial View graph loads furniture art lazily (#471, #474). Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
