...
| Current local cycle | **Beta v1.77.0-beta.1 candidate** — prepared from the exact integrated `dev` tree; `main` stays on v1.76.0. One user-visible fix: furniture pieces with equal declared sizes now have equal footprints on the plan and the “Stairs”/“TV” menu icons stay inside their bounds (#584). Internal in the line: the golden manifest records the capture platform rather than the acceptance one (#571); the CI proof separates the product tree from the accepted golden overlay and release consumers recompute it, so a baseline-only commit after a golden-red candidate reuses smoke, performance smoke, parity and backend and skips caught witnesses (#573); the candidate-only strict screenshot-freshness mode actually engages (#586); the stable Full Performance base is the previous stable tag instead of the `main` tip (#587). In implementation: #585 replaces the hidden 2.5D overlay's lossy 4 px lattice with iterative critical-boundary events, restores the dense Room 2.5 marker pair and returns both full isometric profiles to their ordinary performance allowances; Linux golden/performance evidence is still required before merge. |
