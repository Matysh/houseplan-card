...
| Item | State |
|---|---|
| Version | **v1.77.0-beta.4** everywhere (manifest, const.py, package.json, package-lock in both places, CARD_VERSION in the card and the editor runtime) |
| Current local cycle | **Beta v1.77.0-beta.4 candidate** — prepared from the exact integrated `dev` tree; `main` stays on v1.76.0. User-visible: the redesigned Space and Device dialogs receive a focused polish pass — aligned switches and rows, readable numeric fields, compact one-row desktop footers and inline presence-radar settings that preserve their unsaved draft (#602). Internal: beta/full Validate no longer repeats per-diff mutants already exercised on review and merge candidates; the nightly full mutation registry remains unchanged (#601). |
