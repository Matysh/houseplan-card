...
| Item | State |
|---|---|
| Version | **v1.71.0** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.71.0 is the promotion-only stable candidate for the published beta.1–beta.4 line: reusable private background images (#51), multi-floor vacuum map routing (#162), the room-tooltip option (#426), physical wall-face furniture snapping (#445), HACS naming (#444), and the reviewed reliability work. Publish stable only after exact-SHA Validate and Full Performance are green on the same `main` SHA. |
