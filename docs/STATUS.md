...
| Item | State |
|---|---|
| Version | **v1.61.0** everywhere (manifest, const.py, package.json, CARD_VERSION) — stable promotion of the published and validated v1.61.0-beta.8 candidate |
| Current local cycle | v1.61.0 promotes the tested beta series without product-source changes beyond the version constant. The release combines vacuum integration coverage, independent additive Glow and room-fill controls, geometry-based light propagation, and the #73 coherent-frame continuity contract. Full and compact cards use one bounded recovery barrier and immutable render-device snapshot, so reconnect, delayed protected backdrops and ordinary HA ticks cannot mix device, room, Glow, sun, opening, vacuum or live-text data from different frames. `docs/LIGHT.md` remains canonical. |
