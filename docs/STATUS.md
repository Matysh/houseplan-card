...
| Current local cycle | v1.73.0-beta.6 fixes the first mount of the dedicated `/houseplan` page in real Home Assistant (#488) and adds per-room lower/upper comfort-temperature overrides with independent inheritance from the space (#487). The card graph remains unchanged; only optional room fields extend the saved configuration. Publish from `dev` only after exact-SHA Validate is green; `main` remains at v1.72.0. |
