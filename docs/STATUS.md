...
| Item | State |
|---|---|
| Version | **v1.68.0-beta.4 candidate** everywhere (manifest, const.py, package.json, CARD_VERSION) |
| Current local cycle | v1.68.0-beta.3 is published but carries two write blockers; current `dev` fixes them: the stale-client guard accepts the first write of a newer wall model over an older document (#319), and the initial v9 migration auto-resolves every legacy border/opening conflict — the carrying atom keeps its thickness, ambiguity resolves deterministically, a carrier-less opening persists unhosted (#316). v1.68.0-beta.4 packages the reviewed S8 fixes and remains gated by exact-SHA Validate before publication. |
