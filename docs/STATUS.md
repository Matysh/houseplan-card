...
| Version | **1.78.0-beta.5** in all 7 version sources (`scripts/release-contract.mjs`) |
| Latest stable tag | `v1.77.0` |
| Latest prerelease tag | `v1.78.0-beta.4` |
| Tests | Node unit 3076 · pure backend 389 · HA-harness backend 300 · browser smokes 277 (`npm run inventory`) |
<!-- status-snapshot:end -->
...
| Item | State |
|---|---|
| Current local cycle | **Beta v1.78.0-beta.5 candidate** — prepared from the exact integrated `dev` tree. It keeps the 2.5D room metrics row at the flat-plan distance from the name at every zoom (#665), puts the active editor's close cross inside the highlighted tab (#666) and makes the static space card fully inert again (#664). `main` remains on stable v1.77.0. |
