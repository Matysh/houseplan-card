...
Если вкладка была скрыта не меньше 15 секунд, обычный режим Просмотра включает
короткий `_resumeSettling`: содержимое сцены остаётся за её уже правильным
фоном, пока размер не станет ненулевым и спокойным. После этого сохранённые
центр и zoom пересчитываются по окончательной геометрии и открываются одним
кадром. Памятка `warmBoot` во время промежуточного кадра не обновляется.
Быстрое переключение вкладок, редакторы и киоск в этот путь не входят. При
быстром возврате сохраняются также уже нарисованные day/night-фон и hover:
безусловный сброс этих полей создавал отдельный `skysnap`-кадр и визуальное
мигание даже после двух секунд в фоне.
