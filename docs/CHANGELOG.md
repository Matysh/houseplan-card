...
- The main toolbar is steadier and more compact: its editor close **×** now
  ...
  first dismissing its active tool, selection, drag, popover or dialog, without
...
- In the Plan editor, the **Room settings** button can now be moved temporarily
  ...
  normal click still opens settings. Cancelling a wall resize no longer resets
...
- Space navigation now exposes the current space to assistive technology, an
  ...
  the touch/pen behaviour are unchanged. Room hover no longer replaces a
  keyboard tooltip; hovering another device temporarily shows that device and
  restores the focused one when the pointer leaves
  ([#565](https://github.com/Matysh/houseplan-card/issues/565)).
...
- Switching between floors on a large plan is quick again. Every switch was
  ...
  cost about a tenth of a second of a frozen card each time. Nothing looks
  different, and the door that never moved still does not animate
  ([#534](https://github.com/Matysh/houseplan-card/issues/534)).

- The “Follows the sun” background no longer slows the plan down. The plan's
...
- Summary-panel entity values now update immediately even when the entity is
  ...
  Plan client instead of overwriting them on the next write, while keeping the
  current plan backdrop visible until a concurrent replacement is ready
...
- House Plan adds a private **Help & feedback** flow, localized backend errors
  and contextual help, plus one accessible confirmation experience for
  destructive actions ([#43](https://github.com/Matysh/houseplan-card/issues/43),
...
- Device workflows gain visible discovery filters, position Undo/Redo, automatic
  relocation to a linked Home Assistant Area, a **Do nothing** tap action, and
  safer binding edits ([#44](https://github.com/Matysh/houseplan-card/issues/44),
...
- Camera moves are now smooth, furniture can be resized, mirrored and rotated
  precisely, and tight house framing ignores hidden architecture; compatibility
  guards also protect old and future plan configs
...
- Resize now moves only a provably safe horizontal or vertical room wall. An
  ...
  the space masonry disappear. If persisted thickness or virtual-span metadata
  cannot be moved losslessly, the wall visibly stops at the last safe position
