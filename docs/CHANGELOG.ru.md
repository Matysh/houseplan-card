...
- Статическая карточка пространства (`houseplan-space-card`) больше не
  ...
  единственный кликабельный элемент — кнопка в подвале. Регрессия пришла с
  исправлением перекрытий маркеров в v1.76.0-beta.2
...
- При холодной загрузке дашборда включённый объёмный план (2.5D) больше не
  ...
  фактического цвета бумаги плана, в том числе в киоске; при реальной ошибке
  загрузки рендера по-прежнему открывается безопасный плоский вид, а карточка не
  зависает
  ([#654](https://github.com/Matysh/houseplan-card/issues/654)).
- Значки устройств и замков дверей на объёмном плане (2.5D) больше не меняют
  положение в сцене при приближении, отдалении и перемещении плана. Близкие
  значки теперь отодвигаются от стены одной жёсткой группой: ряды и интервалы
  внутри них остаются такими же, как на обычном плане, вместо превращения в
  диагональ или новой раскладки на другом масштабе
  ([#651](https://github.com/Matysh/houseplan-card/issues/651)).

## v1.78.0-beta.3 — 2026-09-25

- Объёмный план (2.5D) теперь доступен всем: **Общие настройки → Отображение →
  Объёмный вид плана (2.5D)** включает его один раз для всей установки — во
  всех пространствах, на всех устройствах и в киоске, — и после сохранения
  Просмотр меняется сразу. Значки устройств становятся приподнятыми плитками
  на 12 % крупнее с мягкой тенью на полу и рамкой, которая повторяет плитку
  при наведении, фокусе и выборе; из окон на пол ложится мягкий свет по
  реальному солнцу (длиннее при низком солнце, янтарный с тонкими лучиками на
  светлом полу). Стены сохраняют цвет из общих настроек в светлой и тёмной
  теме одинаково, а линии мебели той же толщины, что на обычном плане. Кнопка
  с кубиком в шапке, пункт меню телефона и скрытый вход `hp_alpha` убраны;
  обычный план остаётся по умолчанию и не меняется
  ([#649](https://github.com/Matysh/houseplan-card/issues/649)).
- Полную карточку House Plan в дашборде Sections теперь можно менять по высоте
  стандартной ручкой Home Assistant. Начальный размер — 10 строк, минимум — 6;
  просмотр и все три редактора заполняют выбранное место, не дёргая дашборд.
  Явный размер Home Assistant остаётся главным, а существующая auto-height
  карточка Sections один раз получает новый размер по умолчанию — 10 строк.
  Спасибо @pando80 за сообщение об ограничении и исходное направление решения
...
- На телефоне (окно не шире 480 px) шапка теперь занимает одну строку:
  вкладки пространств, масштаб и одна шестерёнка **«Действия и настройки»**.
  Вкладки прокручиваются вбок, текущая всегда на виду; заголовок карточки
  скрыт. В меню шестерёнки — всё, чего больше нет в строке: редакторы,
  настройки и добавление пространства, общие настройки, PDF, помощь,
  настройки и показ сводной панели; каждый пункт делает ровно то же, что
  прежняя кнопка. Касание вне меню только закрывает его. Пользователь без
  права редактирования видит только пункты сводной панели; на широком экране
  и в киоске ничего не изменилось
...
- Основная панель больше не показывает счётчик устройств («37 устр.») и не
  ...
  своём постоянном месте сразу после кнопок режимов, а вне редакторов это
  место остаётся пустым. Попасть в × так же легко (зона 24 × 24 px), и он
  виден и на окнах средней ширины. Значение **«Количество устройств»**
  сводной панели не изменилось
  ([#647](https://github.com/Matysh/houseplan-card/issues/647)).

## v1.78.0-beta.2 — 2026-09-25

- В редакторе плана кнопку **«Настройки комнаты»** теперь можно временно
  перетащить в другое место внутри комнаты, если она перекрывает стену, проём
  или другой элемент. Положение действует до выхода из редактора и не меняет
  план; обычный клик по-прежнему открывает настройки. Отмена изменения размера
  стены больше не сбрасывает это временное положение
  ([#645](https://github.com/Matysh/houseplan-card/issues/645)).
- Файлы плана до 8 МБ теперь действительно загружаются: план отправляется по
  HTTP, а не через WebSocket Home Assistant, предел сообщения которого рвал
  соединение на любом плане больше примерно 3 МБ. Файл больше 8 МБ отклоняется
  сразу при выборе с сообщением, называющим предел; для растрового изображения
  карточка предлагает загрузить уменьшенную копию
  ([#617](https://github.com/Matysh/houseplan-card/issues/617)).
- Каталог **Устройства** теперь скрывает и показывает много устройств разом. На
  вкладках **На плане** и **Скрытые** отметьте строки или **Выбрать все (N)** —
  N учитывает все подходящие строки, в том числе за кнопкой **Показать ещё**, —
  и нажмите **Скрыть выбранные** / **Показать выбранные**. Весь пакет — одно
  сохранение конфига и тост вида «Скрыто: 7»; неудачное сохранение ничего не
  меняет и оставляет выбор. Строки, отключённые или пропавшие в Home Assistant,
  выбрать нельзя ([#618](https://github.com/Matysh/houseplan-card/issues/618)).
- Плитки цвета в общих настройках (цвета заливки комнат и свечения) снова
  показывают прозрачность: сквозь цвет просвечивает шахматка, поэтому «Нет
  источников света» на 0 % больше не выглядит сплошным серым, а название
  плитки читается на том цвете, который виден. Плашки цвета остаются
  сплошными ([#615](https://github.com/Matysh/houseplan-card/issues/615)).

## v1.78.0-beta.1 — 2026-09-23

- В палитре мебели категория «Посудомоечная машина» теперь показывает новую
  фронтальную иконку; уже размещённые посудомоечные машины, их размеры и
  рисунок на плане не изменились
  ([#640](https://github.com/Matysh/houseplan-card/issues/640)).
- Пустое или неверное обязательное число в общих настройках и настройках
  пространства больше не подменяется прежним скрытым значением: поле сохраняет
  введённый черновик, показывает ошибку и блокирует сохранение. Обязательные
  имя виртуального устройства, цель запуска и источник бейджа теперь тоже
  проверяются прямо под полем вместо позднего уведомления; открытые диалоги
  сохраняют правильное состояние кнопки «Сохранить» после тёплого
  пересоздания карточки ([#614](https://github.com/Matysh/houseplan-card/issues/614)).
- Большой экспорт больше не задерживает обычные операции House Plan, а импорт
  не блокирует Home Assistant во время проверки вложений; заведомо невозможные
  загрузки отклоняются до передачи файла,
  проверка изображений ограничена по памяти, быстрые переключения виртуального
  света не создают лишних записей на диск, а неоднозначные дубликаты активных
  устройств отклоняются без блокировки старых конфигураций
  ([#625](https://github.com/Matysh/houseplan-card/issues/625)).
- Read-only экраны домочадцев и kiosk больше не показывают ошибку о правах
  администратора при появлении нового устройства в Home Assistant; конфликт
  конфига в режиме просмотра теперь безопасно обновляет данные, даже если
  редактор ещё не загружался
  ([#612](https://github.com/Matysh/houseplan-card/issues/612)).
- В вопросе о несохранённых настройках действия теперь названы однозначно:
  **«Вернуться»** сохраняет черновик, а **«Не сохранять»** сбрасывает его;
  иконка потери правок больше не выглядит как открытый замок
  ([#610](https://github.com/Matysh/houseplan-card/issues/610)).
- Числа рядом со слайдерами в настройках комнаты, пространства и устройства
  ...
  применяется после выхода из поля, привязывается к шагу шкалы и остаётся
  синхронным с ползунком
  ([#608](https://github.com/Matysh/houseplan-card/issues/608)).
- После прокрутки дашборда нажатие снова достаётся устройству прямо под
  пальцем, в том числе внутри вложенных панелей Home Assistant; pinch сохраняет
  итоговый масштаб один раз после жеста вместо записи в локальное хранилище на
  каждом движении ([#613](https://github.com/Matysh/houseplan-card/issues/613)).
- При импорте пространства маршруты карт робота теперь следуют за созданной
  копией: перенос в другую установку Home Assistant больше не падает из-за ID
  старого этажа, а копия в той же установке не оставляет трек робота на
  исходном этаже ([#611](https://github.com/Matysh/houseplan-card/issues/611)).
- Формы настроек теперь выглядят внутри Home Assistant так же, как в принятом
  макете: канва шириной 560 px на компьютере с одной прокруткой и ограничением
  высоты, а на телефоне — полноэкранный диалог без отступов. Остальные диалоги
  не изменились ([#609](https://github.com/Matysh/houseplan-card/issues/609)).
- Исправлен крестик реальных диалогов Home Assistant: после выбора
  **«Продолжить»** в вопросе о несохранённых изменениях тот же диалог снова
  виден и доступен для редактирования, а не остаётся невидимым до перезагрузки
  страницы ([#607](https://github.com/Matysh/houseplan-card/issues/607)).

## v1.77.0 — 2026-09-21

- Обновлена библиотека мебели: 60 символов в едином стиле, новые предметы и
  точные видимые габариты при прежних сохранённых размерах и расположении
  ([#593](https://github.com/Matysh/houseplan-card/issues/593),
  [#584](https://github.com/Matysh/houseplan-card/issues/584)).
- Четыре диалога настроек переработаны в карточки с контекстными подсказками,
  понятными переключателями и проверкой полей без изменения сохранённых
  настроек ([#594](https://github.com/Matysh/houseplan-card/issues/594),
  [#598](https://github.com/Matysh/houseplan-card/issues/598),
  [#600](https://github.com/Matysh/houseplan-card/issues/600)).
- Для устройств добавлен режим «Значение + статичный значок»: число или
  состояние видно, значок не перекрашивается и не пульсирует
  ([#588](https://github.com/Matysh/houseplan-card/issues/588)).

## v1.77.0-beta.5 — 2026-09-21

- Исправлена библиотека мебели: в «Прочем» доступен тренажёр, в «Растении»
  больше нет ошибочно подписанного тренажёра, а книжный шкаф и напольный
  стеллаж получили соответствующие названиям рисунки. Уже размещённый
  «Кактус» остаётся видимым без изменения сохранённых размеров, положения и
  оформления ([#606](https://github.com/Matysh/houseplan-card/issues/606)).
- Отполированы четыре диалога настроек: числа прозрачности в плитках видны
  полностью, свотчи сплошные, hex печатается заглавными буквами; у заливки
  стен появился сброс, действия с данными обведены, поле иконки больше не
  дублирует превью ([#605](https://github.com/Matysh/houseplan-card/issues/605)).
- Исправлена полировка диалогов: в подтверждении отмены правок кнопки больше
  не обрезаются, шарик переключателя ровно стоит в дорожке, а настройки комнаты
  не повторяют очевидный заголовок ([#603](https://github.com/Matysh/houseplan-card/issues/603)).

## v1.77.0-beta.4 — 2026-09-21

- Отполированы обновлённые диалоги настроек: тумблеры, строки слоёв,
  прозрачность и значения слайдеров теперь выровнены; убраны лишние подписи и
  сообщение о несохранённых изменениях; кнопки действий остаются удобными на
  узких экранах; настройки радара присутствия раскрываются сразу под тумблером,
  а его повторное включение до сохранения возвращает черновик
  ([#602](https://github.com/Matysh/houseplan-card/issues/602)).

## v1.77.0-beta.3 — 2026-09-20

- Четыре диалога настроек — «Пространство», «Общие настройки», «Настройки
  комнаты» и «Устройство на плане» — собраны по макетам дизайнера
  (`docs/design/600-settings-dialogs/`). Каждый диалог — одна форма 560 px с
  одной полосой прокрутки и названием пространства в заголовке; карточки
  плоские, с заголовком и «?». Внутри: полные строки с иконкой, подписью и
  тумблером справа (границы, слои, названия, свечение, подсказки, подтверждение,
  температура устройства); сегменты там, где короткий список был радио или
  выпадающим (заливка комнаты с короткими подписями — Свой цвет · Zigbee ·
  Свет · Температура, — нулевые стены, фон, лучи солнца и их источник,
  привязка устройства, роль источника света, режим свечения, сторона бейджа);
  плашки цвета — свотч, hex, число прозрачности и «Сбросить» — вокруг
  прежнего пикера; числовые поля с единицей внутри рамки (см, °C, м, %, °);
  слайдеры в паре с редактируемым числом и «Сбросить до 100%»; чипы связанных
  светильников и инструкций; выбор сущности раскрывается панелью внутри
  карточки — с поиском и чекбоксом «Показывать сущности». У заливки комнаты
  появился тумблер «Как у пространства»: выключили — открывается сегмент из
  пяти режимов, стартующий с текущего режима пространства; границы комфорта —
  два поля °C с легендой «Холодно / Комфорт / Жарко». **Сохранить** активна
  только при изменениях, закрытие с несохранёнными изменениями сначала
  спрашивает; сломанные значения (пустая или нечисловая граница, север вне
  0–359°, не выбранная привязка) названы под полем, а ссылка «Проверить N полей» в футере ведёт к первому. Строка
  «Спрашивать подтверждение» показывается только у действий; карточка «Связи
  Zigbee» больше не повторяет заголовок; ленивый редактор загружается с простым
  «Загрузка…». Все настройки, ключи и сохранённые значения прежние — переделаны
  формы, а не конфигурация
  ([#600](https://github.com/Matysh/houseplan-card/issues/600), закрывает
  [#591](https://github.com/Matysh/houseplan-card/issues/591) и
  [#599](https://github.com/Matysh/houseplan-card/issues/599)).

## v1.77.0-beta.2 — 2026-09-20

- Три диалога настроек — «Общие настройки», «Пространство» и «Устройство на
  плане» — собраны в карточки-группы с заголовками. Раньше это были плоские
  столбцы: в «Общих» девять подписей вперемешку с рядами и абзацами, в
  «Устройстве» вообще ни одного деления на 760 строк формы. Пояснения, которые
  лежали абзацами между полями, теперь открываются кнопкой «?» у своего
  заголовка или строки. Три выбора из коротких списков — фон плана в «Общих»,
  фон и вид нулевой стены в «Пространстве» — стали сегментированными
  переключателями вместо выпадающих списков: варианты видны сразу, попадание
  пальцем не требует раскрытия. Остальные списки в этих диалогах длинные или
  перечисляют сущности и остались списками. Значения, единицы, порядок полей и
  все действия прежние;
  предупреждения о сломанных настройках — что нет сущности `sun.sun` или что
  выбранная цель нажатия исчезла — остались на виду, а не уехали под «?»
  ([#598](https://github.com/Matysh/houseplan-card/issues/598)).

- Диалог настроек комнаты собран в четыре карточки-группы — «Основное»,
  «Заливка», «Источники» и «Размеры шрифтов», — и пояснение к каждой открывается
  кнопкой «?» рядом с заголовком вместо абзаца в форме. Источник температуры и
  влажности выбирается переключателем «среднее по комнате / выбранный датчик».
  Все значения, единицы, порядок полей и условия сохранения прежние: это
  перекладка формы, а не смена настроек
  ([#594](https://github.com/Matysh/houseplan-card/issues/594)).
- Все встроенные символы мебели перерисованы в одном стиле, и библиотека выросла
  до 60 видов сверху. Двенадцать предметов — холодильник, посудомоечная и
  стиральная машины, сушильная машина, кондиционер, бойлер, душ, раковина,
  лестница, камин, растение и ковёр — раньше рисовались программно и выбивались
  из общего стиля; теперь у них рисунок дизайнера. Появились четыре новых
  предмета: **компьютер**, **вытяжка**, **духовка** и **кактус**, а вместе с
  первыми тремя в палитре открылись три ранее пустые категории — всего их стало
  32. Кактус лежит в категории «Растение». Уже расставленная мебель остаётся на
  своих местах: положение, размер, поворот, отражение, цвет, прозрачность и
  толщина линии не меняются, размеры по умолчанию у всех прежних предметов тоже
  прежние — меняется только рисунок внутри рамки. Одно поведение стало хуже
  осознанно: если рисунки почему-то не загрузились, теперь не рисуется ни один
  предмет, а не 48 из 60, как было
  ([#593](https://github.com/Matysh/houseplan-card/issues/593)).
- В настройках устройства появился пятый вариант «Отображение» — **«Значение +
  статичный значок»**. Маркер показывает то же число или состояние, что и
  «Значение + состояние», и при этом остаётся одного цвета всегда: состояние,
  тревога, недоступность, цвет RGB-света и активность его не красят, пульсации
  нет. Компактные °/% и LQI и внешний бейдж со значением скрыты, как в режиме
  «Всегда статичный значок» (настройка бейджа сохраняется); у пылесоса не
  рисуются живой puck, след и предупреждение о маршруте, сохранённая история не
  удаляется. Карточка старой версии, встретив новый режим, покажет маркер как
  обычный цветной значок — конфигурация при этом не портится
  ([#588](https://github.com/Matysh/houseplan-card/issues/588)).

## v1.77.0-beta.1 — 2026-09-18

- Предметы мебели с одинаковыми размерами теперь и выглядят одинаково: указанные
  ширина и глубина совпадают с видимым габаритом на плане. Раньше рисунок
  занимал внутри своего бокса около 88 % стороны, поэтому «Кухонный напольный
  модуль» 60 × 60 см выглядел меньше «Посудомоечной машины» 60 × 60 см примерно
  на 14 % по стороне и на 30 % по площади. Уже расставленная мебель сохраняет
  координаты, размеры, поворот и зеркалирование — часть предметов станет
  визуально крупнее, и это и есть исправление
  ([#584](https://github.com/Matysh/houseplan-card/issues/584)).
- В меню мебели значки «Лестница» и «Телевизор» больше не выходят за свои
  границы: части рисунка уезжали из-за склейки контуров
  ([#584](https://github.com/Matysh/houseplan-card/issues/584)).

## v1.76.0 — 2026-09-16

- Щипковое масштабирование в приложении Home Assistant больше не рвёт картинку:
  на больших планах с фоном «Следует за Солнцем» пропали фиксированные белые
  провалы в стенах и полу, план перестал переключаться между режимами
  композитинга посреди жеста, а щипок, начатый на значке устройства, больше не
  запускает его действие и не открывает карточку House Plan вместе с окном Home
  Assistant ([#582](https://github.com/Matysh/houseplan-card/issues/582),
...
- В общих настройках появился выбор источника солнечных лучей: от внутренних
  углов окна (прежнее поведение) или от внешних
  ([#577](https://github.com/Matysh/houseplan-card/issues/577)).
- «Как у пространства» в настройках комнаты снова возвращает ей цвет
  пространства, а комнаты, застрявшие с чужим цветом, красятся цветом
  пространства сами ([#581](https://github.com/Matysh/houseplan-card/issues/581)).
- Близко стоящие значки устройств больше не перехватывают друг у друга
  наведение и нажатие, клавиатурный фокус показывает ту же подсказку, что и
  мышь, а переключение пространств сообщается вспомогательным технологиям
  ([#564](https://github.com/Matysh/houseplan-card/issues/564),
  [#565](https://github.com/Matysh/houseplan-card/issues/565)).
- План, изменённый в другом браузере, больше не откатывается к старой версии, а
  при перетаскивании у входящего края больше не появляется пустая полоса
  ([#543](https://github.com/Matysh/houseplan-card/issues/543),
  [#544](https://github.com/Matysh/houseplan-card/issues/544)).
- Мелкие исправления и улучшения: настройки сводной панели перестали переезжать
  между карточками в Masonry, загрузка вложения больше не жалуется на место
  из-за двойного счёта, источники радара обновляются в предпросмотре, а лучи от
  внешних углов окна не проходят сквозь толстую стену
...
- На больших планах с фоном «Следует за Солнцем» мягкий внешний контур больше
  не выделяется в GPU-слой размером со все внутренние координаты плана. При
  pinch-zoom в приложении Home Assistant такие слои остаются размером с экран,
  поэтому на пространствах с сеткой 1 см/точку в стенах и полу больше не
  появляются фиксированные белые провалы. Внешний вид контура и статичный фон
  не изменились
  ([#582](https://github.com/Matysh/houseplan-card/issues/582)).

- Солнечные лучи от внешних углов окна теперь учитывают затенение откосами
  толстой стены: при косом солнце в комнату проходит только видимая через весь
  оконный проём часть луча, а не свет сквозь сплошную стену
  ([#580](https://github.com/Matysh/houseplan-card/issues/580)).

- Комната, которой вернули «Как у пространства», больше не остаётся своего
  цвета: этот выбор теперь забывает цвет комнаты, а строка цвета в настройках
  комнаты показывается только при «Свой цвет». Комнаты, уже застрявшие с чужим
  цветом (свой цвет без своего режима), после обновления красятся цветом
  пространства без каких-либо действий
...
- При масштабировании щипком в приложении Home Assistant план больше не
  ...
  плановых перерисовках и обновлениях Home Assistant; поведение обычного
  браузера, изометрии и kiosk не изменилось
  ([#579](https://github.com/Matysh/houseplan-card/issues/579)).

- Масштабирование щипком, при котором второй палец попал на значок устройства,
  больше не запускает заново таймер долгого нажатия и не открывает одновременно
  карточку House Plan и стандартное окно Home Assistant. Запоздалое touch-
  событие контекстного меню остаётся частью навигационного жеста, а следующий
  намеренный тап, long-press, правый клик мышью или действие с клавиатуры
  работают сразу
  ([#578](https://github.com/Matysh/houseplan-card/issues/578)).

## v1.76.0-beta.3 — 2026-09-14

- Только внутренние изменения; поведение House Plan по умолчанию не изменилось.

## v1.76.0-beta.2 — 2026-09-14

- Навигация по пространствам теперь сообщает вспомогательным технологиям
  текущее пространство, тревога больше не повторяется в доступном имени
  устройства, а клавиатурный фокус открывает ту же подсказку устройства, что и
  наведение мыши. Цвета подписей комнат и поведение touch/pen не изменились.
  Наведение на комнату не заменяет клавиатурную подсказку; наведение на другое
  устройство временно показывает его и возвращает сфокусированное устройство
  после ухода указателя
  ([#565](https://github.com/Matysh/houseplan-card/issues/565)).

- Близко размещённые значки устройств больше не перехватывают друг у друга
  наведение, нажатие, долгое нажатие или перетаскивание. В нарисованной области
  выбирается видимый значок; пересечение невидимых зон нажатия разрешается в
  пользу ближайшего значка, а минимальная область 44 пикселя не уменьшается.
  Клавиатурная и программная активация остаётся привязана к явно выбранному
  значку
  ([#564](https://github.com/Matysh/houseplan-card/issues/564)).

- Масштабирование плана щипком, начатое на значке устройства, больше не
  запускает его действие из-за запоздалого синтетического нажатия браузера.
  Следующий намеренный одиночный тап работает сразу, без искусственной паузы
  ([#563](https://github.com/Matysh/houseplan-card/issues/563)).

## v1.76.0-beta.1 — 2026-09-13

- Видимость и размеры сводной панели больше не переходят между карточками
  House Plan после полной перезагрузки Masonry на другой ширине. Теперь
  карточки различаются по логическому порядку в дашборде, а не по визуальным
  колонкам; из-за неоднозначности старых ключей существующие настройки панели
  в Masonry может понадобиться один раз выставить заново
  ([#561](https://github.com/Matysh/houseplan-card/issues/561)).
...
- План, изменённый в другом браузере, больше не откатывается к старой версии,
  если подложка старого ответа загрузилась позже новой. Последний принятый план,
  его ревизия и кэш быстрого запуска теперь остаются согласованными при
  одновременных перезагрузках, переподключении и смене пользователя
  ([#543](https://github.com/Matysh/houseplan-card/issues/543)).

- При перетаскивании и масштабировании плана у входящего края больше не
  появляется пустая полоса, пока изображение догоняет жест. Быстрый путь теперь
  удерживает уже нарисованную сцену на всей площади карточки между плановыми
  перерисовками, в том числе если палец или мышь остановились посреди жеста
  ([#544](https://github.com/Matysh/houseplan-card/issues/544)).

## v1.75.0 — 2026-09-12

- Пункт меню House Plan больше не застревает на прошлой версии карточки: он
  тянул её по единственному адресу в дистрибутиве, у которого нет версии, и
  браузер был вправе часами отдавать копию из кэша. Теперь панель просит
  реализацию по имени с хешем содержимого — либо приезжает ровно та карточка, с
  которой она собрана, либо панель вслух говорит, что страницу надо перезагрузить
  ([#535](https://github.com/Matysh/houseplan-card/issues/535)).

## v1.75.0-beta.1 — 2026-09-12

- Пункт меню House Plan больше не может застрять на прошлой версии карточки.
  Он тянул её по единственному адресу в дистрибутиве, у которого нет версии, а
  входные файлы отдаются без указаний по кэшированию — и браузер был вправе
  часами отдавать старую копию. Плашка о несовпадении версий при этом честно
  горела, а перезагрузка страницы помочь не могла: адрес не менялся. Теперь
  панель просит реализацию по имени с хешем содержимого: либо приезжает ровно
  та карточка, с которой она собрана, либо панель вслух говорит, что страницу
  надо перезагрузить
  ([#535](https://github.com/Matysh/houseplan-card/issues/535)).

## v1.74.0 — 2026-09-11

- Переключение между этажами на большом плане снова быстрое. Каждое
  переключение пересобирало маркеры устройств самым долгим способом — сверяло
  двести старых маркеров с двумя сотнями новых, чтобы затем выбросить все, — и
  это стоило около десятой доли секунды подвисшей карточки. Внешне не меняется
  ничего, и дверь, которая никуда не двигалась, по-прежнему не анимируется
  ([#534](https://github.com/Matysh/houseplan-card/issues/534)).

- Фон «Следует за Солнцем» больше не замедляет план. Внешний контур плана —
  это размытие по габариту всего листа, и оно жило в общем слое с планом: любое
  движение мыши заставляло движок прогонять его заново. Теперь контур держится
  отдельным слоем. Замер на панораме демо-стенда: растеризация упала с
  пятнадцатикратной к статичному фону до сопоставимой с ним. Вид прежний:
  ореол вокруг плана совпадает пиксель в пиксель, а штриховка внутри стен
  сглаживается чуть иначе — разделение слоёв сдвигает её на субпиксель
  ([#532](https://github.com/Matysh/houseplan-card/issues/532)).
- План стал заметно плавнее, а в Firefox разница разительная. Перетаскивание
  каждый кадр рисовало весь план заново вместо того, чтобы сдвинуть уже
  нарисованное, а тень маркера устройства заново запускала анимацию сразу у
  всех значков при любом изменении ширины контейнера — шестьдесят одна такая
  анимация роняла Firefox до девяти кадров в секунду. Обоих поводов больше нет,
  картинка при этом ровно та же
  ([#531](https://github.com/Matysh/houseplan-card/issues/531),
  [#524](https://github.com/Matysh/houseplan-card/issues/524)).

- Печатный план стал крупнее примерно на треть: подписи размеров внутри плана
  уменьшены на четверть, а столбец вынесенных размеров сбоку убран — числа
  теперь стоят на самом чертеже. Короткие размеры, которым не хватает места
  внутри комнаты, больше не печатаются
  ([#530](https://github.com/Matysh/houseplan-card/issues/530)).

- План, доставшийся от времён «комнат без стен», снова редактируется. Такой
  план мог отбивать любую правку сообщением про конфликт идентификаторов стен,
  отправлять в «Оптимизировать планы», который отвечал, что всё уже оптимально,
  и вдобавок отказывался выгружаться — выхода не оставалось вовсе. Наследие
  теперь снимается при первом же сохранении, а по-настоящему устаревшей
  карточке честно говорят обновиться и перезагрузить страницу
  ([#529](https://github.com/Matysh/houseplan-card/issues/529),
  сообщено в [#527](https://github.com/Matysh/houseplan-card/issues/527)).

- Направляющие выравнивания снова идут за объектом во время жеста; сводная
  панель больше не подвешивает первый кадр на большом плане и не пишет
  «Источник недоступен», пока считает значения, а её диалог настроек снова
  прокручивается в Home Assistant. Переключение между пространствами не
  доигрывает движение створки соседней двери, и карточка скачивается на
  12,8 КБ меньше при холодном старте
...
- Перетаскивание плана снова плавное. Каждый кадр жеста заново рисовал весь
  план целиком вместо того, чтобы сдвинуть уже нарисованное, — на экране с
  высокой частотой в Firefox панорама превращалась в слайд-шоу. Теперь план
  едет за курсором, а содержимое подтягивается несколько раз в секунду; итоговая
  картинка ровно та же
  ([#531](https://github.com/Matysh/houseplan-card/issues/531)).

## v1.74.0-beta.2 — 2026-09-11

- План, доставшийся от времён «комнат без стен», снова редактируется. Такой план
  мог отбивать любую правку сообщением про конфликт идентификаторов стен,
  отправлять в «Оптимизировать планы», который отвечал, что всё уже оптимально,
  и вдобавок отказывался выгружаться — выхода не оставалось вовсе. Наследие
  теперь снимается при первом же сохранении, а по-настоящему устаревшей карточке
  честно говорят обновиться и перезагрузить страницу
  ([#529](https://github.com/Matysh/houseplan-card/issues/529),
  сообщено в [#527](https://github.com/Matysh/houseplan-card/issues/527)).

- Карточка скачивается на 12,8 КБ меньше при холодном старте. Минификатор
  таблиц стилей не работал ни разу — он искал свой тег в одном написании, а
  сборка отдаёт его в другом, — поэтому все пояснительные комментарии из стилей
  уезжали в браузер. Внешне и по поведению не меняется ничего, план просто
  появляется раньше ([#526](https://github.com/Matysh/houseplan-card/issues/526)).

- План снова плавный в Firefox. Тень маркера устройства считается от размера
  маркера, а он — от ширины контейнера, поэтому всё, что эту ширину меняло —
  подсказка, полоса прокрутки, поворот экрана, — заново запускало анимацию
  тени сразу у всех маркеров. Шестьдесят одна такая анимация роняла Firefox до
  девяти кадров в секунду; теперь тень просто появляется, оставшись прежней
  ([#524](https://github.com/Matysh/houseplan-card/issues/524)).

- При переключении между пространствами двери и окна больше не доигрывают
  движение створки соседней двери — ни в одну сторону, ни в другую. Анимация
  остаётся только за настоящей сменой состояния датчика
  ([#525](https://github.com/Matysh/houseplan-card/issues/525)).

## v1.74.0-beta.1 — 2026-09-10

- Направляющие выравнивания снова идут за объектом во время жеста: за значком
  в редакторе устройств, за фигурой и курсором в подложке и в плане. Пунктирная
  линия и точка-якорь появляются в тот момент, когда объект встаёт на одну ось
  с другим, а не после того, как жест закончился
  ([#521](https://github.com/Matysh/houseplan-card/issues/521)).

- Сводная панель больше не подвешивает первый кадр на большом плане и не пишет
  «Source unavailable», пока значения ещё считаются: в строках появляются
  пульсирующие плашки, которые плавно сменяются числами, а сама панель снова
  появляется и исчезает плавно
  ([#509](https://github.com/Matysh/houseplan-card/issues/509)).

- Диалог настроек сводной панели в Home Assistant снова прокручивается —
  колесом мыши на десктопе и свайпом на телефоне; шапка и кнопки остаются на
  месте ([#508](https://github.com/Matysh/houseplan-card/issues/508)).

## v1.73.0 — 2026-09-09

- House Plan открывается с собственной страницы в боковом меню Home Assistant;
  отдельный дашборд больше не обязателен. Обычная карточка сохранена
  ([#486](https://github.com/Matysh/houseplan-card/issues/486)).
- Настраиваемая сводная панель показывает текущие значения HA, количество
  устройств, площадь комнат, дату и время. Общие группы задаёт администратор,
  а показ на каждой карточке пользователь включает независимо. Панель,
  её кнопки и широкий диалог настроек оформлены по дизайнерскому макету;
  появление и скрытие плавные
  ([#437](https://github.com/Matysh/houseplan-card/issues/437),
  [#505](https://github.com/Matysh/houseplan-card/issues/505)).
- Текущее пространство можно экспортировать в одностраничный PDF A4:
  архитектура, названия и площади комнат, размеры, штриховка стен и компас.
  Подложка, мебель и декор добавляются по выбору; устройства и живые состояния
  в документ не входят
  ([#53](https://github.com/Matysh/houseplan-card/issues/53)).
- Первый этап поддержки радаров присутствия показывает актуальные наблюдения
  на плане и позволяет сопоставить источники, разместить и откалибровать радар.
  Поддерживаются ESPHome LD2450 и явная настройка других форматов наблюдений;
  диагностический след остаётся в текущей сессии, без долговременной истории
  ([#485](https://github.com/Matysh/houseplan-card/issues/485)).
- Для комнаты можно задать собственные нижнюю и верхнюю границы комфортной
  температуры; незаданные границы наследуются от пространства
  ([#487](https://github.com/Matysh/houseplan-card/issues/487)).
- Каждый принятый отрезок цепочки стен сразу сохраняется как обычная стена;
  незавершённые цепочки больше не требуют отдельного режима восстановления.
  Завершение, удаление и объединение комнат согласованно обновляют геометрию,
  привязки устройств и карты пылесоса через Undo/Redo
  ([#478](https://github.com/Matysh/houseplan-card/issues/478),
  [#477](https://github.com/Matysh/houseplan-card/issues/477)).
- Линии и подписи Zigbee-топологии видны поверх названий комнат и посторонних
  маркеров, а связанные устройства остаются над линиями. Серый пунктир получил
  контрастную обводку; в окнах выбора цвета появилась явная кнопка «ОК»
  ([#464](https://github.com/Matysh/houseplan-card/issues/464),
  [#476](https://github.com/Matysh/houseplan-card/issues/476)).
- Повышена надёжность восстановления после ошибок записи при импорте,
  оптимизации и удалении пространства; удалённые маршруты робота не возвращаются
  после перезапуска. Исправлен подсчёт квоты загрузки вложений и усилены проверки
  SVG и приватности пакетов поддержки
...
- Опубликован стабильный набор UI-хуков для внешних end-to-end тестов и правил
  card-mod, без привязки к приватным CSS-классам
  ([#489](https://github.com/Matysh/houseplan-card/issues/489)).

## v1.73.0-beta.9 — 2026-09-09

- Повторное открытие плана на странице, где сводная панель уже загружена,
  больше не вызывает позднего перемера шапки и лишнего круга тяжёлых
  перерисовок сразу после первого кадра: элементы панели входят в первый кадр,
  и первое обновление устройств/свечения ложится на стабильную сцену
...
- Применение импорта из резервной копии больше не отвечает «preview expired»
  ...
  ([#495](https://github.com/Matysh/houseplan-card/issues/495)).
- Сводная панель, её кнопки в шапке и широкий диалог настроек приведены к
  дизайнерскому макету. Скрытие панели тоже плавное; лишние настройки размеров
  убраны из её диалога, а переключатель показа на мобильных недоступен, когда
  панель выключена на карточке. В остальных диалогах сохраняется удобный размер
  кнопок действий независимо от панели ([#505](https://github.com/Matysh/houseplan-card/issues/505)).

## v1.73.0-beta.7 — 2026-09-09

- Настройки сводной панели теперь остаются отзывчивыми даже при большом числе
  сущностей Home Assistant: у каждого показателя открывается один поисковый
  список максимум со 100 видимыми результатами вместо полного списка в каждой
  строке. Локальные размеры значков и текста больше не скачут после обновления
  конфигурации, узкая форма и увеличенный системный текст не обрезаются по
  горизонтали, а уход со страницы или смена пользователя/прав безопасно
  отбрасывает старый черновик. Оптимизация теперь сохраняет и проверяет панель
  по тем же правилам, что обычное сохранение
  ([#493](https://github.com/Matysh/houseplan-card/issues/493)).
- Оптимизация, её серверная отмена, импорт и удаление пространства теперь
  используют единый восстанавливаемый протокол записи конфигурации и
  расположения. Если временная ошибка хранилища прервала такую запись,
  следующая правка сначала завершит восстановление и только потом проверит
  ревизии или запишет свои данные — половина плана и позиции устройств больше
  не могут быть незаметно потеряны
  ([#491](https://github.com/Matysh/houseplan-card/issues/491)).
- Значения сущностей в сводной панели теперь обновляются сразу, даже если эта
  сущность больше нигде не размещена на плане. Если ответ на сохранение был
  потерян, восстановление также сохраняет несвязанные параллельные изменения
  из другого клиента House Plan и не затирает их следующей записью; текущая
  подложка остаётся видимой, пока параллельно заменённая подложка не готова
  ([#490](https://github.com/Matysh/houseplan-card/issues/490)).
- House Plan теперь публикует документированный машиночитаемый набор
  стабильных UI-хуков для внешних end-to-end тестов и расширенных правил
  card-mod. Тесты могут определять готовность карточки и режим редактора,
  находить действия шапки, панели инструментов, диалоги, сообщения и элементы
  боковой страницы без привязки к приватным CSS-классам и деталям реализации
  ([#489](https://github.com/Matysh/houseplan-card/issues/489)).
- Над планом появилась настраиваемая информационная панель: в ней можно
  показывать состояния сущностей Home Assistant, количество размещённых
  устройств, общую чистую площадь комнат, дату и время. Администратор задаёт
  общие блоки и пространства, а каждый пользователь независимо включает
  панель и выбирает размеры значков и текста View для каждой карточки. Панель
  располагается справа или снизу, учитывает мобильный режим Home Assistant и
  скрывается, если карточка слишком мала. Настройки удобны для касания и
  остаются привязаны к той же карточке при перестроении дашборда.
  Первый жест камеры также применяется уже к итоговой геометрии шапки, поэтому
  план не смещается, пока новый контрол завершает компоновку; при тёплом
  пересоздании карточка удерживает фактически показанную высоту сцены до полной
  стабилизации новой шапки и больше не сдвигает её на несколько пикселей в
  первом кадре. Загрузка другого языка
  остаётся визуально нейтральной, а подписи комнат не меняют размер при переходе
  в редактор и исчезновении View-контролов
  ([#437](https://github.com/Matysh/houseplan-card/issues/437)).
- Радары присутствия теперь могут показывать на плане актуальные наблюдения,
  обрезанные по границам выбранной комнаты. House Plan распознаёт координатные
  наборы ESPHome LD2450, поддерживает явное сопоставление декартовых, полярных,
  дальностных, зональных и presence-only источников и проводит установку и
  калибровку прямо на плане с диагностикой состояния данных. Сырые наблюдения и
  короткий диагностический след остаются только в текущей сессии. Автоопределение
  LD2450 теперь требует точного совпадения модели и общего устройства ESPHome;
  смена сохранённой привязки не удаляет настройку радара, оставляя её доступной
  для исправления, а отмена установки, статусы источников и неверное размещение
  контрольных точек получают отдельные понятные сообщения
  ([#485](https://github.com/Matysh/houseplan-card/issues/485)).

## v1.73.0-beta.6 — 2026-09-08

- Страница House Plan в боковом меню теперь показывает план с первого захода в
  реальном Home Assistant: панель принимает свойства, которые HA присваивает до
  определения элемента, и берёт высоту от viewport, а не от контейнера без
  высоты — страница больше не пустая со стейджем 0 px
  ([#488](https://github.com/Matysh/houseplan-card/issues/488)).
- В настройках отдельной комнаты теперь можно задать собственные нижнюю и
  верхнюю границы комфортной температуры. Каждая пустая граница независимо
  наследуется от пространства; настройка меняет только температурную заливку
  комнаты и проёмов ([#487](https://github.com/Matysh/houseplan-card/issues/487)).

## v1.73.0-beta.5 — 2026-09-08

- House Plan теперь открывается напрямую с собственной страницы в боковом меню
  Home Assistant — с теми же пространствами, живым управлением и редакторами,
  что у полной карточки. Необязательная карточка для дашборда сохранена и в
  Sections по умолчанию занимает всю ширину; пользователь без права
  редактирования получает безопасное read-only состояние пустого плана
  ([#486](https://github.com/Matysh/houseplan-card/issues/486)).
- PDF-экспорт теперь сохраняет полную цепь горизонтальных и вертикальных
  размеров у прямоугольных выступов фасада: глубина выступа и соседние внешние
  участки больше не пропадают
  ([#484](https://github.com/Matysh/houseplan-card/issues/484)).

## v1.73.0-beta.4 — 2026-09-07

- PDF-экспорт теперь эффективнее вписывает в лист всю сцену, печатает серые
  стены со штриховкой и полноценный векторный компас, а также показывает более
  чистые горизонтальные и вертикальные размеры без локального дублирования
  противоположных сторон и наложения подписей на геометрию плана. Техническая
  легенда убрана, а узкому диалогу больше
  не нужна горизонтальная прокрутка
  ([#482](https://github.com/Matysh/houseplan-card/issues/482)).

## v1.73.0-beta.3 — 2026-09-07

- Администратор теперь может сохранить текущее пространство как
  чистый одностраничный PDF A4 с архитектурой, названиями комнат,
  чистой площадью пола и размерами; мебель, декор и подложка добавляются
  по желанию. Устройства и живые состояния в PDF не попадают
  ([#53](https://github.com/Matysh/houseplan-card/issues/53)).

## v1.73.0-beta.2 — 2026-09-06

- Во всех окнах выбора цвета появилась понятная полноширинная кнопка **«ОК»**.
  Цвет и прозрачность по-прежнему применяются сразу, а при некорректном HEX
  окно остаётся открытым, чтобы значение можно было исправить
  ([#476](https://github.com/Matysh/houseplan-card/issues/476)).
- Цепочка стен теперь сразу сохраняет каждый принятый отрезок как обычную
  независимую стену. Незавершённые цепочки остаются на плане, но больше не
  возобновляются особым образом после перезагрузки; старые сохранённые контуры
  безопасно мигрируют в новую модель, а поглощённые созданной комнатой стены не
  остаются дубликатами кладки
  ([#478](https://github.com/Matysh/houseplan-card/issues/478)).
- Завершённая цепочка стен теперь сразу объединяет собственные совместимые
  прямые участки и устраняет доказанно дублирующую кладку. Удаление и объединение
  комнат согласованно обновляет привязки устройств и межэтажные карты пылесоса
  через Undo/Redo, а оптимизация сохраняет плавные трансформации мебели и
  изображений ([#477](https://github.com/Matysh/houseplan-card/issues/477)).

## v1.73.0-beta.1 — 2026-09-06

- Активные линии и подписи Zigbee-топологии теперь читаются поверх названий
  комнат и посторонних маркеров, а полные маркеры связанных устройств остаются
  сверху, включая маркер прямо под курсором. У серого пунктира неизвестного
  качества также появилась тонкая тёмная контрастная обводка
  ([#464](https://github.com/Matysh/houseplan-card/issues/464)).

## v1.72.0 — 2026-09-05

- Администратор теперь может явно загрузить снимок топологии ZHA или
  Zigbee2MQTT и посмотреть на плане наблюдаемых прямых соседей
  выбранного устройства. Качество связи, пунктир для неизвестного
  LQI, стрелки к координатору и честные окончания пути в другом
  пространстве или у неразмещённого узла собраны в одну контекстную
  легенду; hover никогда не запускает сканирование сети
...
- Клик или тап по комнате теперь центрирует и вписывает её с полями
  10%, а двойной клик или тап по свободному фону выполняет
  **«Вписать всё»**. В настройках пространства также можно создать копию
  без комнат, сохранив архитектуру, подложку и настройки отображения,
  но не копируя размещённые устройства
...
- На больших планах тяжёлая сцена остаётся стабильной при hover, pan, pinch
  и перетаскиваниях в редакторах, а промежуточные отрезки цепочки стен
  фиксируются ограниченной локальной проверкой с полной fail-closed
  проверкой при завершении
  ([#451](https://github.com/Matysh/houseplan-card/issues/451),
  [#461](https://github.com/Matysh/houseplan-card/issues/461)).
- Установка и обновление карточки теперь сами объясняют своё состояние,
  безопасно восстанавливаются при несовпадении версий frontend и интеграции,
  а нативные подтверждения и резервные диалоги сохраняют центрирование и
  модальность после переподключения Home Assistant
  ([#462](https://github.com/Matysh/houseplan-card/issues/462),
  [#463](https://github.com/Matysh/houseplan-card/issues/463)).
- Мебель можно примагничивать к обеим физическим граням наружной стены,
  стрелки двигают выбранные объекты подложки на одну клетку, а превью
  мебели надёжно исчезает при уходе указателя с плана
  ([#447](https://github.com/Matysh/houseplan-card/issues/447),
  [#460](https://github.com/Matysh/houseplan-card/issues/460)).
- Скрытые эксперименты теперь включаются единым постоянным локальным ключом
  браузера `hp_alpha=1`; `hp_alpha=0` выключает весь набор
  ([#448](https://github.com/Matysh/houseplan-card/issues/448)).

## v1.72.0-beta.5 — 2026-09-05

- Нативные подтверждения и резервные диалоги House Plan теперь остаются по
  центру экрана с настоящим затемнённым модальным фоном. Если Home Assistant
  временно отсоединяет и восстанавливает карточку, открытый диалог возвращается
  в верхний слой браузера вместо немодального окна, прижатого к краю экрана
  ([#463](https://github.com/Matysh/houseplan-card/issues/463)).
- При рисовании цепочки стен на большом плане каждый промежуточный отрезок
  теперь фиксируется через ограниченную локальную проверку вместо двух полных
  пересчётов всего этажа; завершение цепочки по-прежнему запускает полную
  fail-closed проверку ([#461](https://github.com/Matysh/houseplan-card/issues/461)).
- Установка и обновление карточки теперь сами объясняют своё состояние:
  House Plan различает режимы ресурсов Lovelace, показывает регистрацию в
  «Состоянии системы», один раз напоминает о полной перезагрузке страницы и
  безопасно восстанавливает открытую карточку при несовпадении версий frontend
  и интеграции; киоск перезагружается не более одного раза и только в покое
  ([#462](https://github.com/Matysh/houseplan-card/issues/462)).

## v1.72.0-beta.4 — 2026-09-05

- Превью размещения мебели теперь надёжно исчезает при уходе указателя с плана,
  в том числе после полной перерисовки редактора; у живого редактора также
  появилась детерминированная точка ожидания отрисовки для браузерных проверок
  ([#460](https://github.com/Matysh/houseplan-card/issues/460)).
- На линиях топологии Zigbee при наведении теперь видны стрелки в сторону
  координатора; входящие стрелки показывают устройства за роутером, а если
  следующий узел находится на другом плане или не размещён, короткая подпись
  честно завершает путь ([#457](https://github.com/Matysh/houseplan-card/issues/457)).

- У настройки «Связи Zigbee» появилась контекстная подсказка: она коротко
  объясняет функцию, разбирает легенду — цвет линии как качество связи, пунктир
  как несообщённое качество, стрелки как путь к координатору — и называет
  ограничения: данные из последней загрузки устаревают, а связи видны только
  при работе мышью
  ([#459](https://github.com/Matysh/houseplan-card/issues/459)).

- В настройках пространства теперь можно создать под новым именем копию без
  комнат: стены, проёмы, колонны, элементы подложки, положение фонового плана,
  масштаб и настройки отображения сохраняются, а размещение устройств не
  переносится; отдельное подтверждение появляется только тогда, когда сначала
  требуется оптимизировать весь план
  ([#456](https://github.com/Matysh/houseplan-card/issues/456)).
...
- Двойной клик или двойной тап по свободному фону плана в Просмотре и киоске
  теперь выполняет то же действие **«Вписать всё»**, что кнопка в панели;
  комнаты, устройства, проёмы и редакторы сохраняют собственные жесты
  ([#449](https://github.com/Matysh/houseplan-card/issues/449)).
- Обновление топологии Zigbee2MQTT теперь понимает реальные camelCase-поля
  raw-карты сети, сохраняя совместимость со snake_case: корректная карта
  заполняет устройства и связи вместо ошибки неподдерживаемых данных
  ([#450](https://github.com/Matysh/houseplan-card/issues/450)).

## v1.72.0-beta.1 — 2026-09-04

- Скрытые эксперименты теперь включаются одним бессрочным локальным ключом
  браузера: `hp_alpha=1` активирует весь alpha-набор и возвращает существующий
  3-D Просмотр, а `hp_alpha=0` выключает его. Прежнее состояние
  `hp-labs=iso` не переносится, поэтому тестировщикам нужно включить alpha
  заново ([#448](https://github.com/Matysh/houseplan-card/issues/448)).
- Администратор теперь может явно загрузить снимок топологии ZHA или
  Zigbee2MQTT и при наведении реальной мыши видеть только наблюдаемых прямых
  соседей выбранного устройства. Диагностика по умолчанию выключена, никогда не
  запускает сканирование от наведения, считает вместо отрисовки связи с другими
  пространствами и не появляется на touch, в киоске, редакторах и статической
  карточке ([#54](https://github.com/Matysh/houseplan-card/issues/54)).
- В Просмотре и киоске клик или тап по комнате теперь центрирует и вписывает
  её видимый пол и ограничивающие стены с полями 10%; видимая подпись комнаты
  даёт то же действие с клавиатуры
  ([#152](https://github.com/Matysh/houseplan-card/issues/152)).
- Мебель теперь может примагничиваться к обеим физическим граням наружной стены
  и не проваливается сквозь кладку, а стрелки сдвигают любой выбранный объект
  подложки ровно на одну текущую клетку без повторного магнита стен или декора
  ([#447](https://github.com/Matysh/houseplan-card/issues/447)).

## v1.71.0 — 2026-09-04

- В редакторе подложки теперь можно загружать и повторно использовать закрытые
  PNG, JPEG, WebP и безопасные SVG, а затем размещать, менять размер, отражать,
  вращать, заменять и раскладывать их по слоям как мебель, но без магнита к
  стенам; для пропавшего файла сохраняется восстанавливаемая рамка, а переносной
  экспорт хранит данные целостности без встраивания байтов
  ([#51](https://github.com/Matysh/houseplan-card/issues/51)).
- Роботу с несколькими картами теперь можно сопоставить и откалибровать каждую
  карту на своём этаже: живая позиция и след рисуются там, а док остаётся на
  месте; для несопоставленной или неоднозначной активной карты план показывает
  точную причину вместо попытки угадать этаж
  ([#162](https://github.com/Matysh/houseplan-card/issues/162)).
- В общих настройках можно скрыть информационное окно комнаты, не отключая её
  подсветку; мебель примагничивается к видимой физической грани стены, а в
  каталоге HACS интеграция теперь называется просто **House Plan**
...
- Подтверждения и **«Помощь и обратная связь»** теперь безопасно завершаются при
  пропавшем пространстве, позднем превью, изменённом повторе, совместимых
  смешанных версиях и некорректных вложенных данных; отслеживание HA-зон и
  отклонённые сохранения общих настроек, устройства или калибровки также
  сохраняют последний подтверждённый план
...
- Мебель теперь примагничивается задней гранью к видимой физической поверхности
  стены, а не к её оси: учитываются локальная толщина и выбранная сторона общей
  стены, поэтому предмет больше не оказывается внутри кладки
  ([#445](https://github.com/Matysh/houseplan-card/issues/445)).
- Если Home Assistant отклоняет сохранение общих настроек, устройства или
  калибровки робота, план теперь возвращается к последнему подтверждённому
  состоянию даже после канонизации отправляемого конфига, но сохраняет открытый
  редактор, автоматическое предложение или ручную подгонку для повтора;
  сообщение об успехе появляется только после принятой записи
  ([#442](https://github.com/Matysh/houseplan-card/issues/442)).
- После удаления последнего маршрута карты робота список теперь остаётся явно
  пустым и не возвращает старую калибровку; маршруты в удалённые пространства
  собраны в понятную группу, а живой слой больше не перебирает повторно все
  устройства, не являющиеся роботами
  ([#443](https://github.com/Matysh/houseplan-card/issues/443)).
- Вторую и последующие карты робота теперь можно добавить: сначала
  редактор просит выбрать этаж и только потом сохраняет маршрут,
  а отказ Home Assistant не оставляет ложную локальную запись
  ([#441](https://github.com/Matysh/houseplan-card/issues/441)).

## v1.71.0-beta.3 — 2026-09-03

- Если робот убирает, а его текущая карта не сопоставлена этажу, не
  откалибрована, неоднозначна или ссылается на удалённое пространство, у дока
  появляется янтарное предупреждение с точной причиной — вместо плана, на
  котором робота просто нет
  ([#162](https://github.com/Matysh/houseplan-card/issues/162)).
- В редакторе устройства появился блок «Карты и этажи»: каждую карту робота
  можно сопоставить пространству, откалибровать её именно в этом пространстве, а
  позже перенести или удалить — док при этом остаётся на своём месте
  ([#162](https://github.com/Matysh/houseplan-card/issues/162)).
- Робот-пылесос с несколькими картами теперь показывается на своём этаже: док
  остаётся в своём пространстве, а живое положение и след рисуются в том
  пространстве, которому назначена активная карта; несопоставленная,
  неоднозначная или незакалиброванная карта не рисует ничего вместо того, чтобы
  угадывать этаж ([#162](https://github.com/Matysh/houseplan-card/issues/162)).
- Пользовательские изображения подложки теперь отклоняют специальные файловые
  объекты без зависания Home Assistant, переживают исчезновение файлов во время
  подсчёта лимита и отличают повреждённое изображение от переполненного
  хранилища; движение пером или пальцем также снимает зависшее наведение комнаты
  при выключенной подсказке
  ([#440](https://github.com/Matysh/houseplan-card/issues/440)).
- Если Home Assistant отклоняет сохранение общих настроек, план снова показывает
  последнее подтверждённое сервером состояние вместо несохранённого локального
  черновика; введённые значения остаются в открытом диалоге для повторной
  попытки ([#439](https://github.com/Matysh/houseplan-card/issues/439)).
...
- В редакторе подложки теперь можно загружать и повторно использовать закрытые
  PNG, JPEG, WebP и безопасные SVG, а затем размещать, плавно менять размер,
  отражать, вращать, заменять и раскладывать их по слоям как мебель, но без
  магнита к стенам. Для пропавшего файла сохраняется восстанавливаемая рамка, а
  экспорт записывает хэш без встраивания байтов
  ([#51](https://github.com/Matysh/houseplan-card/issues/51)).
- **«Помощь и обратная связь»** теперь работает при совместимых смешанных
  версиях карточки и интеграции, использует безопасное имя вложения на основе
  SHA, сообщает обо всех семействах Repair House Plan и отклоняет исчерпанный
  лимит превью до сборки диагностического пакета
  ([#423](https://github.com/Matysh/houseplan-card/issues/423)).
- При построении privacy-проекции support-пакет теперь проверяет типы флага и
  позиции бейджа значения, поэтому некорректный вложенный конфиг не может
  дословно попасть в отчёт
  ([#420](https://github.com/Matysh/houseplan-card/issues/420)).
- Провенанс автоматического переноса между HA-зонами теперь переживает временно
  пустой или усечённый реестр Home Assistant: уборка сверяется с полным roster
  привязок и забывает исчезнувшее устройство лишь после двух непустых
  авторитетных наблюдений
  ([#419](https://github.com/Matysh/houseplan-card/issues/419)).
- **«Помощь и обратная связь»** теперь учитывает отказ от вложения даже при
  позднем ответе превью, а изменённая повторная отправка получает новый ключ
  идемпотентности, чтобы исправленный репорт дошёл до поддержки
  ([#418](https://github.com/Matysh/houseplan-card/issues/418)).
- Подтверждение опасного действия больше не зависает, если активное пространство
  исчезло или запрос пришёл во время переключения лениво загружаемого языка:
  когда показать диалог невозможно, House Plan безопасно отвечает отказом
  ([#417](https://github.com/Matysh/houseplan-card/issues/417)).
...
- Первая загрузка после обновления больше не рассчитывает большой этаж дважды,
  пока House Plan создаёт служебную привязку к зонам Home Assistant. Время
  запуска тестового этажа из 20 комнат вернулось к уровню предыдущей стабильной
  версии ([#126](https://github.com/Matysh/houseplan-card/issues/126)).

## v1.70.0-beta.4 — 2026-09-02

- Кнопка **«Помощь и обратная связь»** теперь совпадает по размеру 44 px с
  кнопкой «Общие настройки», а поле сообщения использует обычный фон поля ввода
  вместо серой заливки
...
- Отказ записи конфига при переносе устройства в другую HA-зону больше не
  стирает молча поставленную вручную позицию: House Plan восстанавливает точку
  перед повтором, а если не удалось и это — отмечает маркер для внимания.
  Перенос теперь удаляет Undo/Redo только этого маркера, а не очищает историю
  позиций всех устройств
  ([#403](https://github.com/Matysh/houseplan-card/issues/403)).

- Подтверждения работают и до создания первого пространства: удаление
  сохранённого плана в онбординге теперь спрашивает и отвечает, а не молчит
  ([#402](https://github.com/Matysh/houseplan-card/issues/402)).

## v1.70.0-beta.2 — 2026-08-31

- Перенос устройства или отдельно размещённой сущности в другую HA-зону теперь
  переносит и маркер в связанную комнату House Plan, даже если маркер раньше
  двигали вручную. Новое место выбирает обычная сетка комнаты и отмечает его
  для внимания; явный выбор комнаты в House Plan по-прежнему важнее
  ([#126](https://github.com/Matysh/houseplan-card/issues/126)).
...
- Опасные действия теперь используют единый доступный диалог подтверждения
  ...
  последствия, безопасные «Отмена»/X/`Esc` и защиту от уже изменившейся цели
  ([#32](https://github.com/Matysh/houseplan-card/issues/32)).
- Зум снова честный: масштаб, который вы видите в момент касания плана,
  и запоминается, а быстрая серия колеса или трекпада удерживает точку под
  курсором ровно на месте
  ([#396](https://github.com/Matysh/houseplan-card/issues/396)).

## v1.70.0-beta.1 — 2026-08-30

- Колесо, кнопки масштаба, «Вписать всё», стрелка возврата и двойной тап в
  киоске теперь плавно перемещают камеру за короткую анимацию. Щипок и
  панорамирование остаются непосредственными, а при системном уменьшении
  движения масштаб меняется мгновенно
  ([#82](https://github.com/Matysh/houseplan-card/issues/82)).

- Ошибки бэкенда теперь всегда приходят локализованными: у каждого
  стабильного кода есть перевод на все четыре языка, неизвестный код даёт
  локализованный текст с кодом вместо сырой английской фразы, а детали
  ошибок проёмов передаются структурированно. Под капотом бэкенд получил
  гейты покрытия, линта и типизации в CI
  ([#42](https://github.com/Matysh/houseplan-card/issues/42)).

- В редакторе устройств появилась отдельная история отмены и повтора позиций
  маркеров. Одно завершённое перетаскивание — один шаг; отменённые и неудачные
  перемещения в историю не попадают. До 50 шагов текущей сессии доступны через
  постоянные кнопки панели или `Ctrl/Cmd+Z`, `Ctrl/Cmd+Shift+Z` и `Ctrl+Y`
  ([#74](https://github.com/Matysh/houseplan-card/issues/74)).

- У обнаружения устройств не осталось скрытых ручек: группировка света и
  список исключённых интеграций стали видимыми «Фильтрами обнаружения» на
  вкладке «Доступны» каталога — со счётчиками «появится/скроется» до записи
  и именем интеграции у каждого исключённого кандидата. Климат комнаты
  теперь следует тем же пользовательским исключениям, что и обнаружение
  ([#44](https://github.com/Matysh/houseplan-card/issues/44)).

- Старые и будущие конфигурации плана теперь защищены механикой, а не
  привычкой: схема бэкенда выгружается в закоммиченный манифест, списки
  значений фронта и бэкенда сверяются в CI с явным allow-list'ом немногих
  легитимных компат-расхождений, а lifecycle-фикстуры доказывают, что конфиг
  старейшей поддерживаемой эпохи — или из будущего — загружается без тихих
  потерь ([#33](https://github.com/Matysh/houseplan-card/issues/33)).

- У критичных настроек плана, света, фона и каталога устройств появились
  короткие контекстные подсказки на всех четырёх языках интерфейса; открытие
  подсказки не меняет саму настройку
  ([#86](https://github.com/Matysh/houseplan-card/issues/86)).

- Клик по уже выбранной привязке в диалоге маркера больше не сбрасывает молча
  настроенный источник значения и бейдж на «авто» — сброс происходит только
  при реальной смене привязки
  ([#385](https://github.com/Matysh/houseplan-card/issues/385)).

- Мебель теперь можно плавно масштабировать угловыми и одноосевыми ручками,
  зеркалить по горизонтали/вертикали перетаскиванием через грань или в
  свойствах и свободно вращать с привязкой к 45° по `Shift`. Область выбора
  расширена на физические 10 см вдоль линий предмета, не захватывая пустую
  рамку ([#383](https://github.com/Matysh/houseplan-card/issues/383)).

- Для маркера устройства теперь можно выбрать действие **«Ничего не делать»**.
  Короткий клик, тап, Enter и Space ничего не открывают и не отправляют, при
  этом внешний вид маркера, карточка House Plan по долгому нажатию и HA
  more-info по правому клику сохраняются
  ([#381](https://github.com/Matysh/houseplan-card/issues/381)).

- `fit: house` в `houseplan-space-card` больше не позволяет скрытой архитектуре
  расширять плотный кадр: при выключенном `show_borders` стены, перегородки и
  колонны не голосуют — как и скрытые символы проёмов
  ([#384](https://github.com/Matysh/houseplan-card/issues/384)).
...
- Glow в `houseplan-space-card` больше не пересобирает граф света и сцену стен
  ...
  переиспользует оба своих состояния, быстрый recut-путь кладки и кэш выреза
  комнат). Заодно исправлен регресс кэша графа света, который полная карта
  получила в beta.4
  ([#375](https://github.com/Matysh/houseplan-card/issues/375)).
- В `houseplan-space-card` появилось опциональное `fit: house`: вся конструкция
  дома заполняет компактную карточку без обычного внешнего поля 5%, а подложка,
  декор, подписи и маркеры больше не уменьшают дом. Прежнее кадрирование по
...
- House Plan заговорил по-французски: полный перевод сообщества от @OUARZA,
  загружается лениво, как немецкий, — обычный View-бандл не вырос. `Français`
  выбирается явно и включается автоматически для французских профилей
  Home Assistant ([#371](https://github.com/Matysh/houseplan-card/issues/371)).
...
- В редактор устройств вернулась отдельная кнопка **Добавить** перед
  **Устройства**: новый маркер снова можно создать напрямую, не открывая
  сначала каталог ([#363](https://github.com/Matysh/houseplan-card/issues/363)).
...
- Редактор подложки теперь постоянно показывает основной цвет и прозрачность на
  главной панели при любом инструменте. Один выбор применяется к новым линиям,
  контурам фигур, тексту и мебели, не меняя готовые объекты и независимую
  заливку фигур ([#360](https://github.com/Matysh/houseplan-card/issues/360)).

- Толщина контура мебели теперь меняется вместе с масштабом плана, как та же
  физическая толщина у линий и фигур, а независимое изменение ширины и глубины
  предмета по-прежнему не искажает горизонтальные, вертикальные и диагональные
  штрихи ([#361](https://github.com/Matysh/houseplan-card/issues/361)).
- Пакет мелких честных фиксов из аудита: климат комнаты добирается до
  легаси-маркеров без явного признака «нет HA Area»; устройство с полностью
  отключёнными сущностями больше не светится как живое; превью мебели
  реагирует на Shift мгновенно; правый клик с выбранным инструментом больше
  не ставит объекты; отброшенный сегмент следа пылесоса (битая калибровка)
  теперь виден в консоли, а не исчезает молча
  ([#369](https://github.com/Matysh/houseplan-card/issues/369)).

- Маркеры устройств в редакторе подложки теперь служат только полупрозрачными
  ориентирами: ядро, кольцо, капсула значения и бейджи больше не реагируют на
  наведение, не открываются, не выполняют действия и не перетаскиваются, а
  инструмент рисования или мебели получает нажатие сквозь всю видимую область
  маркера ([#362](https://github.com/Matysh/houseplan-card/issues/362)).
- Очень большие подложки больше не вешают планшет: карточка читает
  разрешение из заголовка файла до каких-либо тяжёлых операций, показывает
  честные цифры и предлагает безопасную уменьшенную копию (пропорции и
  прозрачность сохраняются). Исходный файл не изменяется, а текущий план
  переживает любой сбой
  ([#39](https://github.com/Matysh/houseplan-card/issues/39)).

## v1.69.0-beta.3 — 2026-08-29

- При размещении мебели в редакторе подложки мышь теперь показывает на плане
  полупрозрачный символ с точным будущим размером, поворотом и привязкой к
  стене. Предпросмотр обновляется вместе с полями размера и исчезает без
  изменений при отмене; на touch движение, отмена или второй палец не создают
  случайный предмет ([#359](https://github.com/Matysh/houseplan-card/issues/359)).

- След пылесоса теперь повторяет те же записанные точки с мягко скруглёнными
  поворотами вместо заметной ломаной. Текущая и предыдущая уборки используют
  одну ограниченную кривую, сохраняют разрывы телеметрии и не отклоняются от
  записанного маршрута более чем на 17,5 см
  ([#209](https://github.com/Matysh/houseplan-card/issues/209)).

- Робот-пылесос с живой позицией больше не замораживает карточку на только
  что открытой странице: определение id карты выполнялось внутри цикла
  отрисовки и требовало код редактора, который холодная вкладка ещё не
  загрузила, — первый кадр оказывался последним
  ([#358](https://github.com/Matysh/houseplan-card/issues/358)).

- Glow теперь учитывает текущее состояние привязанных внутренних дверей и
  ворот: закрытый проём останавливает свет, открытый пропускает его, а cover с
  позицией открывает соответствующую долю прохода. Открытые проходы остаются
  прозрачными, окна и наружные проёмы — непрозрачными; посторонние обновления
  Home Assistant переиспользуют готовую геометрию света
  ([#20](https://github.com/Matysh/houseplan-card/issues/20)).

- Нажатие на устройство на только что открытой странице снова работает без
  предварительного захода в какой-либо редактор. Выключатель, управляющий
  виртуальными источниками света (и любое другое устройство с тап-переключением),
  молча игнорировал нажатие в холодной вкладке, пока настройки устройства не
  откроют один раз — теперь View-карточка разрешает нажатие сама, без ленивого
  кода редактора ([#357](https://github.com/Matysh/houseplan-card/issues/357)).
...
- Если необязательный языковой пакет (сейчас — немецкий) не загрузился,
  ...
  набор i18n ([#354](https://github.com/Matysh/houseplan-card/issues/354)).

- Устаревший или сторонний клиент больше не может молча заменить сохранённые
  позиции устройств, отправив весь layout без его ревизии. Инициализация без
  ревизии остаётся доступной только пока хранилище layout пусто
  ([#356](https://github.com/Matysh/houseplan-card/issues/356)).

- Температура и влажность комнаты теперь следуют фактическому размещению
  реального датчика в House Plan, а не остаются в его HA-зоне. Автоматические
  климатические показатели работают и в комнатах без HA-зоны; подпись,
  подсказка и температурная заливка согласованы
  ([#317](https://github.com/Matysh/houseplan-card/issues/317)).

- Активные физические настенные выключатели, для которых Home Assistant не
  предоставляет собственных сущностей, больше не выглядят постоянно
  недоступными: теперь они остаются непрозрачными и становятся жёлтыми при
  работающей управляемой цели или нейтральными в остальных случаях
  ([#318](https://github.com/Matysh/houseplan-card/issues/318)).

- Устаревшая вкладка или сторонний клиент больше не может молча заменить новый
  сохранённый план, не передав ревизию конфигурации. House Plan отклоняет такую
  запись как конфликт и сохраняет серверную копию; инициализация без ревизии
  разрешена только пока хранилище конфигурации пусто
  ([#340](https://github.com/Matysh/houseplan-card/issues/340)).

- Ленивый редактор переживает нестабильную сеть и обновления: после сетевого
  сбоя повторное нажатие «Редактировать» пробует загрузку заново, а не молчит
  до перезагрузки страницы; тост подсказывает, что делать. Устаревший
  `houseplan-card.js` из кэша после обновления показывает плашку «перезагрузите
  страницу» вместо пустой карточки. Чанки с контент-хэшами теперь отдаются с
  иммутабельным кэшированием
  ([#353](https://github.com/Matysh/houseplan-card/issues/353)).

- Очень острая вершина между стенами разной толщины (например, 15 и 30 см)
  на старых планах снова рисуется честным остриём, а не «трезубцем»: расчёт
...
- В House Plan появилась полная немецкая локализация. `Deutsch` можно выбрать
  вручную, а для немецкого профиля Home Assistant она включается автоматически;
  словарь загружается по требованию и не утяжеляет обычный режим просмотра
  ([#348](https://github.com/Matysh/houseplan-card/issues/348)).
...
- Режим просмотра теперь запускается без загрузки трёх редакторов: их общий
  ...
  проверкой версии сборки. Релизные инструменты сверяют версии обоих runtime,
  а тестовые — всё дерево frontend-ассетов, а не один JavaScript-файл
  ([#337](https://github.com/Matysh/houseplan-card/issues/337)).
...
- Острые углы комнат снова выглядят честно: очень острая вершина
  ...
  узле, стены короче 20 см или собственной толщины, узлы ближе 5 см и
  комнаты без внутреннего просвета. Длина меряется по всей стене, поэтому
  короткий доборный участок, компенсирующий перепад толщин, остаётся законным
  ([#329](https://github.com/Matysh/houseplan-card/issues/329)).
- В редакторе устройств появился единый каталог с поиском: устройства
  на плане, доступные привязки, скрытые маркеры и привязки для
  повторного добавления. Статус Home Assistant объясняется отдельно, а
  поиск использует видимые на плане имена комнат; скрытие, показ и повторное добавление собраны в одном
  месте ([#29](https://github.com/Matysh/houseplan-card/issues/29)).

## v1.68.1 — 2026-08-27

- Создание пространства больше не выдаёт ошибку
  `v8+ space requires wall_segments` и не оставляет на экране несохранённое
  пространство-призрак; после неудачного сохранения состояние возвращается к
  серверному до следующего создания или удаления
  ([#324](https://github.com/Matysh/houseplan-card/issues/324)).

## v1.68.0 — 2026-08-27

- Структурные изменения плана теперь опираются на стабильные ID сегментов стен:
  толщина, происхождение черновиков и принадлежность проёмов сохраняются при
  Resize, Undo/Redo, импорте, оптимизации и повторном редактировании. Переход на
  model v9 выполняется атомарно, а конфликты старых границ с проёмами разрешаются
  автоматически и детерминированно. Перед первым структурным сохранением или
  запуском «Оптимизировать планы» рекомендуется экспортировать резервную копию:
  старые версии не понимают новую модель стен
...
- Виртуальные стены заменены обычными стенами толщиной `0 см`. Их можно рисовать
  и менять инструментами «Стены» и «Толщина», а в настройках пространства —
  выбрать пунктирный вид, пропускающий Glow и солнечные лучи, либо сплошной,
  работающий как световой барьер нулевой площади
  ([#306](https://github.com/Matysh/houseplan-card/issues/306)).
- Улучшена работа редактора стен: оси и узлы остаются видимыми во всех
  инструментах, отдельно стоящие и черновые стены доступны инструменту
  «Толщина», а сложные стыки и острые углы больше не оставляют зубцов, ступеней
  и белых клиньев.
- Мелкие исправления и улучшения.

## v1.68.0-beta.4 — 2026-08-27

- Обновление на новую модель стен больше не замораживает редактирование:
  сервер распознаёт первую запись более новой карточки поверх старого
  документа (например, с осиротевшей легаси-границей) вместо требования
  обновить уже обновлённую карточку
  ([#319](https://github.com/Matysh/houseplan-card/issues/319)).
- Рисование снова работает везде: преобразование плана к новой модели стен
  больше не останавливается на конфликте легаси-границы с проёмом. Дверь
  внутри бывшей «границы» сохраняет настоящую стену (разрыв продолжается по
  обе стороны), неоднозначный носитель выбирается детерминированно, а проём
  вовсе без пригодной стены сохраняется в данных — бездействует, пока его не
  переставят ([#316](https://github.com/Matysh/houseplan-card/issues/316)).

## v1.68.0-beta.3 — 2026-08-26

- Теперь у стен одна модель при любой толщине. Отдельные инструмент
  **«Граница»** и сущность виртуальной стены удалены: **«Стены»** и
  **«Толщина»** принимают `0 см` для контуров комнат, сохранённых черновиков и
  независимых стен. Для пространства можно выбрать общий вид нулевых стен:
  пунктир (по умолчанию, пропускает Glow и солнце) либо сплошную линию (точный
  световой барьер нулевой площади). Старые виртуальные участки атомарно
  мигрируют в стабильные сегменты model v9; все прежние `cm:0` намеренно
  получают ту же семантику, поэтому у небольшой части старых планов может
  измениться вид или прохождение света. Нулевая стена не создаёт кладку,
  площадь или тоннель и не может содержать проём. Перед первым структурным
  сохранением, импортом или **«Оптимизировать планы»**, записывающим model v9,
  экспортируйте резервную копию: старая версия не понимает новую модель стен,
  и поддерживаемый путь назад — восстановление этой копии
  ([#306](https://github.com/Matysh/houseplan-card/issues/306)).

## v1.68.0-beta.2 — 2026-08-26

- Флаг диалога привязки называется «Показывать сущности» — как в руководстве
  пользователя ([#269](https://github.com/Matysh/houseplan-card/issues/269)).
- Отказ проверки геометрии теперь объясняет себя: диалог называет причину по
  каждому пространству, даёт скопировать диагностический блок одной кнопкой
  (с фолбэком прямо в диалоге, если буфер недоступен) и пишет одну
  структурированную запись в консоль разработчика. Совет «обновите House
  Plan» показывается только при реальном расхождении версий карточки и
  интеграции ([#295](https://github.com/Matysh/houseplan-card/issues/295)).

- Инструмент «Толщина» теперь берёт любую стену: отдельно стоящие перегородки
  и сегменты сохранённых черновиков выбираются и меняются так же, как контуры
  комнат; при точном наложении клик достаётся отдельно стоящей стене. Ноль
  для неё по-прежнему отклоняется — стены без толщины не существует
  ([#313](https://github.com/Matysh/houseplan-card/issues/313)).
- Рисование комнат на планах model v8 снова работает надёжно: незавершённая
  цепочка стен сохраняет ID сегментов при промежуточных сохранениях и отмене, а
  отклонённая сервером запись сразу возвращает последнюю принятую геометрию,
  не оставляя временную комнату или случайную перегородку
...
- Структурные изменения стен теперь сохраняют толщину, происхождение черновых
  сегментов и принадлежность дверей, окон, ворот и проходов через стабильные ID
  сегментов стен. Существующий план обновляется атомарно при первом структурном
  изменении или через **«Оптимизировать планы»**; Resize, отмена/повтор и
  последующие правки сохраняют те же ID без накопления координат вне сетки;
  серверная транзакция Optimize также гарантирует это обновление при прямом
  вызове. Принятая пользователем цепочка остаётся допустимой при преобразовании
  в комнату или независимые стены, а отклонённая транзакция не записывает
  частичный результат и не показывает ложное сообщение об успехе. Невалидный
  открытый проход по-прежнему получает своё точное сообщение об ошибке, а не
  общий блокер миграции стен. Компактная карточка пространства также не теряет
  после миграции двери, окна, ворота и проходы, привязанные к стенам, включая
  физический разрыв кладки в месте прохода.
  Окна на стенах продолжают пропускать солнечные лучи после правки, частичные
  открытые границы сохраняются и на уже мигрированных планах, а удалённая
  толщина стены больше не восстанавливается в прежнее значение.
  Неоднозначная старая геометрия остаётся без изменений с подсказкой по
  восстановлению. Кнопки редактора плана и поведение стен нулевой толщины не
  менялись
  ([#282](https://github.com/Matysh/houseplan-card/issues/282)).
...
- Доступность Resize на реальных планах теперь защищена точным аудитом каждой
  стены через production safety-resolver. Отключённые ручки понятнее объясняют
  реальное геометрическое ограничение; click/tap, Enter и Space показывают одну
  и ту же причину, а для перекрывающей отдельной перегородки, чернового контура
  или колонны явно предлагается безопасное действие — удалить или переместить
  объект ([#292](https://github.com/Matysh/houseplan-card/issues/292)).
- Активная ручка Resize теперь действительно следует за указателем на
  проблемной планировке второго этажа, а не остаётся неподвижной. Жест считает
  смещение от точки нажатия, сохраняет одну непрерывную запись толщины боковой
  стены, продолжает работать за пределами ручки благодаря pointer capture и
  останавливается на последнем отображаемом preview с явным сообщением вместо
  молчаливого бездействия. Undo остаётся доступным после подтверждения записи,
  в том числе для старого плана, который storage-барьер ремонтирует при
  сохранении
  ([#293](https://github.com/Matysh/houseplan-card/issues/293)).
- Все сохраняемые координаты плана теперь приводятся к точному общему узлу
  сетки `1/240`, если отличаются от него только невидимым хвостом вычислений.
  Авторская off-grid/диагональная геометрия не сдвигается. «Оптимизировать
  планы» отдельно показывает число очищенных значений, максимальный
  субмиллиметровый сдвиг и только затронутые этажи, а обычные записи
  config/layout не дают шуму вернуться
  ([#291](https://github.com/Matysh/houseplan-card/issues/291)).
- Resize теперь до начала жеста проверяет обе боковые стены. Движение упирается
  в точный узел сетки, где стена перестала бы быть общей или стала бы общей, а
  жест без безопасного направления отключается. Поэтому общая толщина 20 см
  больше не растягивается на новый наружный участок
  ([#289](https://github.com/Matysh/houseplan-card/issues/289)).
- Короткий луч в многостенном узле больше не позволяет локальной маске фаски
  вырезать общую стену, примыкающую к его дальнему концу. Четыре разрыва по 45
  шагов на реальном плане второго этажа закрыты во всех потребителях геометрии
  стен без удлинения короткого луча и без изменения сохранённого плана
  ([#288](https://github.com/Matysh/houseplan-card/issues/288)).
- Инструмент «Стены» теперь сохраняет визуально горизонтальный или вертикальный
  отрезок строго по оси и не создаёт незаметный уклон в один шаг.
  **Оптимизировать планы** может явно выпрямить такой дефект в старом плане:
  общая стена считается один раз, максимальный сдвиг показывается до
  подтверждения, настоящие диагонали сохраняются, а Cancel и одноуровневый Undo
  делают операцию обратимой
  ([#290](https://github.com/Matysh/houseplan-card/issues/290)).

## v1.67.0-beta.9 — 2026-08-24

- «Оптимизировать планы» теперь безопасно убирает точные лишние перегородки
  поверх внешних стен комнат, включая неизменное перепривязывание размещённых
  на них окон. После этого «Изменить размер» действительно двигает доступную
  общую стену; ручка без единого допустимого шага больше не выглядит активной
  и объясняет причину блокировки
  ([#281](https://github.com/Matysh/houseplan-card/issues/281)).
- Backend теперь принимает доказанное преобразование совпадающей перегородки
  из Optimize: её проём сохраняется на той же общей стене и корректно проходит
  Apply/reload/Undo. Произвольное снятие host обычной записью или поддельным
  candidate по-прежнему отклоняется
  ([#280](https://github.com/Matysh/houseplan-card/issues/280)).
...
- Локальный сбой объединения стен больше не скрывает кладку всего пространства.
  Валидные компоненты стен одинаково остаются видимыми в Plan, View, Static,
  скрытой изометрии и световых барьерах, а любое изменение физической геометрии
  проверяется до истории и записи. Небезопасное изменение отменяется с явным
  сообщением; настройки, не меняющие геометрию, остаются доступны для
  восстановления и экспорта
  ([#278](https://github.com/Matysh/houseplan-card/issues/278)).
- «Изменить размер» теперь двигает только доказуемо безопасную горизонтальную
  или вертикальную стену комнаты. Точная общая стена может изменить размеры не
  более двух комнат и упирается в первый угол, косяк проёма, третью комнату или
  другое изменение топологии; частичные, наклонные и неоднозначные стены
  остаются видимыми, но объясняют причину блокировки. Preview и сохранение
  используют один результат с неизменной топологией, поэтому ресайз больше не
  разрезает записи толщины, не оставляет фрагменты стен и не заставляет кладку
  пространства исчезнуть. Если записи толщины или виртуальных отрезков нельзя
  перенести без потерь, стена видимо останавливается в последней безопасной точке
  ([#277](https://github.com/Matysh/houseplan-card/issues/277)).
- «Оптимизировать планы» теперь исправляет точную лишнюю перегородку,
  нарисованную поверх одной сплошной общей стены комнат. Её дверь, окно или
  ворота перепривязываются к стене без сдвига и потери датчиков, сохраняется
  более широкое из двух соосных тел, а «Граница» и «Толщина» затем работают с
  единственной канонической стеной. Частичные и неоднозначные совпадения не
  меняются ([#276](https://github.com/Matysh/houseplan-card/issues/276)).

## v1.67.0-beta.7 — 2026-08-24

- В перпендикулярных T/X-стыках теперь сохраняется полная конечная ширина
  каждой участвующей стены, в том числе у соседних стыков с пересекающимися
  окнами обработки. В Plan, View, Static, скрытой изометрии, подложке/поле и
  световых барьерах больше нет соединённых с фоном белых уголков, видимой
  потери толщины и крупной выемки при смешанной толщине. «Оптимизировать планы»
  может корректно остаться no-op: валидные сохранённые оси и толщины не
  переписываются ([#275](https://github.com/Matysh/houseplan-card/issues/275)).

## v1.67.0-beta.6 — 2026-08-24

- Беспроводной контроллер теперь одинаково показывает доступность и работу на
  плане и в preview диалога устройства, даже если отдельно удалённый маркер
  цели отфильтровывает настроенный свет из runtime-реестра. Живые диагностические
  сущности battery/LQI/update по-прежнему подтверждают связь с контроллером, а
  жёлтым его делает только активная управляемая цель
  ([#274](https://github.com/Matysh/houseplan-card/issues/274)).
- «Оптимизировать планы» теперь удаляет микроскопическую ступень толщины рядом
  с T-стыком, если короткий фрагмент ограничен двумя участками одинаковой
  толщины одной прямой стены. T-узел и перпендикулярная стена остаются на
  месте; фрагменты между двумя топологическими узлами или на границе проёма
...
- В стыках нескольких стен больше не остаются замкнутые белые треугольные
  дыры, когда допустимая ограниченная фаска соприкасалась с внешней пустотой
  лишь одной математической точкой. У фаски появился конечный локальный выход
  в окружающую комнату/фон, но предел `1.25 × H`, осевые линии ветвей и защита
  от длинных mitre-зубов не изменились. Геометрические, браузерные и golden-
  инвентари теперь требуют ноль замкнутых дыр в Plan, View, kiosk, Static,
  скрытой изометрии и световых/солнечных барьерах
  ([#272](https://github.com/Matysh/houseplan-card/issues/272)).

## v1.67.0-beta.5 — 2026-08-23

- После удаления целого HA-устройства с плана теперь можно отдельно вернуть
  любую его активную сущность через **Добавить → Показывать сущности**.
  Выбранная сущность появляется как обычный живой маркер на свежей позиции, а
  родительское устройство и невыбранные siblings остаются удалёнными; само
  устройство по-прежнему доступно для осознанного последующего возврата
  ([#262](https://github.com/Matysh/houseplan-card/issues/262)).
- В толстых T-стыках больше не появляются белые треугольные щели с наружной
  стороны, когда сходятся три и более стены разной толщины. Ограниченная фаска
  теперь сохраняет допустимую кладку и подложку до одного предела по обе стороны
  контура комнаты — одинаково в Plan, View, kiosk, Static, скрытой изометрии и
  световых/солнечных барьерах — и не возвращает прежний длинный mitre-зуб
...
- Изменение размера комнаты больше не превращает толстую стену в тонкую осевую
  линию, когда перемещаемое ребро покрывает только часть более длинной стены.
  Покрытая часть теперь сохраняет толщину и следует за комнатой, нетронутое
  продолжение остаётся на месте, а проём на перемещаемой части переносится в
  той же операции Undo/Redo
  ([#253](https://github.com/Matysh/houseplan-card/issues/253)).
- «Оптимизировать планы» теперь удаляет сохранённую позицию, только когда
  доказано, что её подписи комнаты, устройства или групповой метки больше не
  существует. Предпросмотр описывает очистку понятными категориями, прячет
  технические id в свёрнутые «Подробности» и сохраняет живых либо непроверенных
  владельцев. Отдельная явная опция позволяет включить старые позиции живых
  объектов в основной Apply; Cancel ничего не записывает, а служебная отмена
  восстанавливает прежнюю раскладку целиком. Это уточняет обещание v1.59
  сохранять все непривязанные записи: временно недоступные и непроверенные
  объекты по-прежнему защищены, а доказанно забытые позиции больше не
  накапливаются
...
- Беспроводной выключатель или другой контроллер больше не выглядит
  недоступным только из-за недоступности настроенной лампы или группы света.
  Полупрозрачность теперь определяется собственными живыми сущностями
  контроллера, включая батарею и Zigbee LQI, а цели по-прежнему определяют
  жёлтое рабочее состояние. Если ни одна настроенная цель недоступна, нажатие
  теперь называет её и объясняет, что действие не выполнено
...
- Символы дверей, окон и ворот теперь остаются точно на осевой линии стены при
  обоих направлениях открывания. Настройка «Открывается в другую сторону»
  только зеркалит геометрию двери/окна либо меняет знак 10° поворота ворот и
  больше не сдвигает уже сохранённую перевёрнутую дверь или окно к грани стены.
  Косяки и вырез стены по-прежнему проходят через полную физическую глубину во
  Flat, Static и скрытой изометрии
  ([#250](https://github.com/Matysh/houseplan-card/issues/250)).

- Повторный запуск «Оптимизировать планы» сразу после успешной оптимизации
  теперь честно сообщает, что исправлять нечего, в том числе после события
  обновления backend или перезагрузки страницы. Optimize сравнивает и оставляет
  то же девятизнаковое представление геометрии, которое сохраняет интеграция,
  поэтому двоичный шаг сетки `1/240` больше не возвращается как заново
  устранённый шум координат
  ([#248](https://github.com/Matysh/houseplan-card/issues/248)).

- «Оптимизировать планы» теперь восстанавливает ссылки устройств, оставшиеся
  после удаления или независимого импорта пространства. Для точной импортной
  копии сохраняются позиции; в остальных случаях активное устройство теряет
  только отсутствующее размещение и возвращается через свою Area HA либо в
  первое пространство. Предпросмотр отдельно показывает исправленные и
  неразрешённые ссылки. Пространство с активными устройствами удалить нельзя,
  пока остаётся другое пространство; единственное оставшееся можно удалить без
  потери привязок и настроек маркеров. Импорт пространства исправляет совпавшие
  ссылки в текущем плане, а редактор карточки предупреждает, если `default_floor`
  указывает на отсутствующее
  пространство
...
- Символы дверей, окон и ворот теперь по умолчанию находятся на центральной
  ...
  а флаг без повторной зеркальной инверсии меняет направление их 10° открытия
  на общей стене комнат и независимой стене. Косяки по-прежнему проходят через
  полную глубину стены
