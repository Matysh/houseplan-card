...
- Если Home Assistant отклоняет сохранение устройства или калибровки робота,
  план теперь возвращается к последнему подтверждённому состоянию, но сохраняет
  открытый редактор, автоматическое предложение или ручную подгонку для повтора;
