...
- `Shift` не отключает позиционную сетку; он используется только для явно угловых действий: свободного поворота декора и грубого шага компаса 15°.
...
| Подложка | Отменяет черновик, выбор или текущий инструмент | Отдельной истории отмены нет | — |
...
| Управляет источниками света | Группа `light.*`/`switch.*`, переключаемая вместе |
| Это источник света | Позволяет реле/выключателю создавать световое пятно |
...
| Иконка | Ручная `mdi:*`; пусто — автоматический подбор |
...
Удаление автоматически найденного маркера фактически скрывает его, чтобы автообнаружение не вернуло значок. Виртуальный маркер удаляется полностью.
...
Приоритет статусов: **авария → работа → открыто → нейтрально**. Если все сущности, реально описывающие маркер, недоступны, маркер бледнеет.
...
| Жёлтая подложка | Устройство сейчас выполняет работу | Свет/реле/вентилятор/увлажнитель `on`; climate реально heating/cooling; media playing; vacuum cleaning/returning; script on; техника washing/drying/etc. |
...
| Бледный маркер | Данные недоступны | Все рабочие сущности маркера `unknown`, `unavailable` или отсутствуют |
...
| Работа | Пока устройство работает | Свет, switch, fan, humidifier, climate action, media playing, vacuum cleaning, script и известные рабочие состояния техники |
...
| `media_player.*` | Жёлтый при `playing` | Работа при `playing` | Нет | Внутренняя карточка |
...
|---|---|---|
| Выбрать | Клик выбирает объект | Drag перемещает; двойной клик всегда открывает свойства; `Delete`/`Backspace` удаляет |
| Подложка | Доступна, если есть изображение | Drag перемещает изображение, угловая ручка равномерно масштабирует; позиционная привязка обязательна |
| Линия | Drag между двумя точками | Цвет и толщина; показываются длина и угол |
| Прямоугольник | Drag по диагонали | Цвет, толщина и заливка; показываются ширина × высота |
| Овал | Drag по габаритам | Цвет, толщина и заливка; показываются ширина × высота |
| Текст | Клик открывает диалог | Многострочный текст, переменные HA, цвет; масштаб и поворот ручками |
| Мебель | Выбор символа и клик по центру | Размер, цвет, толщина, поворот; магнитится к стене |
| Стереть | Клик по объекту | Удаляет сразу |

Новые фигуры создаются не меньше половины клетки. После создания объект остаётся выбранным. Создание, движение и изменение размера всегда квантуются по сетке.
...
| Линия | Цвет и толщина линии |
| Прямоугольник/овал | Цвет, толщина и заливка |
| Мебель | Цвет и толщина контура |
...
- меняет ширину и глубину независимыми угловыми ручками;
