...
Составная кнопка в режиме просмотра открывает шестерёнкой **Настройки сводной
панели**, а второй половиной показывает или скрывает её. На новом экране панель
выключена, пока пользователь не включит её сам. Состав панели общий для всей
установки House Plan; выбор показа и два ползунка размеров View относятся
только к текущему пользователю Home Assistant и конкретной карточке или
странице в боковом меню. Пользователь без права редактирования и киоск могут
менять эти локальные параметры, но не общее содержимое.
...
Два ползунка размеров сохраняются для текущего пользователя и карточки и не
сбрасываются при обновлении конфигурации. На узком экране или при увеличенном
системном тексте подпись и процент располагаются над полноширинным ползунком,
поэтому Сохранить, Отмена и Сброс доступны без горизонтальной прокрутки. Уход
со страницы House Plan, переподключение под другим пользователем или смена
права редактирования закрывает открытый черновик, а не переносит его в новый
контекст.

Панель висит поверх плана и не уменьшает его рабочую область. Если ширина
