...
открывает компактное подменю **Окно / Дверь / Ворота**. После выбора типа
...
переключает её собственное ручное состояние. Новая лампа начинается
включённой; состояние общее для всех полных и статических карточек, сохраняется
после перезагрузки страницы и Home Assistant и одинаково меняет Glow, заливку
«Свет» и статистику комнаты. Переключать может любой вошедший пользователь
дашборда — права редактора плана для этого не требуются.

Это точное исключение из обычной схемы `controls`. Пока три условия активны,
сохранённые внешние `controls` не вызывают сервисы HA и не переопределяют
ручное состояние, но остаются в настройках без потерь. Смена роли, привязки
или действия возвращает обычное управление; при повторном включении ручного
режима лампа снова начинается включённой. Скрытие сохраняет состояние, удаление
маркера очищает его. Ручное состояние не является entity/helper HA и не входит
в экспорт или импорт плана.
