...
| Поле с единицей | Число и `см`, `°C`, `м`, `%` или `°` внутри рамки | Пустое поле означает «как в общих настройках» или «как у пространства», если так написано в подсказке |
...
правки. Неверное значение — пустая или нечисловая
граница температуры, север вне 0–359°, не выбранная привязка — подписано под
полем красным, а ссылка **Проверить N полей** в футере ведёт к первому такому
полю.
