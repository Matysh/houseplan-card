...
толщины, роли и связей источника света, а также фильтра **Показывать скрытые на
плане**. Для стен нулевой толщины подсказка дополнительно напоминает: пунктир
пропускает Glow и солнечные лучи, сплошная линия блокирует их.
...
Диалог комнаты собран в четыре карточки-группы: **Основное** (имя и зона),
**Заливка** (режим заливки этой комнаты, свой цвет и пороги температуры),
**Источники** (датчики температуры и влажности) и **Размеры шрифтов** (масштабы
названия и подписи с предпросмотром). Пояснение к каждой группе открывается
кнопкой «?» рядом с её заголовком. Источник измерения выбирается переключателем
«среднее по комнате / выбранный датчик»: под ним это по-прежнему обычные
радиокнопки, поэтому стрелки и чтение с экрана работают как раньше.
...
| Скрыть / Показать | Кнопка в левом нижнем углу диалога. После сохранения убирает или возвращает значок; LQI и климатические данные скрытого устройства всё ещё могут участвовать в среднем назначенной комнаты |
...
соответствующей границе пространства. Кнопка «Как у пространства» очищает оба
переопределения. Эти значения меняют только цвет пола комнаты и продолжение
...
«Устройство на плане» — собраны в карточки-группы с заголовками. Пояснение к
группе или отдельной настройке открывается кнопкой **«?»** рядом с её
заголовком; предупреждения о том, что настройка сломана прямо сейчас, остаются
видимыми абзацами и под «?» не прячутся.
