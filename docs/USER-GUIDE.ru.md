...
ссылается на это пространство, его комнату или сохранённую на нём позицию.
Сначала перенесите или удалите такие устройства; затем подтверждённое удаление
уберёт пространство и принадлежащую ему раскладку. Файл подложки и вложения при
этом автоматически не удаляются.
...
попадающий в виртуальный участок разрыв нужно соединить вручную.
...
| Ссылки устройств | Точная подпись независимого импорта восстанавливает пространство, комнату и позицию. Иначе реальное устройство следует однозначной Area HA либо теряет только мёртвую привязку и старые координаты; настройки маркера сохраняются |
...
Неразрешённые позиции и вложенные сопоставления робота не удаляются: диалог
показывает их отдельным предупреждением даже тогда, когда применять больше
нечего.
