...
управляют одной лампой по OR. Если не связать пассивную лампу ни с одним
контроллером, явная роль **Всегда** означает постоянно включённый источник.
`marker:*` является только внутренней ссылкой плана и никогда не отправляется
в Home Assistant как entity ID.
