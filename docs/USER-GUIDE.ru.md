...
Кнопка **Сохранить** активна только когда что-то изменилось; рядом в футере
написано «Есть несохранённые изменения». Если закрыть диалог крестиком,
кнопкой **Отмена** или щелчком мимо него при несохранённых изменениях, House
Plan сначала спросит, сбросить ли их. Неверное значение — пустая или нечисловая
...
| Внешний вид | Заливка комнат | Сегмент «Свой цвет / Сигнал Zigbee / Свет / Температура»; при температуре ниже появляются два поля границ °C с легендой |
...
его — ниже появится сегмент из пяти режимов (Нет / Сигнал Zigbee / Свет /
...
Диалог устройства собран в пять карточек: **Основное**, **Действие по
нажатию**, **Свет и свечение**, **Внешний вид** и **Подробности**. Слева в футере
— **Скрыть** и **Удалить**, справа — **Отмена** и **Сохранить**; последняя активна
...
| Использовать температуру climate | Добавляет `current_temperature` термостата/кондиционера в маркер и среднее назначенной ему комнаты |
...
У распознанного радара в настройках маркера появляется раздел **«Присутствие
на плане»**. Включите радар, проверьте точные источники Home Assistant и
выберите комнату, контуром которой ограничиваются наблюдения. Для неизвестного
реального устройства используйте **«Дополнительные действия → Это радар
присутствия»** и явно задайте профиль данных. House Plan не угадывает единицы,
направления осей или ориентацию по названиям сущностей.
