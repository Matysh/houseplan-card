...
| Проём | Дверь, окно или ворота на стене | Размер, ориентацию, контактный датчик и, для двери/ворот, замок |
...
| Заливка | Нет/LQI/свет/температура/свой цвет | Выбирает данные или постоянный цвет заливки комнат пространства |
| Свечение источников света | Вкл/выкл | Добавляет световые пятна; базовое затемнение используется только без другой заливки |
...
3. При необходимости укажите толщину новых стен в появившейся над холстом
   панели параметров. `0`/пусто создаёт тонкую осевую границу.
...
в диалоге действие **Оставить замкнутыми стенами**.
...
| Opening | Точку на стене | Создаёт дверь, окно или ворота; клик существующего проёма открывает его настройки |
...
## 9. Двери, окна, ворота и замки

Проём всегда привязан к стене. Дверь по умолчанию создаётся шириной 90 см, окно — 120 см, ворота — 300 см. Допустимый размер в интерфейсе: 20–600 см с шагом 5 см.
...
| Поле | Дверь | Окно | Ворота |
|---|---:|---:|---:|
| Размер | Да | Да | Да |
| Контактный датчик | Да | Да | Да |
| Инверсия датчика | Да | Да | Да |
| Петли с другой стороны | Да | Да | Нет: створки симметричны |
| Открывается в другую сторону | Да | Да | Да; по умолчанию — наружу |
| Замок `lock.*` | Да | Нет | Да |
...
Тело стены вырезается по ширине проёма, откосы идут через всю глубину стены. Створка/дуга двери и окна смещается к внутренней грани комнаты, а створки ворот — к наружной. Свет проходит через дверь или ворота по ширине внутреннего тоннеля и отсекается откосами. Солнечный луч начинается из внутренних углов оконного тоннеля.
...
|---|---:|---:|---:|---:|
| Значок + динамическая подложка | Да | Да | Нет | Отдельные компактные °/% и LQI возможны |
| Иконка + активность | Да | Да | Да | Отдельные компактные °/% и LQI возможны |
| Значение вместо иконки | Если значение получить нельзя или источник неоднозначен | Да | Нет | Температура/влажность либо числовое или текстовое состояние, отформатированное HA |
...
фактическую сущность отображения. Для «Иконка + активность» можно безопасно
запустить локальный пример кольца на 3,3 секунды: состояние HA, конфигурация и
эффект marker на плане при этом не меняются.
...
| Составное switch-only устройство с отдельной сущностью Power | Нейтральное при Power=`on`; приглушённое при Power=`off`/`unknown`/`unavailable` | Нет: соседние переключатели режимов и функций не считаются работой всего устройства | Обычно нет | Внутренняя карточка |
...
режимом заливки. При режиме «Нет» оно добавляет базовое затемнение и световые
пятна. При LQI, свете, температуре или своём цвете базовое затемнение не
рисуется: исходный цвет и прозрачность заливки сохраняются точно, а световые
пятна остаются видимыми поверх неё. Новое вручную созданное пространство по
...
от отдельного маркера реального `light.*` либо от маркера с явным флагом «Это
источник света». Без явных настроек автоматически учитываются собственные
...
| Реле не светит в Glow | Включите «Это источник света». `controls` управляет внешними лампами, но не переносит их Glow на координаты реле; реальные лампы должны иметь свои маркеры |
