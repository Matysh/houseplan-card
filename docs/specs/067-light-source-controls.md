...
SVG-слой получает готовые `{c, alpha}` — так формула не разойдётся между пулом, проливом и тестами.
...
7. «Задать цвет» применяет цвет к пятну и проливу, яркость продолжает следовать источнику.
...
- Ручной цвет: стопы градиента маркера несут выбранный цвет, соседняя авто-лампа — свой; пролив
  через проём совпадает по цвету с пятном.
