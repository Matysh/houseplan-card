...
- Автор и владелец репозитория явно подтвердил в запросе на реализацию, что все
  иконки нарисованы им собственноручно. В репозиторий попадает нормализованная
  копия с `author: Matysh`, `license: MIT`; исходный `TBD` из архива не
  переносится как релизная метаинформация.
...
Vendored source соответствует зафиксированному архиву по всем 77 SVG;
нормализованы только метаданные author/license/pack ID и документация. Генератор
отклоняет запрещённый SVG и `--check` подтверждает актуальность двух generated
каталогов.
