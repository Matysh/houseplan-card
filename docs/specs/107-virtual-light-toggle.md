...
> **Уточнение #174:** это ТЗ остаётся authority для несвязанного ручного
> режима. Если на exact marker существует валидная входящая связь controller,
> HA state реальных drivers имеет приоритет, а клик по лампе выполняет HA group
> operation. Снятие последней связи возвращает сохранённый manual state; полный
> контракт описан в [ТЗ #174](174-linked-virtual-light-controller.md).
