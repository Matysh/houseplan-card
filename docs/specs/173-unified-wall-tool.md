...
> **Частично заменено #306.** Единый инструмент **«Стены»** остаётся
> каноническим, но все положения этого документа об отдельном инструменте
> **«Граница»**, virtual/open-wall UX и записи `open_spans/open_to` больше не
> действуют. Нулевая толщина создаётся и редактируется как обычная стена по
> `docs/specs/306-zero-thickness-walls.md`; модель v9 хранит только `cm:0`.
