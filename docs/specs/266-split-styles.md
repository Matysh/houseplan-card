...
Статус: ревизия 3 (после SPEC-REVIEW-266-r2: AC2 сведён с арифметикой финального прототипа, счёт reduced-motion блоков исправлен на фактические 10).
...
2. Пять файлов `src/styles/*.styles.ts`; замер r1-прототипа (4 431 суммарно) не проходил собственный порог — источником роста были пустые строки-разделители генератора; после уплотнения (блоки стык в стык, как в исходнике) фактический замер: base 248, chrome 381, devices 540, dialogs 1 180, plan 1 359, сборщик 19 — сумма 3 727 ≤ 3 690 + 5 % (3 874). Порог: каждый файл ≤ 1 650, сумма ≤ исходник + 5 %. Доказательство: ревью кода (`wc -l`).
...
6a. Юнит присутствия медиа-обёрток: оба блока `@media (forced-colors: active)` и все **10** блоков `@media (prefers-reduced-motion: reduce)` присутствуют в итоговой склейке с исходными правилами (фактический счёт по `grep -n` исходника: строки 72, 255, 399, 691, 875, 988, 1995, 2000, 2768, 3537 — в r1/r2 фигурировали «8», это была неточность обоих раундов). Доказательство: `unit`.
