...
Статус: ревизия 2 (после SPEC-REVIEW-266-r1: scope-ключ сверки, обязательные смоки медиа-обёрток, фактические пороги размера, класс B для scripts/**).
...
2. Пять файлов `src/styles/*.styles.ts`; фактический замер прототипа расщепления: base 300, chrome 447, devices 623, dialogs 1452, plan 1609 строк — порог: каждый ≤ 1 650, сумма пяти файлов + сборщик ≤ исходных 3 690 строк + 5 % (накладные заголовки/импорты). Доказательство: ревью кода.
...
6a. Юнит присутствия медиа-обёрток: оба блока `@media (forced-colors: active)` и все 8 блоков `@media (prefers-reduced-motion: reduce)` присутствуют в итоговой склейке с исходными правилами. Доказательство: `unit`.
