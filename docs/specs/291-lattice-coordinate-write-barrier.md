...
Placeholder `{cm}` передаёт измеренный maximum с тремя значащими цифрами:
обычной десятичной записью при `|cm| >= 0.001`, с удалением незначащих нулей,
и scientific notation при `0 < |cm| < 0.001`. Ненулевой maximum никогда не
округляется до `0` и не использует политику округления вверх до `0.1 см` из
обычного grid-align report; например, `0.000033 cm` показывается как
`3.30e-5 cm`.
