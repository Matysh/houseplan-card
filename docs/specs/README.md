...
Единственный канонический backlog проекта — GitHub Issues, а статус задачи —
её метка `S1-new`…`S8-merged` (`PROCESS.md` §9; GitHub Projects v2 не
используется с 2026-08-14, #139). Этот каталог содержит развёрнутые ТЗ полного
трека: каждое ТЗ ссылается на issue, а issue — на соответствующий файл. На лёгком
треке `small` (путь по умолчанию, #338) ТЗ живёт в теле issue и файла здесь не
имеет. Статус, приоритет и факт завершения меняются только в GitHub — словесные
пометки в файлах ТЗ («готово к реализации», «реализовано») справочные и статусом
не являются.
...
| [#514](https://github.com/Matysh/houseplan-card/issues/514) E2E на реальном HA как гейт стабильного релиза | [514-e2e-stable-release-gate.md](514-e2e-stable-release-gate.md) |
