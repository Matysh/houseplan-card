...
- Ревизия: 4 (2026-08-30) — по SPEC-REVIEW-44-r2 (M1 + Low); policy v1 принята
...
Хозяин плана открывает «Устройства» и на вкладке «Доступны» видит новый
...
- Раскрываемая секция внизу вкладки «Доступны» диалога «Устройства»
...
- Пересчёт превью на большом registry — переиспользуется кэшируемый
  ...
  на каждый тик.
- Текст причины обогащается плейсхолдером на уже существующей вкладке
  «Доступны» — новых классов записей не появляется; смок проверяет
  отсутствие дублей.
...
- Размещение — вкладка «Доступны» (фильтры определяют её содержимое);
...
- Причина остаётся на «Доступны», где уже живёт (#29); ни переноса
  категории, ни новой вкладки.
