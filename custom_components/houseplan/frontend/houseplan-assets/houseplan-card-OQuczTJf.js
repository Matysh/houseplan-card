    ...
    <span class=${l} aria-hidden="true">
      ...
      <span class="device-core">
        ${null!=e.valueText?B`<span class="valtext" title=${e.valueFullText||e.valueText}
              style=${`--value-font-scale:${qu(e.valueFullText||e.valueText)}`}
      ...
      ${a?B`<span class="device-sections">
        ${o?B`<span
              ...
              title=${nu(o)}
              style=${`--value-font-scale:${qu(o.fullText||o.text)}`}
            >${o.text}</span>`:G}
        ${r.map(e=>B`<span
          class="value-badge legacy-secondary available tone-${e.kind}"
          title=${e.text+e.suffix}
          style=${`--value-font-scale:${qu(e.text+e.suffix)}`}
        ...
        <div class="stage ${this._markup?"markup tool-"+this._tool+("split"!==this._tool||this._splitSel?"":" pickstage")+("wallthick"===this._tool&&this._wallThickHover?" wallhot":""):""} ${"decor"===this._mode?"dtool-"+this._decorTool:""} ${o.bg?"":"noplan"} mode-${this._mode}${this._bdMovable?" bdgrab":""}${this._bdDrag?" bdgrabbing":""}${P?` daycycle phase-${P.phase}`:""}${this._booting?" hpboot":""}${this._bootSoft?" hpsettle":""}${this._modeTransitionBusy?" mode-transition":""}"
          ...
          <svg class=${_?.structural?"plan-svg":G}
            ...
            ${(()=>{const e=this._openCuts(),t=this._thickWallCuts(),i=this._wallUnionGeometry()?.roomGeom,n=new Map,r=e=>(n.has(e)||n.set(e,Zt(e)),n.get(e));return o.rooms.filter(e=>e.area||"view"===this._mode||this._markup||p.showBorders).map(n=>{let s="room "+(o.bg?"overlay":"yard")+(this._markup?" outlined":"");!this._markup||n.id!==this._mergeSel&&n.id!==this._splitSel?.roomId||(s+=" picked");let a="";const l=Rn(p.fill,n);if(!this._markup&&(p.showBorders||"none"!==l)){s+=" styled";const e=[];e.push(`--room-stroke:${p.color}`,`--room-stroke-op:${p.showBorders?p.opacity:0}`);const t=g.byRoom.get(n)||null;t?(s+=" filled",e.push(`--room-fill:${t.color}`,`--room-fill-op:${t.opacity}`)):e.push("--room-fill:transparent","--room-fill-op:0"),a=e.join(";")}let c;const h=e=>{"view"===this._mode&&(void 0===c&&(c=this._roomArea(n)),this._showTip(e,n.name||this._t("room.unnamed"),c?this._t("tip.area",{value:c}):"",v?this._roomLqi(n.area):null,this._roomTemp(n),this._roomHum(n)))},d=r(n),u=this._markup&&(n.id===this._mergeSel||n.id===this._splitSel?.roomId),_=d&&!u?e.filter(e=>{const t=[(e[0]+e[2])/2,(e[1]+e[3])/2];return d.some((e,i)=>Pn(t,[e[0],e[1],...d[(i+1)%d.length]])<=.08*this._gridPitch)}):[],m=u?[]:t,f=_.concat(m);f.length&&(s+=" noedge");const y=this._spaceWalls.length&&n.id&&d&&this._innerRoomContour(o,n.id,e,i)||d,b=y?vi(y,(w=n,o.rooms.filter(e=>e!==w).map(r).filter(Boolean))):[];var w;const k=e=>"M "+e.map(e=>e[0]+" "+e[1]).join(" L ")+" Z",x=y?this._cleanFloor(n,y,o).path:"",$=n.id||G,S=n.area||G,M=x&&y?W`<path class="${s}" style="${a}" fill-rule="evenodd"
                    ...
                    @pointerenter=${e=>{this._notePointer(e),this._pointerModality.hoverEnabled&&(this._hoverRoom={space:o.id,room:n})}}
                    @pointermove=${h}
                    @pointerleave=${()=>this._clearTransientHover()}></rect>`,R=f.length&&d?kn(d,f,.02*this._gridPitch):null,T=R?W`<path class="room-outline ${this._markup?"outlined":""}"
                    d="${R.map(e=>`M ${e[0]} ${e[1]} L ${e[2]} ${e[3]}`).join(" ")}"
                    style=${this._markup?G:`stroke:${p.color};stroke-opacity:${p.showBorders?p.opacity:0}`}></path>`:G;return W`${M}${T}`})})()}
          ...
          ${T?B`<div class="measurelayer">${T.map(e=>B`<div
          ...
          ${R?B`<div class="measurelayer"><div
                ...
                style="left:${((R.x-k.x)/k.w*100).toFixed(2)}%;top:${((R.y-k.y)/k.h*100).toFixed(2)}%">${R.text}</div></div>`:G}
          </div>
