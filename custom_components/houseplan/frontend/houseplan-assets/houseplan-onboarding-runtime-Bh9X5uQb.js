globalThis.__HOUSEPLAN_BUILD_FINGERPRINT__="967d2749e25a30292223ad85baf8d03e0d0d2840b00ee6e6a5876ab3292a4f76";import{bU as t,bV as s,bW as e,bX as o,aq as a,bY as l,c1 as i,bZ as h,b_ as n,b$ as c,c0 as r,b as p,A as _,cV as d,ap as u,ar as g,cW as m}from"./houseplan-card-DJctcs0x.js";import{i as b,c as $,a as v,s as y,t as f}from"./space-deletion-BIiYcPJz.js";const w=1e3,D=t=>{if(!t.trim())return null;const s=Number(t.replace(",","."));return Number.isFinite(s)?s:null},S="967d2749e25a30292223ad85baf8d03e0d0d2840b00ee6e6a5876ab3292a4f76";class C{constructor(t){this.host=t,this._toggleServerPlans=async()=>{const t=this.host._spaceDialog;if(t)if(t.pickSaved)this.host._spaceDialog={...t,pickSaved:!1};else{this.host._spaceDialog={...t,pickSaved:!0,savedBusy:!0};try{const t=await this.host.hass.callWS({type:"houseplan/plans/list"}),s=this.host._spaceDialog;s&&(this.host._spaceDialog={...s,saved:t?.plans||[],savedBusy:!1})}catch(t){const s=this.host._spaceDialog;s&&(this.host._spaceDialog={...s,saved:[],savedBusy:!1}),this.host._showToast(this.host._t("toast.plans_list_failed",{err:this.host._errText(t)}))}}}}_openSpaceDialog(p,_){if(!this.host._serverStorage||!this.host._serverCfg)return void this.host._showToast(this.host._t("toast.integration_missing"));if("edit"===p){const i=this.host._serverCfg.spaces.find(t=>t.id===_);if(!i)return;const h=t(i),n=i.settings?.custom_fill&&"object"==typeof i.settings.custom_fill?s(i.settings.custom_fill):null,c="none"===h.fill?{...n||e,a:0}:n;return void(this.host._spaceDialog={mode:p,spaceId:_,title:i.title,planUrl:i.plan_url||null,planFile:null,source:i.plan_url?"file":"draw",showBorders:h.showBorders,showNames:h.showNames,zeroWallStyle:l(i),displayTouched:!0,hideDecor:h.hideDecor,hideOpenings:h.hideOpenings,roomColor:h.color,roomOpacity:h.opacity,fillMode:"none"===h.fill?"custom":h.fill,customFill:c,glowEnabled:h.glow,bgColor:h.bgColor,bgMode:"static"===i.settings?.bg_mode||"daynight"===i.settings?.bg_mode?i.settings.bg_mode:null,northDeg:a({},i.settings),sunRays:"boolean"==typeof i.settings?.sun_rays?i.settings.sun_rays:null,tempMin:h.tempMin,tempMax:h.tempMax,showLqi:h.showLqi??this.host._config?.show_signal??!0,cardFontScale:h.cardFontScale,labelTemp:h.labelTemp,labelHum:h.labelHum,labelLqi:h.labelLqi,labelLight:h.labelLight,cellCm:Number(i.cell_cm)>0?Number(i.cell_cm):5,cellCmInput:o(Number(i.cell_cm)>0?Number(i.cell_cm):5,this.host._imperial),cellCmTouched:!1,busy:!1})}const d=i(this.host._imperial);this.host._spaceDialog={mode:p,title:"",planUrl:null,planFile:null,...b(),hideDecor:!1,hideOpenings:!1,zeroWallStyle:"dashed",roomColor:r,roomOpacity:c,fillMode:"custom",customFill:{...e,a:0},glowEnabled:!0,bgColor:null,bgMode:"daynight",northDeg:null,sunRays:null,tempMin:n,tempMax:h,showLqi:this.host._config?.show_signal??!0,cardFontScale:1,labelTemp:!1,labelHum:!1,labelLqi:!1,labelLight:!1,cellCm:d,cellCmInput:o(d,this.host._imperial),cellCmTouched:!1,busy:!1}}async _pickPlanFile(t){const s=t.target,e=s.files?.[0];if(!e||!this.host._spaceDialog)return;const o={"image/svg+xml":"svg","image/png":"png","image/jpeg":"jpg","image/webp":"webp"}[e.type]||(e.name.toLowerCase().endsWith(".svg")?"svg":"");if(!o)return void this.host._showToast(this.host._t("toast.plan_formats"));const a=new Uint8Array(await e.arrayBuffer());let l="";for(let t=0;t<a.length;t+=32768)l+=String.fromCharCode(...a.subarray(t,t+32768));const i=btoa(l),h=URL.createObjectURL(e),n=await new Promise(t=>{const s=new Image;s.onload=()=>t(s.naturalWidth&&s.naturalHeight?s.naturalWidth/s.naturalHeight:1.414),s.onerror=()=>t(1.414),s.src=h});URL.revokeObjectURL(h),this.host._spaceDialog={...this.host._spaceDialog,planFile:{ext:o,b64:i,aspect:n,name:e.name}}}_useServerPlan(t){const s=this.host._spaceDialog;s&&(this.host._spaceDialog={...s,planUrl:t,planFile:null,pickSaved:!1,savedAspect:void 0},this.host._aspectJob=this._readPlanAspect(t))}async _readPlanAspect(t){for(let s=0;s<40;s++){const s=this.host._display(t);if(s){const e=await new Promise(t=>{const e=new Image;e.onload=()=>t(e.naturalWidth&&e.naturalHeight?e.naturalWidth/e.naturalHeight:0),e.onerror=()=>t(0),e.src=s}),o=this.host._spaceDialog;return o&&o.planUrl===t&&Number.isFinite(e)&&e>0?(this.host._spaceDialog={...o,savedAspect:e},e):0}if(await new Promise(t=>setTimeout(t,150)),this.host._spaceDialog?.planUrl!==t)return 0}return 0}async _deleteServerPlan(t){if(confirm(this.host._t("confirm.delete_plan",{name:t})))try{await this.host.hass.callWS({type:"houseplan/plans/delete",name:t});const s=this.host._spaceDialog;s?.saved&&(this.host._spaceDialog={...s,saved:s.saved.filter(s=>s.name!==t)})}catch(t){this.host._showToast(this.host._t("toast.plan_delete_failed",{err:this.host._errText(t)}))}}_renderServerPlans(t){if(t.savedBusy)return p`<div class="savedplans muted">${this.host._t("space.loading")}</div>`;const s=t.saved||[];if(!s.length)return p`<div class="savedplans muted">${this.host._t("space.no_saved")}</div>`;return p`<div class="savedplans">
      ...
      icon="mdi:home-floor-1" @hp-close=${()=>this.host._importDialog=null}>
        <div class="body">
          ...
          ${t.floors.map((s,e)=>p`<label class="floorrow">
            ...
            <span>${s.name}</span>
            ${null!=s.level?p`<span class="floorlvl">L${s.level}</span>`:_}
          </label>`)}
        </div>
      ...
      icon="mdi:floor-plan" wide @hp-close=${l}>
        <div class="body">
          ...
          <label class="srcrow">
            <input type="radio" name="plansrc" .checked=${"file"===t.source}
              @change=${()=>this.host._spaceDialog=y(t,"file")} />
            <span>${this.host._t("space.source_file")}</span>
          </label>
          ${"file"===t.source?p`<div class="planrow">
              ${t.planFile?p`<span class="planname">${t.planFile.name}</span>`:t.planUrl?p`<img class="planprev" src=${this.host._display(t.planUrl)}
                      alt=${this.host._t("space.plan_alt")} />`:p`<span class="planname muted">${this.host._t("space.no_plan")}</span>`}
            ...
            ${t.pickSaved?this._renderServerPlans(t):_}`:_}
          <label class="srcrow">
            <input type="radio" name="plansrc" .checked=${"draw"===t.source}
              @change=${()=>this.host._spaceDialog=y(t,"draw")} />
          ...
          <div class="colorrow">
            <input class="namein tempin" type="number"
              ...
              max=${o(w,this.host._imperial)}
              step="0.1"
              .value=${t.cellCmInput??o(t.cellCm,this.host._imperial)}
              @input=${s=>{const e=s.target.value,o=D(e),a=null==o?null:d(o,this.host._imperial);this.host._spaceDialog={...t,cellCmInput:e,cellCmTouched:!0,cellCm:null!=a&&a>0?Math.max(.1,Math.min(w,a)):t.cellCm}}} />
          ...
          ${[["labelTemp","space.label_temp"],["labelHum","space.label_hum"],["labelLqi","space.label_lqi"],["labelLight","space.label_light"]].map(([s,e])=>p`<label class="srcrow">
          ...
          ${"static"===(t.bgMode??u(this.host._settings,{}))?p`
            <div class="colorrow">
              ...
              ${t.bgColor?p`<button class="btn ghost" @click=${()=>{this.host._spaceDialog={...t,bgColor:null}}}>${this.host._t("space.bg_inherit")}</button>`:p`<span class="opl">${this.host._t("space.bg_inherited")}</span>`}
            </div>`:_}
          ...
          ${m.map(t=>[t,`fill.${t}`]).map(([s,o])=>p`<label class="srcrow">
              <input type="radio" name="fillmode" .checked=${t.fillMode===s}
                @change=${()=>this.host._spaceDialog={...t,fillMode:s}} />
              <span>${this.host._t(o)}</span>
              ${"temp"===s&&"temp"===t.fillMode?p`<span class="temprange">
                <input class="namein tempin" type="number" step="0.5" .value=${String(t.tempMin)}
                  @input=${s=>{const e=D(s.target.value);null!=e&&(this.host._spaceDialog={...t,tempMin:e})}} />
                –
                <input class="namein tempin" type="number" step="0.5" .value=${String(t.tempMax)}
                  @input=${s=>{const e=D(s.target.value);null!=e&&(this.host._spaceDialog={...t,tempMax:e})}} /> °C
              </span>`:_}
            </label>
            ${"custom"===s&&"custom"===t.fillMode?p`
              <div class="colorrow gsrow">
        ...
        <div class="row dialog-action-footer" slot="footer">
          ...
          <div class="dialog-action-group dialog-action-commit">
            ${this.host._importTotal>0&&"create"===t.mode?p`<button class="btn ghost" @click=${()=>this._skipImport()}>
                  ${this.host._t("btn.skip")}</button>`:_}
    ...
    </hp-dialog>`}}export{C as HouseplanOnboardingRuntime,S as ONBOARDING_RUNTIME_FINGERPRINT};
