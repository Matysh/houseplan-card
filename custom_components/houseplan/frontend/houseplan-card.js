...
.dev .hval {
  ...
  `}let Ti=null,Ri=null,Ni=!1;const Ii=new Set;function Oi(){if(Ti)return Ti;try{const t=JSON.parse(localStorage.getItem("houseplan_card_cfg_v1")||"null");if(t&&t.config&&Array.isArray(t.config.spaces))return{config:t.config,rev:t.rev||0,layout:t.layout||{}}}catch{}return null}function Li(t){return Ti?Promise.resolve(Ti):Ri||(Ri=async function(t){const[e,i]=await Promise.all([t.callWS({type:"houseplan/config/get"}),t.callWS({type:"houseplan/layout/get"})]);if(Ti={config:e?.config??null,rev:e?.rev??0,layout:i?.layout??{}},!Ni&&t.connection?.subscribeEvents){Ni=!0;try{await t.connection.subscribeEvents(()=>{Ti=null,Ii.forEach(t=>t())},"houseplan_config_updated")}catch{Ni=!1}}return Ti}(t).finally(()=>{Ri=null}),Ri)}class qi extends lt{constructor(){super(...arguments),this._spaces=null,this._spacesLoading=!1}setConfig(t){this._config=t}async _loadSpaces(){if(!this._spaces&&!this._spacesLoading&&this.hass){this._spacesLoading=!0;try{const t=await this.hass.callWS({type:"houseplan/config/get"});this._spaces=(t?.config?.spaces||[]).map(t=>({value:t.id,label:t.title||t.id}))}catch{this._spaces=[]}finally{this._spacesLoading=!1}}}get _lang(){return ki(this.hass,this._config?.language)}get _schema(){const t=this._spaces||[];return[t.length?{name:"space",selector:{select:{mode:"dropdown",options:t}}}:{name:"space",selector:{text:{}}},{name:"title",selector:{text:{}}},{name:"show_button",selector:{boolean:{}}},{name:"button_label",selector:{text:{}}},{name:"button_target",selector:{text:{}}},{name:"aspect_ratio",selector:{text:{}}},{name:"icon_size",selector:{number:{min:1,max:6,step:.1,mode:"box"}}}]}render(){if(!this.hass||!this._config)return G;this._loadSpaces();const t=this._lang,e={space:Si(t,"editor.space"),title:Si(t,"editor.title"),show_button:Si(t,"editor.show_button"),button_label:Si(t,"editor.button_label"),button_target:Si(t,"editor.button_target"),aspect_ratio:Si(t,"editor.aspect_ratio"),icon_size:Si(t,"editor.icon_size")};return B`<ha-form
    ...
    ><span class="rlname">${t.name}${!this._markup&&t.area?B`<ha-icon class="rlgo" icon="mdi:open-in-new"
