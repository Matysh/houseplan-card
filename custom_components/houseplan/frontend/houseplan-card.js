      ...
      <ha-card>
        ...
        <div class="stage ${this._markup?"markup":""}"
          ...
          <svg viewBox="${n.x} ${n.y} ${n.w} ${n.h}" preserveAspectRatio="xMidYMid meet">
            ...
            ${e.rooms.filter(t=>t.area||this._markup||o.showBorders).map(t=>{let i="room "+(e.bg?"overlay":"yard")+(this._markup?" outlined":""),s="";if(!this._markup&&(o.showBorders||"none"!==o.fill)){i+=" styled";const e=[];o.showBorders?e.push(`--room-stroke:${o.color}`,`--room-stroke-op:${o.opacity}`):e.push("--room-stroke:transparent","--room-stroke-op:0");const l=t.area?(a=o.fill,r="lqi"===o.fill?this._roomLqi(t.area):null,n="light"===o.fill?function(t,e,i){let s=!1;for(const o of e)if(o.area===i)for(const e of o.entities)if(e.startsWith("light.")&&(s=!0,"on"===t.states[e]?.state))return"on";return s?"off":"none"}(this.hass,this._devices,t.area):"none","lqi"===a?null==r?null:ft(r):"light"===a?"none"===n?null:"on"===n?"#ffd45c":"#9aa0a6":null):null;l?e.push(`--room-fill:${l}`,`--room-fill-op:${(.3*o.opacity).toFixed(3)}`):e.push("--room-fill:transparent","--room-fill-op:0"),s=e.join(";")}var a,r,n;const l=e=>this._showTip(e,t.name,this._t("tip.room"),this._config?.show_signal?this._roomLqi(t.area):null),c=!e.bg&&!o.showNames||this._markup,h=this._roomCenter(t),d=t.poly?j`<polygon class="${i}" style="${s}" points="${t.poly.map(t=>t.join(",")).join(" ")}"
                    @click=${()=>this._clickRoom(t)} @mousemove=${l}
                    @mouseleave=${()=>this._tip=null}></polygon>`:j`<rect class="${i}" style="${s}"
                    x="${t.x}" y="${t.y}" width="${t.w}" height="${t.h}" rx="${.03*Math.min(t.w,t.h)}"
                    @click=${()=>this._clickRoom(t)} @mousemove=${l}
                    @mouseleave=${()=>this._tip=null}></rect>`;return j`${d}${c?j`<text class="rlabel" x="${h[0]}" y="${h[1]}">${t.name}</text>`:W}`})}
      ...
      <div class="dialog" @click=${t=>t.stopPropagation()}>
        ...
        <div class="body">
          ...
          <select class="areasel"
            ...
            ${[["none","fill.none"],["lqi","fill.lqi"],["light","fill.light"]].map(([e,i])=>F`<option value=${e} ?selected=${t.fillMode===e}>${this._t(i)}</option>`)}
          </select>
        </div>
        <div class="row">
