        ...
        <div class="stage ${this._markup?"markup tool-"+this._tool+("split"!==this._tool||this._splitSel?"":" pickstage")+("wallthick"===this._tool&&this._wallThickHover?" wallhot":""):""} ${"decor"===this._mode?"dtool-"+this._decorTool:""} ${s.bg?"":"noplan"} mode-${this._mode}${this._bdMovable?" bdgrab":""}${this._bdDrag?" bdgrabbing":""}${F?` daycycle phase-${F.phase}`:""}${this._booting?" hpboot":""}${this._bootSoft?" hpsettle":""}${this._modeTransitionBusy?" mode-transition":""}"
          ...
          <svg class=${m?.structural?"plan-svg":V}
            ...
            ${(()=>{const e=this._openCuts(),t=this._thickWallCuts(),i=this._wallUnionGeometry()?.roomGeom,n=new Map,r=e=>(n.has(e)||n.set(e,Nt(e)),n.get(e));return s.rooms.filter(e=>e.area||"view"===this._mode||this._markup||_.showBorders).map(n=>{let o="room "+(s.bg?"overlay":"yard")+(this._markup?" outlined":"");!this._markup||n.id!==this._mergeSel&&n.id!==this._splitSel?.roomId||(o+=" picked");let a="";const l=vn(_.fill,n);if(!this._markup&&(_.showBorders||"none"!==l)){o+=" styled";const e=[];e.push(`--room-stroke:${_.color}`,`--room-stroke-op:${_.showBorders?_.opacity:0}`);const t=g.byRoom.get(n)||null;t?(o+=" filled",e.push(`--room-fill:${t.color}`,`--room-fill-op:${t.opacity}`)):e.push("--room-fill:transparent","--room-fill-op:0"),a=e.join(";")}let c;const h=e=>{"view"===this._mode&&(void 0===c&&(c=this._roomArea(n)),this._showTip(e,n.name||this._t("room.unnamed"),c?this._t("tip.area",{value:c}):"",y?this._roomLqi(n.area):null,this._roomTemp(n),this._roomHum(n)))},d=r(n),u=this._markup&&(n.id===this._mergeSel||n.id===this._splitSel?.roomId),p=d&&!u?e.filter(e=>{const t=[(e[0]+e[2])/2,(e[1]+e[3])/2];return d.some((e,i)=>$n(t,[e[0],e[1],...d[(i+1)%d.length]])<=.08*this._gridPitch)}):[],m=u?[]:t,f=p.concat(m);f.length&&(o+=" noedge");const v=this._spaceWalls.length&&n.id&&d&&this._innerRoomContour(s,n.id,e,i)||d,b=v?ci(v,(w=n,s.rooms.filter(e=>e!==w).map(r).filter(Boolean))):[];var w;const k=e=>"M "+e.map(e=>e[0]+" "+e[1]).join(" L ")+" Z",x=v?this._cleanFloor(n,v,s).path:"",S=n.id||V,$=n.area||V,M=x&&v?j`<path class="${o}" style="${a}" fill-rule="evenodd"
                    ...
                    @pointerenter=${e=>{this._notePointer(e),this._pointerModality.hoverEnabled&&(this._hoverRoom={space:s.id,room:n})}}
                    @pointermove=${h}
                    @pointerleave=${()=>this._clearTransientHover()}></rect>`,T=f.length&&d?pn(d,f,.02*this._gridPitch):null,C=T?j`<path class="room-outline ${this._markup?"outlined":""}"
                    d="${T.map(e=>`M ${e[0]} ${e[1]} L ${e[2]} ${e[3]}`).join(" ")}"
                    style=${this._markup?V:`stroke:${_.color};stroke-opacity:${_.showBorders?_.opacity:0}`}></path>`:V;return j`${M}${C}`})})()}
