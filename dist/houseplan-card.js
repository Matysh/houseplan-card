...
.dev .hval {
  ...
  `}let Vi=null,Ki=null,Zi=!1;const Yi=new Set;function Ji(){if(Vi)return Vi;try{const t=JSON.parse(localStorage.getItem("houseplan_card_cfg_v1")||"null");if(t&&t.config&&Array.isArray(t.config.spaces))return{config:t.config,rev:t.rev||0,layout:t.layout||{}}}catch{}return null}function Xi(t){return Vi?Promise.resolve(Vi):Ki||(Ki=async function(t){const[e,i]=await Promise.all([t.callWS({type:"houseplan/config/get"}),t.callWS({type:"houseplan/layout/get"})]);if(Vi={config:e?.config??null,rev:e?.rev??0,layout:i?.layout??{}},!Zi&&t.connection?.subscribeEvents){Zi=!0;const e=()=>{Vi=null,Yi.forEach(t=>t())};try{await t.connection.subscribeEvents(e,"houseplan_config_updated"),await t.connection.subscribeEvents(e,"houseplan_layout_updated")}catch{Zi=!1}}return Vi}(t).finally(()=>{Ki=null}),Ki)}class Qi extends lt{constructor(){super(...arguments),this._spaces=null,this._spacesLoading=!1}setConfig(t){this._config=t}async _loadSpaces(){if(!this._spaces&&!this._spacesLoading&&this.hass){this._spacesLoading=!0;try{const t=await this.hass.callWS({type:"houseplan/config/get"});this._spaces=(t?.config?.spaces||[]).map(t=>({value:t.id,label:t.title||t.id}))}catch{this._spaces=[]}finally{this._spacesLoading=!1}}}get _lang(){return Li(this.hass,this._config?.language)}get _schema(){const t=this._spaces||[];return[t.length?{name:"space",selector:{select:{mode:"dropdown",options:t}}}:{name:"space",selector:{text:{}}},{name:"title",selector:{text:{}}},{name:"show_button",selector:{boolean:{}}},{name:"button_label",selector:{text:{}}},{name:"button_target",selector:{text:{}}},{name:"aspect_ratio",selector:{text:{}}},{name:"icon_size",selector:{number:{min:1,max:6,step:.1,mode:"box"}}}]}render(){if(!this.hass||!this._config)return G;this._loadSpaces();const t=this._lang,e={space:qi(t,"editor.space"),title:qi(t,"editor.title"),show_button:qi(t,"editor.show_button"),button_label:qi(t,"editor.button_label"),button_target:qi(t,"editor.button_target"),aspect_ratio:qi(t,"editor.aspect_ratio"),icon_size:qi(t,"editor.icon_size")};return B`<ha-form
      ...
      <ha-card>
        ...
        <div class="stage ${this._markup?"markup tool-"+this._tool+("split"!==this._tool||this._splitSel?"":" pickstage")+("openwall"===this._tool&&this._openWallHover?" wallhot":""):""} ${"decor"===this._mode?"dtool-"+this._decorTool:""} ${e.bg?"":"noplan"} mode-${this._mode}"
          ...
          <svg viewBox="${l.x} ${l.y} ${l.w} ${l.h}" preserveAspectRatio="xMidYMid meet">
            ...
            ${(()=>{const t=this._openPairs(),i=new Map,s=t=>(i.has(t)||i.set(t,we(t)),i.get(t));return e.rooms.filter(t=>t.area||this._markup||o.showBorders).map(i=>{let r="room "+(e.bg?"overlay":"yard")+(this._markup?" outlined":"");!this._markup||i.id!==this._mergeSel&&i.id!==this._splitSel?.roomId||(r+=" picked");let a="";const l=vi(o.fill,i);if(!this._markup&&(o.showBorders||"none"!==l)){r+=" styled";const t=[];t.push(`--room-stroke:${o.color}`,`--room-stroke-op:${o.showBorders?o.opacity:0}`);const e="glow"===l?this._fillColors.glow_base:"temp"===l?oi("temp",null,"none",this._roomTemp(i),o.tempMin,o.tempMax,this._fillColors):i.area?oi(l,"lqi"===l?this._roomLqi(i.area):null,"light"===l?Ai(this.hass,this._devices,i.area):"none",null,o.tempMin,o.tempMax,this._fillColors):null;e?(r+=" filled",t.push(`--room-fill:${e.c}`,`--room-fill-op:${e.a.toFixed(3)}`)):t.push("--room-fill:transparent","--room-fill-op:0"),a=t.join(";")}const c=t=>this._showTip(t,i.name,"",n?this._roomLqi(i.area):null,this._roomTemp(i)),h=!e.bg&&!o.showNames&&!this._markup,p=this._roomCenter(i),d=this._markup&&(i.id===this._mergeSel||i.id===this._splitSel?.roomId),u=i.id&&!d?t.filter(t=>t.a.id===i.id||t.b.id===i.id).flatMap(t=>t.segs):[];u.length&&(r+=" noedge");const _=s(i),g=_?function(t,e,i=1e-6){const s=e.filter(e=>Ne(t,e,i));return s.filter(t=>!s.some(e=>e!==t&&Ne(e,t,i)))}(_,(m=i,e.rooms.filter(t=>t!==m).map(s).filter(Boolean))):[];var m;const f=g.length&&_?j`<path class="${r}" style="${a}" fill-rule="evenodd"
                    ...
                    @mousemove=${c}
                    @mouseleave=${()=>this._tip=null}></rect>`,v=u.length&&_?function(t,e,i=1e-6){const s=[];for(let e=0;e<t.length;e++){const i=t[e],o=t[(e+1)%t.length];s.push([i[0],i[1],o[0],o[1]])}return ui(s,e,i)}(_,u,.02*this._gridPitch):null,b=v?j`<path class="room-outline ${this._markup?"outlined":""}"
                    d="${v.map(t=>`M ${t[0]} ${t[1]} L ${t[2]} ${t[3]}`).join(" ")}"
                    style=${this._markup?G:`stroke:${o.color};stroke-opacity:${o.showBorders?o.opacity:0}`}></path>`:G;return j`${f}${b}${h?j`<text class="rlabel" x="${p[0]}" y="${p[1]}">${i.name}</text>`:G}`})})()}
        ...
        ${this._tip?B`<div class="tip" style="left:${this._tip.x+12}px;top:${this._tip.y+12}px">
              ...
              ${null!=this._tip.lqi?B`<span class="m">${this._t("tip.lqi")}
                    <b style="color:${ve(this._tip.lqi)}">${this._tip.lqi}</b></span>`:G}
            </div>`:G}
    ...
    `}_renderDevice(t,e,i=!0){const s=this._pos(t),o=(s.x-e.x)/e.w*100,n=(s.y-e.y)/e.h*100,r=this._stateClass(t),a=this._liveTemp(t),l=this._liveHum(t),c=i&&!t.virtual?Mi(this.hass,t.entities):null,h=t.marker,p=h?.display||"badge",d="ripple"===p||"icon_ripple"===p,u=t.primary?this.hass.states[t.primary]:void 0,_="value"===p?null!=a?a+"°":null!=l?l+"%":u&&!isNaN(parseFloat(u.state))?parseFloat(u.state)+(u.attributes?.unit_of_measurement?" "+u.attributes.unit_of_measurement:""):null:null,g=t.primary?t.primary.split(".")[0]:null,m=this._config?.live_states?ni(t.icon,g,u?.attributes?.device_class,u?.state,!!h?.icon):t.icon,f=(h?.controls||[]).filter(hi),v=this._config?.live_states?f.length?f.map(t=>ri(this.hass.states[t])).find(t=>t)||null:"light"===g?ri(u):null:null,b=this._config?.live_states&&function(t,e,i){return"on"===i&&("siren"===t||"binary_sensor"===t&&!!e&&wi.has(e))}(g,u?.attributes?.device_class,u?.state),y=d&&!!t.primary&&ke(this.hass.states[t.primary]?.state),w=Number(h?.size)>0?Number(h.size):1,x=Number(h?.angle)||0,$=Number(h?.ripple_size)>0?Number(h.ripple_size):3,k=[`left:${o}%`,`top:${n}%`];return 1!==w&&k.push(`--dev-scale:${w}`),d&&(k.push(`--ripple-scale:${$}`),h?.ripple_color?k.push(`--ripple-color:${h.ripple_color}`):v&&k.push(`--ripple-color:${v}`)),v&&k.push(`--light-color:${v}`),B`<div
      class="dev ${r} ${this._selId===t.id?"sel":""} ${t.virtual?"virtual":""} ${"ripple"===p?"noicon":""} ${null!=_?"valonly":""} ${v?"rgb":""} ${b?"alarm":""}"
      style="${k.join(";")}"
    ...
    >
      ...
      ${null!=_?B`<span class="valtext">${_}</span>`:"ripple"!==p?B`<ha-icon icon="${m}" style=${x?`transform:rotate(${x}deg)`:G}></ha-icon>`:G}
      ${null!=a&&null==_?B`<span class="tval">${a}°</span>`:G}
      ${null!=l&&null==_?B`<span class="hval">${l}%</span>`:G}
      ${null!=c?B`<span class="lqi" style="color:${ve(c)}">${c}</span>`:G}
        ...
        <div class="body">
          ${(()=>{const e=this._cardEntities(t);return e.length?B`<div class="entlist">
              ${e.map(({eid:t,kind:e})=>{const i=this.hass.states[t],s=this.hass.entities[t]?.name||i?.attributes?.friendly_name||t,o=i?this.hass.formatEntityState?.(i)??i.state:"",n="on"===i?.state||["open","unlocked","playing","cleaning"].includes(i?.state);return B`<div class="entrow ${n?"on":""}">
                  <ha-icon icon=${ni(mt(s,"",this._iconRules),t.split(".")[0],i?.attributes?.device_class,i?.state,!1)}></ha-icon>
