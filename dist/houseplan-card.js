...
.dev .hval {
  ...
  `}let no=null,ro=null,ao=!1;const lo=new Set;function co(){if(no)return no;try{const t=JSON.parse(localStorage.getItem("houseplan_card_cfg_v1")||"null");if(t&&t.config&&Array.isArray(t.config.spaces))return{config:t.config,rev:t.rev||0,layout:t.layout||{}}}catch{}return null}function ho(t){return no?Promise.resolve(no):ro||(ro=async function(t){const[e,i]=await Promise.all([t.callWS({type:"houseplan/config/get"}),t.callWS({type:"houseplan/layout/get"})]);if(no={config:e?.config??null,rev:e?.rev??0,layout:i?.layout??{}},!ao&&t.connection?.subscribeEvents){ao=!0;const e=()=>{no=null,lo.forEach(t=>t())};try{await t.connection.subscribeEvents(e,"houseplan_config_updated"),await t.connection.subscribeEvents(e,"houseplan_layout_updated")}catch{ao=!1}}return no}(t).finally(()=>{ro=null}),ro)}class po extends lt{constructor(){super(...arguments),this._spaces=null,this._spacesLoading=!1}setConfig(t){this._config=t}async _loadSpaces(){if(!this._spaces&&!this._spacesLoading&&this.hass){this._spacesLoading=!0;try{const t=await this.hass.callWS({type:"houseplan/config/get"});this._spaces=(t?.config?.spaces||[]).map(t=>({value:t.id,label:t.title||t.id}))}catch{this._spaces=[]}finally{this._spacesLoading=!1}}}get _lang(){return Ks(this.hass,this._config?.language)}get _schema(){const t=this._spaces||[];return[t.length?{name:"space",selector:{select:{mode:"dropdown",options:t}}}:{name:"space",selector:{text:{}}},{name:"title",selector:{text:{}}},{name:"show_button",selector:{boolean:{}}},{name:"button_label",selector:{text:{}}},{name:"button_target",selector:{text:{}}},{name:"aspect_ratio",selector:{text:{}}},{name:"icon_size",selector:{number:{min:1,max:6,step:.1,mode:"box"}}}]}render(){if(!this.hass||!this._config)return V;this._loadSpaces();const t=this._lang,e={space:Zs(t,"editor.space"),title:Zs(t,"editor.title"),show_button:Zs(t,"editor.show_button"),button_label:Zs(t,"editor.button_label"),button_target:Zs(t,"editor.button_target"),aspect_ratio:Zs(t,"editor.aspect_ratio"),icon_size:Zs(t,"editor.icon_size")};return B`<ha-form
    ...
    `}}_o.properties={hass:{attribute:!1},_config:{state:!0},_snap:{state:!0}},_o.styles=[Js,n`
      .hp-static-title {
        font-weight: 700;
    ...
    </div>`}_saveKioskScale(t){this._kioskScale={...this._kioskScale,...t};try{localStorage.setItem(bo,JSON.stringify(this._kioskScale))}catch{}this.requestUpdate()}_renderKioskDialog(){const t=this._kioskScale,e=(e,i)=>B`<label>${i}</label>
      <div class="colorrow">
