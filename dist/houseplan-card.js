...
.dev .hval {
  ...
  `}let wi=null,xi=null,$i=!1;const ki=new Set;function Si(){if(wi)return wi;try{const t=JSON.parse(localStorage.getItem("houseplan_card_cfg_v1")||"null");if(t&&t.config&&Array.isArray(t.config.spaces))return{config:t.config,rev:t.rev||0,layout:t.layout||{}}}catch{}return null}function Ci(t){return wi?Promise.resolve(wi):xi||(xi=async function(t){const[e,i]=await Promise.all([t.callWS({type:"houseplan/config/get"}),t.callWS({type:"houseplan/layout/get"})]);if(wi={config:e?.config??null,rev:e?.rev??0,layout:i?.layout??{}},!$i&&t.connection?.subscribeEvents){$i=!0;try{await t.connection.subscribeEvents(()=>{wi=null,ki.forEach(t=>t())},"houseplan_config_updated")}catch{$i=!1}}return wi}(t).finally(()=>{xi=null}),xi)}class Ei extends lt{constructor(){super(...arguments),this._spaces=null,this._spacesLoading=!1}setConfig(t){this._config=t}async _loadSpaces(){if(!this._spaces&&!this._spacesLoading&&this.hass){this._spacesLoading=!0;try{const t=await this.hass.callWS({type:"houseplan/config/get"});this._spaces=(t?.config?.spaces||[]).map(t=>({value:t.id,label:t.title||t.id}))}catch{this._spaces=[]}finally{this._spacesLoading=!1}}}get _lang(){return di(this.hass,this._config?.language)}get _schema(){const t=this._spaces||[];return[t.length?{name:"space",selector:{select:{mode:"dropdown",options:t}}}:{name:"space",selector:{text:{}}},{name:"title",selector:{text:{}}},{name:"show_button",selector:{boolean:{}}},{name:"button_label",selector:{text:{}}},{name:"button_target",selector:{text:{}}},{name:"aspect_ratio",selector:{text:{}}},{name:"icon_size",selector:{number:{min:1,max:6,step:.1,mode:"box"}}}]}render(){if(!this.hass||!this._config)return W;this._loadSpaces();const t=this._lang,e={space:ui(t,"editor.space"),title:ui(t,"editor.title"),show_button:ui(t,"editor.show_button"),button_label:ui(t,"editor.button_label"),button_target:ui(t,"editor.button_target"),aspect_ratio:ui(t,"editor.aspect_ratio"),icon_size:ui(t,"editor.icon_size")};return H`<ha-form
      .hass=${this.hass}
      .data=${this._config}
      .schema=${this._schema}
      .computeLabel=${t=>e[t.name]||t.name}
      @value-changed=${this._valueChanged}
    ></ha-form>`}_valueChanged(t){const e={...this._config,...t.detail.value},i=new Event("config-changed",{bubbles:!0,composed:!0});i.detail={config:e},this.dispatchEvent(i)}}Ei.properties={hass:{attribute:!1},_config:{state:!0},_spaces:{state:!0}},customElements.get("houseplan-space-card-editor")||customElements.define("houseplan-space-card-editor",Ei);const Di=t=>{history.pushState(null,"",t),((t,e,i)=>{const s=new Event(e,{bubbles:!0,composed:!0});s.detail=i??{},t.dispatchEvent(s)})(window,"location-changed",{replace:!1})};class Mi extends lt{constructor(){super(...arguments),this._snap=null,this._loading=!1,this._loadedOnce=!1,this._goToSpace=()=>{const t=(this._config?.button_target||"/plan-doma").replace(/#.*$/,"");Di(`${t}#space=${encodeURIComponent(this._config.space)}`)}}static getConfigElement(){return document.createElement("houseplan-space-card-editor")}static getStubConfig(t){const e=Si();return{type:"custom:houseplan-space-card",space:fi(e?.config||null)[0]?.id||"",show_button:!0}}setConfig(t){if(!t||!t.space)throw new Error('houseplan-space-card: "space" is required');this._config={show_button:!0,button_target:"/plan-doma",...t},this._snap=this._snap||Si()}connectedCallback(){var t;super.connectedCallback(),this._unsub=(t=()=>{this._loading=!1,this._snap=null,this.requestUpdate()},ki.add(t),()=>ki.delete(t))}disconnectedCallback(){this._unsub?.(),this._unsub=void 0,super.disconnectedCallback()}willUpdate(t){!this.hass||this._loading||this._snap&&!t.has("hass")||this._snap&&this._loadedOnce||this._load()}async _load(){if(this.hass&&!this._loading){this._loading=!0;try{const t=await Ci(this.hass);this._snap=t,this._loadedOnce=!0}catch{}finally{this._loading=!1,this.requestUpdate()}}}get _lang(){return di(this.hass,this._config?.language)}getCardSize(){const t=fi(this._snap?.config||null).find(t=>t.id===this._config?.space);if(t){const e=t.vb[3]/t.vb[2];return Math.max(3,Math.round(8*e))+(!1===this._config?.show_button?0:1)}return 6}_errorCard(t){return H`<ha-card><div class="hp-static-error">${t}</div></ha-card>`}render(){if(!this._config)return W;const t=this._snap?.config;if(!t)return H`<ha-card><div class="hp-static-error">${ui(this._lang,"space_card.loading")}</div></ha-card>`;const e=this._config.space,i=yi({hass:this.hass,cfg:t,layout:this._snap?.layout||{},spaceId:e,iconSize:this._config.icon_size,lang:this._lang});if(!i)return this._errorCard(ui(this._lang,"space_card.not_found",{id:e}));const s=fi(t).find(t=>t.id===e),o=void 0!==this._config.title?this._config.title:s?.title||"",n=!1!==this._config.show_button,r=this._config.button_label||ui(this._lang,"space_card.button");return H`
      <ha-card>
    ...
    `}}Mi.properties={hass:{attribute:!1},_config:{state:!0},_snap:{state:!0}},Mi.styles=[_i,n`
      .hp-static-title {
        font-weight: 700;
      ...
      <ha-card>
        ...
        <div class="head">
          ...
          <div class="tabs">
            ${t.map(t=>H`<button
              ...
              >
                ${t.title}${this._norm?H`<ha-icon class="tabedit" icon="mdi:cog-outline"
                      title=${this._t("title.configure_space")}
                      @click=${e=>{e.stopPropagation(),this._openSpaceDialog("edit",t.id)}}></ha-icon>`:W}
              </button>`)}
            ${this._norm?H`<button class="tab tabadd" title=${this._t("title.add_space")}
          ...
          ${this._norm?H`<button class="btn" @click=${()=>this._openMarkerDialog()}
                title=${this._t("title.add_device")}>
                <ha-icon icon="mdi:plus-box-outline"></ha-icon>
              </button>`:W}
          ${this._norm?H`<button class="btn ${this._showAll?"on":""}" @click=${this._toggleShowAll}
        ...
        <div class="stage ${this._markup?"markup":""} ${e.bg?"":"noplan"}"
          ...
          <svg viewBox="${l.x} ${l.y} ${l.w} ${l.h}" preserveAspectRatio="xMidYMid meet">
            ...
            ${e.rooms.filter(t=>t.area||this._markup||o.showBorders).map(t=>{let i="room "+(e.bg?"overlay":"yard")+(this._markup?" outlined":"");!this._markup||t.id!==this._mergeSel&&t.id!==this._splitSel?.roomId||(i+=" picked");let s="";if(!this._markup&&(o.showBorders||"none"!==o.fill)){i+=" styled";const e=[];e.push(`--room-stroke:${o.color}`,`--room-stroke-op:${o.showBorders?o.opacity:0}`);const n=t.area?Je(o.fill,"lqi"===o.fill?this._roomLqi(t.area):null,"light"===o.fill?li(this.hass,this._devices,t.area):"none","temp"===o.fill?ci(this.hass,this._devices,t.area):null,o.tempMin,o.tempMax,this._fillColors):null;n?(i+=" filled",e.push(`--room-fill:${n.c}`,`--room-fill-op:${n.a.toFixed(3)}`)):e.push("--room-fill:transparent","--room-fill-op:0"),s=e.join(";")}const r=e=>this._showTip(e,t.name,this._t("tip.room"),n?this._roomLqi(t.area):null,t.area?ci(this.hass,this._devices,t.area):null),a=!e.bg&&!o.showNames||this._markup,l=this._roomCenter(t),c=t.poly?j`<polygon class="${i}" style="${s}" points="${t.poly.map(t=>t.join(",")).join(" ")}"
    ...
    `}_renderDevice(t,e,i=!0){const s=this._pos(t),o=(s.x-e.x)/e.w*100,n=(s.y-e.y)/e.h*100,r=this._stateClass(t),a=this._liveTemp(t),l=this._liveHum(t),c=i&&!t.virtual?ei(this.hass,t.entities):null,h=t.marker,p=h?.display||"badge",d="ripple"===p||"icon_ripple"===p,u=d&&!!t.primary&&ke(this.hass.states[t.primary]?.state),g=Number(h?.size)>0?Number(h.size):1,_=Number(h?.angle)||0,m=Number(h?.ripple_size)>0?Number(h.ripple_size):3,f=[`left:${o}%`,`top:${n}%`];return 1!==g&&f.push(`--dev-scale:${g}`),d&&(f.push(`--ripple-scale:${m}`),h?.ripple_color&&f.push(`--ripple-color:${h.ripple_color}`)),H`<div
      class="dev ${r} ${this._selId===t.id?"sel":""} ${t.virtual?"virtual":""} ${"ripple"===p?"noicon":""}"
      style="${f.join(";")}"
    ...
    >
      ${d?H`<span class="ripple ${u?"active":""}"><i></i><i></i><i></i></span>`:W}
      ${"ripple"!==p?H`<ha-icon icon="${t.icon}" style=${_?`transform:rotate(${_}deg)`:W}></ha-icon>`:W}
      ${null!=a?H`<span class="tval">${a}°</span>`:W}
      ${null!=l?H`<span class="hval">${l}%</span>`:W}
      ...
      <div class="dialog wide" @click=${t=>t.stopPropagation()}>
        ...
        <div class="body">
          ...
          <select class="areasel"
            ...
            ${[["badge","display.badge"],["ripple","display.ripple"],["icon_ripple","display.icon_ripple"]].map(([e,i])=>H`<option value=${e} ?selected=${t.display===e}>${this._t(i)}</option>`)}
          </select>
          ${"badge"!==t.display?H`<div class="colorrow">
    ...
    </div>`}}Ni.properties={hass:{attribute:!1},_config:{state:!0},_space:{state:!0},_layout:{state:!0},_devices:{state:!0},_tip:{state:!0},_selId:{state:!0},_toast:{state:!0},_serverCfg:{state:!0},_markup:{state:!0},_tool:{state:!0},_path:{state:!0},_cursorPt:{state:!0},_mergeSel:{state:!0},_openingDialog:{state:!0},_openingInfo:{state:!0},_mergeDialog:{state:!0},_splitSel:{state:!0},_areaSel:{state:!0},_nameSel:{state:!0},_roomDialog:{state:!0},_spaceDialog:{state:!0},_infoCard:{state:!0},_rulesDialog:{state:!0},_settingsDialog:{state:!0},_importDialog:{state:!0},_markerDialog:{state:!0},_zoom:{state:!0},_view:{state:!0}},Ni.styles=_i,customElements.get("houseplan-card")||customElements.define("houseplan-card",Ni),window.customCards=window.customCards||[],window.customCards.find(t=>"houseplan-card"===t.type)||window.customCards.push({type:"houseplan-card",name:"House Plan Card",description:"Interactive house plan: spaces, rooms and devices with live states and drag layout."}),console.info("%c HOUSEPLAN-CARD %c v1.24.1 ","background:#3ea6ff;color:#04121f;font-weight:700","");
