...
.dev .hval {
  ...
  `,Zt=1e3;function Yt(t){return t&&Array.isArray(t.spaces)?t.spaces.map(t=>{const e=Zt/t.aspect;return{id:t.id,title:t.title,vb:[t.view_box[0]*Zt,t.view_box[1]*e,t.view_box[2]*Zt,t.view_box[3]*e],bg:t.plan_url?{href:t.plan_url,x:0,y:0,w:Zt,h:e}:null,rooms:(t.rooms||[]).map(t=>({id:t.id,name:t.name,area:t.area??null,x:null!=t.x?t.x*Zt:void 0,y:null!=t.y?t.y*e:void 0,w:null!=t.w?t.w*Zt:void 0,h:null!=t.h?t.h*e:void 0,poly:t.poly?t.poly.map(t=>[t[0]*Zt,t[1]*e]):void 0}))}}):[]}function Jt(t){if(t.poly&&t.poly.length){const e=t.poly.map(t=>t[0]),i=t.poly.map(t=>t[1]),s=Math.min(...e),a=Math.min(...i);return{x:s,y:a,w:Math.max(...e)-s,h:Math.max(...i)-a}}return{x:t.x??0,y:t.y??0,w:t.w??0,h:t.h??0}}function Xt(t){if(t.poly){const e=t.poly.length;return[t.poly.reduce((t,e)=>t+e[0],0)/e,t.poly.reduce((t,e)=>t+e[1],0)/e]}return[t.x+t.w/2,t.y+.1*Math.min(t.w,t.h)]}function Gt(t){const e=Yt(t.cfg),i=e.find(e=>e.id===t.spaceId);if(!i)return null;const s=i.vb,a=Mt(t.cfg.spaces.find(e=>e.id===t.spaceId)),o=t.iconSize??2.5,r=o>8?2.5:o,n={};for(const e of t.cfg.spaces||[])for(const t of e.rooms||[])t.area&&(n[t.area]=e.id);const l=t.cfg.settings?.exclude_integrations?new Set(t.cfg.settings.exclude_integrations):ht,c=pt(t.cfg.settings?.icon_rules?.length?t.cfg.settings.icon_rules:dt),h=Ut({hass:t.hass,areaToSpace:n,markers:t.cfg.markers||[],settings:t.cfg.settings||{},excluded:l,showAll:!!t.cfg.settings?.show_all,firstSpaceId:e[0]?.id||"",loc:e=>Wt(t.lang,e),iconRules:c}),d=h.filter(e=>e.space===t.spaceId),p=function(t,e,i){const s={},a=i/100*Zt*1.3;for(const i of e.rooms){if(!i.area)continue;const e=t.filter(t=>t.area===i.area);if(!e.length)continue;const o=Jt(i),r=.1*Math.min(o.w,o.h),n=o.w-2*r,l=o.h-2*r,c=Math.max(1,Math.round(Math.sqrt(e.length*n/Math.max(l,1)))),h=n/c,d=l/Math.max(Math.ceil(e.length/c),1),p=e.map((t,e)=>({x:o.x+r+h*(e%c+.5),y:o.y+r+d*(Math.floor(e/c)+.5)}));$t(p,o,a,.5*r),e.forEach((t,e)=>s[t.id]=p[e])}return s}(d,i,r),_=i.rooms.filter(t=>t.area||a.showBorders).map(e=>{let s="room "+(i.bg?"overlay":"yard"),o="";if(a.showBorders||"none"!==a.fill){s+=" styled";const i=[`--room-stroke:${a.color}`,`--room-stroke-op:${a.showBorders?a.opacity:0}`],r=e.area?Dt(a.fill,"lqi"===a.fill?function(t,e,i){const s=[];for(const a of e){if(a.area!==i||a.virtual)continue;const e=Tt(t,a.entities);null!=e&&s.push(e)}return yt(s)}(t.hass,d,e.area):null,"light"===a.fill?Ht(t.hass,d,e.area):"none","temp"===a.fill?Lt(t.hass,d,e.area):null,a.tempMin,a.tempMax):null;r?(s+=" filled",i.push(`--room-fill:${r}`,`--room-fill-op:${(.3*a.opacity).toFixed(3)}`)):i.push("--room-fill:transparent","--room-fill-op:0"),o=i.join(";")}const r=!i.bg&&!a.showNames,n=Xt(e),l=e.poly?B`<polygon class="${s}" style="${o}" points="${e.poly.map(t=>t.join(",")).join(" ")}"></polygon>`:B`<rect class="${s}" style="${o}" x="${e.x}" y="${e.y}" width="${e.w}" height="${e.h}" rx="${.03*Math.min(e.w,e.h)}"></rect>`;return B`${l}${r?B`<text class="rlabel" x="${n[0]}" y="${n[1]}">${e.name}</text>`:K}`}),u=d.map(e=>{const a=function(t,e,i,s,a){const o=e[t.id];if(o&&o.s===t.space){const e=i.spaces.find(e=>e.id===t.space)?.aspect||1;return{x:o.x*Zt,y:o.y*(Zt/e)}}if(s[t.id])return s[t.id];const r=a.vb;return{x:r[0]+r[2]/2,y:r[1]+r[3]/2}}(e,t.layout,t.cfg,p,i),o=(a.x-s[0])/s[2]*100,r=(a.y-s[1])/s[3]*100;return j`<div class="dev ${e.virtual?"virtual":""}" style="left:${o}%;top:${r}%">
      <ha-icon icon="${e.icon}"></ha-icon>
    </div>`}),m=a.showNames?i.rooms.filter(t=>t.name).map(e=>{const o=function(t,e,i,s){const a=i["rl_"+(t.id||"")];if(a&&a.s===e){const t=s.spaces.find(t=>t.id===e)?.aspect||1;return{x:a.x*Zt,y:a.y*(Zt/t)}}const o=Xt(t);return{x:o[0],y:o[1]}}(e,i.id,t.layout,t.cfg),r=(o.x-s[0])/s[2]*100,n=(o.y-s[1])/s[3]*100,l=Math.min(1,a.opacity+.25);return j`<div class="roomlabel" style="left:${r}%;top:${n}%;color:${a.color};opacity:${l}">${e.name}</div>`}):[];return j`
    <div class="hp-static-stage" style="aspect-ratio:${s[2]}/${s[3]}">
      <svg viewBox="${s[0]} ${s[1]} ${s[2]} ${s[3]}" preserveAspectRatio="xMidYMid meet">
        ${i.bg?B`<image href="${i.bg.href}" x="${i.bg.x}" y="${i.bg.y}" width="${i.bg.w}" height="${i.bg.h}" preserveAspectRatio="none" />`:K}
        ${_}
      </svg>
      <div class="devlayer" style="--icon-size:${r}cqw">${u}${m}</div>
    </div>
  `}let Qt=null,te=null,ee=!1;const ie=new Set;function se(){if(Qt)return Qt;try{const t=JSON.parse(localStorage.getItem("houseplan_card_cfg_v1")||"null");if(t&&t.config&&Array.isArray(t.config.spaces))return{config:t.config,rev:t.rev||0,layout:t.layout||{}}}catch{}return null}function ae(t){return Qt?Promise.resolve(Qt):te||(te=async function(t){const[e,i]=await Promise.all([t.callWS({type:"houseplan/config/get"}),t.callWS({type:"houseplan/layout/get"})]);if(Qt={config:e?.config??null,rev:e?.rev??0,layout:i?.layout??{}},!ee&&t.connection?.subscribeEvents){ee=!0;try{await t.connection.subscribeEvents(()=>{Qt=null,ie.forEach(t=>t())},"houseplan_config_updated")}catch{ee=!1}}return Qt}(t).finally(()=>{te=null}),te)}class oe extends lt{constructor(){super(...arguments),this._spaces=null,this._spacesLoading=!1}setConfig(t){this._config=t}async _loadSpaces(){if(!this._spaces&&!this._spacesLoading&&this.hass){this._spacesLoading=!0;try{const t=await this.hass.callWS({type:"houseplan/config/get"});this._spaces=(t?.config?.spaces||[]).map(t=>({value:t.id,label:t.title||t.id}))}catch{this._spaces=[]}finally{this._spacesLoading=!1}}}get _lang(){return Bt(this.hass,this._config?.language)}get _schema(){const t=this._spaces||[];return[t.length?{name:"space",selector:{select:{mode:"dropdown",options:t}}}:{name:"space",selector:{text:{}}},{name:"title",selector:{text:{}}},{name:"show_button",selector:{boolean:{}}},{name:"button_label",selector:{text:{}}},{name:"button_target",selector:{text:{}}},{name:"aspect_ratio",selector:{text:{}}},{name:"icon_size",selector:{number:{min:1,max:6,step:.1,mode:"box"}}}]}render(){if(!this.hass||!this._config)return K;this._loadSpaces();const t=this._lang,e={space:Wt(t,"editor.space"),title:Wt(t,"editor.title"),show_button:Wt(t,"editor.show_button"),button_label:Wt(t,"editor.button_label"),button_target:Wt(t,"editor.button_target"),aspect_ratio:Wt(t,"editor.aspect_ratio"),icon_size:Wt(t,"editor.icon_size")};return j`<ha-form
      .hass=${this.hass}
      .data=${this._config}
      .schema=${this._schema}
      .computeLabel=${t=>e[t.name]||t.name}
      @value-changed=${this._valueChanged}
    ></ha-form>`}_valueChanged(t){const e={...this._config,...t.detail.value},i=new Event("config-changed",{bubbles:!0,composed:!0});i.detail={config:e},this.dispatchEvent(i)}}oe.properties={hass:{attribute:!1},_config:{state:!0},_spaces:{state:!0}},customElements.get("houseplan-space-card-editor")||customElements.define("houseplan-space-card-editor",oe);const re=t=>{history.pushState(null,"",t),((t,e,i)=>{const s=new Event(e,{bubbles:!0,composed:!0});s.detail=i??{},t.dispatchEvent(s)})(window,"location-changed",{replace:!1})};class ne extends lt{constructor(){super(...arguments),this._snap=null,this._loading=!1,this._loadedOnce=!1,this._goToSpace=()=>{const t=(this._config?.button_target||"/plan-doma").replace(/#.*$/,"");re(`${t}#space=${encodeURIComponent(this._config.space)}`)}}static getConfigElement(){return document.createElement("houseplan-space-card-editor")}static getStubConfig(t){const e=se();return{type:"custom:houseplan-space-card",space:Yt(e?.config||null)[0]?.id||"",show_button:!0}}setConfig(t){if(!t||!t.space)throw new Error('houseplan-space-card: "space" is required');this._config={show_button:!0,button_target:"/plan-doma",...t},this._snap=this._snap||se()}connectedCallback(){var t;super.connectedCallback(),this._unsub=(t=()=>{this._loading=!1,this._snap=null,this.requestUpdate()},ie.add(t),()=>ie.delete(t))}disconnectedCallback(){this._unsub?.(),this._unsub=void 0,super.disconnectedCallback()}willUpdate(t){!this.hass||this._loading||this._snap&&!t.has("hass")||this._snap&&this._loadedOnce||this._load()}async _load(){if(this.hass&&!this._loading){this._loading=!0;try{const t=await ae(this.hass);this._snap=t,this._loadedOnce=!0}catch{}finally{this._loading=!1,this.requestUpdate()}}}get _lang(){return Bt(this.hass,this._config?.language)}getCardSize(){const t=Yt(this._snap?.config||null).find(t=>t.id===this._config?.space);if(t){const e=t.vb[3]/t.vb[2];return Math.max(3,Math.round(8*e))+(!1===this._config?.show_button?0:1)}return 6}_errorCard(t){return j`<ha-card><div class="hp-static-error">${t}</div></ha-card>`}render(){if(!this._config)return K;const t=this._snap?.config;if(!t)return j`<ha-card><div class="hp-static-error">${Wt(this._lang,"space_card.loading")}</div></ha-card>`;const e=this._config.space,i=Gt({hass:this.hass,cfg:t,layout:this._snap?.layout||{},spaceId:e,iconSize:this._config.icon_size,lang:this._lang});if(!i)return this._errorCard(Wt(this._lang,"space_card.not_found",{id:e}));const s=Yt(t).find(t=>t.id===e),a=void 0!==this._config.title?this._config.title:s?.title||"",o=!1!==this._config.show_button,r=this._config.button_label||Wt(this._lang,"space_card.button");return j`
      <ha-card>
        ${a?j`<div class="hp-static-title">${a}</div>`:K}
        ${i}
        ${o?j`<div class="hp-static-foot">
              <button class="hp-static-btn" @click=${this._goToSpace}>${r}</button>
            </div>`:K}
      </ha-card>
    `}}ne.properties={hass:{attribute:!1},_config:{state:!0},_snap:{state:!0}},ne.styles=[Vt,o`
      .hp-static-title {
        font-weight: 700;
        ...
        <div class="body">
          ...
          ${t.floors.map((e,i)=>j`<label class="floorrow">
              <input type="checkbox" .checked=${e.checked}
                @change=${s=>{const a=[...t.floors];a[i]={...e,checked:s.target.checked},this._importDialog={floors:a}}} />
              <span>${e.name}</span>
              ${null!=e.level?j`<span class="floorlvl">L${e.level}</span>`:K}
            </label>`)}
        </div>
        <div class="row">
      ...
      <div class="dialog wide" @click=${t=>t.stopPropagation()}>
        ...
        <div class="body">
          ...
          <div class="rtest">
            ...
            ${i?j`<ha-icon icon=${i}></ha-icon><span class="rtesticon">${i}</span>`:K}
          </div>
          ${t.rules.map((e,i)=>{const a=""!==e.pattern.trim()&&!function(t){try{return new RegExp(t,"i"),!0}catch{return!1}}(e.pattern);return j`<div class="rrow">
              <input class="namein rpat ${a?"bad":""}" type="text"
                placeholder=${this._t("rules.pattern_ph")}
                title=${a?this._t("rules.invalid"):""}
                .value=${e.pattern}
                @input=${s=>{const a=[...t.rules];a[i]={...e,pattern:s.target.value},this._rulesSet(a)}} />
              <input class="namein ricon" type="text" placeholder=${this._t("rules.icon_ph")}
                .value=${e.icon}
                @input=${s=>{const a=[...t.rules];a[i]={...e,icon:s.target.value},this._rulesSet(a)}} />
    ...
    </div>`}render(){if(!this._config||!this.hass)return K;const t=this._model;if(!t.length)return j`<ha-card>
        <div class="head">
        ...
        ${this._spaceDialog?this._renderSpaceDialog():K}
        ${this._importDialog?this._renderImportDialog():K}
        ${this._toast?j`<div class="toast">${this._toast}</div>`:K}
      </ha-card>`;const e=this._spaceModel(),i=e.vb,s=this._devices.filter(t=>t.space===e.id),a=Mt(this._curSpaceCfg),o=this._config.icon_size??2.5,r=o>8?2.5:o,n=this._viewOr(i);return j`
      <ha-card>
        <div class="hdr">
        <div class="head">
          <div class="title">
            ...
            ${t.map(t=>j`<button
              ...
              >
                ${t.title}${this._norm?j`<ha-icon class="tabedit" icon="mdi:pencil"
                      ...
                      @click=${e=>{e.stopPropagation(),this._openSpaceDialog("edit",t.id)}}></ha-icon>`:K}
              </button>`)}
        ...
        ${this._markup?this._renderMarkupBar():K}
        </div>
        ...
        <div class="stage ${this._markup?"markup":""}"
          ...
          <svg viewBox="${n.x} ${n.y} ${n.w} ${n.h}" preserveAspectRatio="xMidYMid meet">
            ${this._markup?this._renderMarkupDefs(i):K}
            ${e.bg?B`<image href="${e.bg.href}" x="${e.bg.x}" y="${e.bg.y}" width="${e.bg.w}" height="${e.bg.h}" preserveAspectRatio="none" />`:K}
            ${e.rooms.filter(t=>t.area||this._markup||a.showBorders).map(t=>{let i="room "+(e.bg?"overlay":"yard")+(this._markup?" outlined":""),s="";if(!this._markup&&(a.showBorders||"none"!==a.fill)){i+=" styled";const e=[];e.push(`--room-stroke:${a.color}`,`--room-stroke-op:${a.showBorders?a.opacity:0}`);const o=t.area?Dt(a.fill,"lqi"===a.fill?this._roomLqi(t.area):null,"light"===a.fill?Ht(this.hass,this._devices,t.area):"none","temp"===a.fill?Lt(this.hass,this._devices,t.area):null,a.tempMin,a.tempMax):null;o?(i+=" filled",e.push(`--room-fill:${o}`,`--room-fill-op:${(.3*a.opacity).toFixed(3)}`)):e.push("--room-fill:transparent","--room-fill-op:0"),s=e.join(";")}const o=e=>this._showTip(e,t.name,this._t("tip.room"),this._config?.show_signal?this._roomLqi(t.area):null,t.area?Lt(this.hass,this._devices,t.area):null),r=!e.bg&&!a.showNames||this._markup,n=this._roomCenter(t),l=t.poly?B`<polygon class="${i}" style="${s}" points="${t.poly.map(t=>t.join(",")).join(" ")}"
                    ...
                    @click=${()=>this._clickRoom(t)} @mousemove=${o}
                    @mouseleave=${()=>this._tip=null}></rect>`;return B`${l}${r?B`<text class="rlabel" x="${n[0]}" y="${n[1]}">${t.name}</text>`:K}`})}
            ${this._markup?this._renderMarkupLayer(i):K}
          </svg>
        ...
        ${this._roomDialog?this._renderRoomDialog():K}
        ${this._spaceDialog?this._renderSpaceDialog():K}
        ${this._markerDialog?this._renderMarkerDialog():K}
        ${this._infoCard?this._renderInfoCard():K}
        ${this._rulesDialog?this._renderRulesDialog():K}
        ${this._importDialog?this._renderImportDialog():K}
        ${this._tip?j`<div class="tip" style="left:${this._tip.x+12}px;top:${this._tip.y+12}px">
              <b>${this._tip.title}</b>${this._tip.meta?j`<span class="m">${this._tip.meta}</span>`:K}
              ${null!=this._tip.temp?j`<span class="m">${this._t("tip.temp_avg")} <b>${this._tip.temp}°</b></span>`:K}
              ${null!=this._tip.lqi?j`<span class="m">${this._t("tip.lqi")}
                    <b style="color:${vt(this._tip.lqi)}">${this._tip.lqi}</b></span>`:K}
            </div>`:K}
        ${this._toast?j`<div class="toast">${this._toast}</div>`:K}
      </ha-card>
    `}_renderDevice(t,e){const i=this._pos(t),s=(i.x-e.x)/e.w*100,a=(i.y-e.y)/e.h*100,o=this._stateClass(t),r=this._liveTemp(t),n=this._liveHum(t),l=this._config?.show_signal&&!t.virtual?Tt(this.hass,t.entities):null;return j`<div
    ...
    >${t.name}</div>`}_roomCenter(t){if(t.poly){const e=t.poly.length;return[t.poly.reduce((t,e)=>t+e[0],0)/e,t.poly.reduce((t,e)=>t+e[1],0)/e]}return[t.x+t.w/2,t.y+.1*Math.min(t.w,t.h)]}_renderMarkupDefs(t){const e=this._gridPitch,i=.14*e;return B`<defs>
      ...
      ${i.length>1?B`<polyline class="pathline" points="${i.map(t=>t.join(",")).join(" ")}"></polyline>`:K}
      ${i.length&&this._cursorPt&&"draw"===this._tool&&!this._contourClosed?B`<line class="preview" x1="${i[i.length-1][0]}" y1="${i[i.length-1][1]}"
            x2="${this._cursorPt[0]}" y2="${this._cursorPt[1]}"></line>`:K}
    ...
    `}_renderMarkupBar(){return j`<div class="editbar">
      ...
      ${"draw"===this._tool?j`<span class="hint">${this._path.length?this._t("markup.hint_points",{n:this._path.length}):this._t("markup.hint_start")}</span>
            ${this._path.length?j`<button class="btn ghost" @click=${this._cancelPath}>${this._t("btn.reset")}</button>`:K}`:K}
      ...
      <div class="dialog" @click=${t=>t.stopPropagation()}>
        ...
        <div class="body">
          ${t.model?j`<div class="inforow"><span class="k">${this._t("info.model")}</span><span>${t.model}</span></div>`:K}
          ${i?j`<div class="inforow"><span class="k">${this._t("info.state")}</span><span>${i}</span></div>`:K}
          ${xt(t.link)?j`<div class="inforow"><span class="k">${this._t("info.link")}</span>
                <a href="${xt(t.link)}" target="_blank" rel="noreferrer noopener">${t.link}</a></div>`:K}
          ${t.description?j`<div class="infodesc">${t.description}</div>`:K}
          ${t.pdfs&&t.pdfs.length?j`<div class="inforow"><span class="k">${this._t("info.manuals")}</span><span class="pdflist">
                ${t.pdfs.map(t=>j`<a class="pdf" href="${xt(t.url)||"#"}" target="_blank" rel="noreferrer noopener">
                    <ha-icon icon="mdi:file-pdf-box"></ha-icon>${t.name}</a>`)}</span></div>`:K}
          ${t.model||i||t.link||t.description||t.pdfs&&t.pdfs.length?K:j`<div class="infodesc muted">${this._t("info.none")}</div>`}
        </div>
        <div class="row">
      ...
      <div class="dialog wide" @click=${t=>t.stopPropagation()}>
        ...
        <div class="body">
          ...
          <div class="bindsel ${e?"virt":""}">
            ...
            ${e?K:j`<div class="curbind"><ha-icon icon="mdi:link-variant"></ha-icon>
            ...
            <div class="candlist">
              ...
              ${i.length?K:j`<div class="cand muted">${this._t("marker.nothing_found")}</div>`}
            </div>
          </div>
          ...
          <div class="pdfedit">
            ${t.pdfs.map(t=>j`<span class="pdftag"><ha-icon icon="mdi:file-pdf-box"></ha-icon>
                <a href="${xt(t.url)||"#"}" target="_blank" rel="noreferrer noopener">${t.name}</a>
      ...
      <div class="dialog wide" @click=${t=>t.stopPropagation()}>
        <div class="hd"><ha-icon icon="mdi:floor-plan"></ha-icon>
          ...
          ${this._importTotal>0&&"create"===t.mode?j`<span class="importprog">${this._t("import.progress",{i:this._importTotal-this._importQueue.length,n:this._importTotal})}</span>`:K}</div>
        <div class="body">
        ...
        <div class="row">
          ...
          <span class="spacer"></span>
          ${this._importTotal>0&&"create"===t.mode?j`<button class="btn ghost" @click=${()=>this._skipImport()}>${this._t("btn.skip")}</button>`:K}
