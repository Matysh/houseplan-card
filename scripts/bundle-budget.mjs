 ...
 *
 * 2026-09-25, #649: 292 600 → 294 900 (замер 294 004 после ребейза на #648). 2.5D стал публичным
 * режимом: переключатель «Общих настроек», стили плиток и их сгенерированная
 * таблица состояний (`iso-tiles`), материалы стен и определение светлого пола
 * нужны карточке до загрузки ленивого `iso-scene-render` — CSS не может ждать
 * граф, иначе первый кадр 2.5D рисует круги Flat. Геометрия света и коллизии
 * остались в ленивом графе. Факт оставляет 896 Б сверху и 1 104 Б до нижней
 * границы; общий бюджет не меняется.
 *
 * 2026-09-26, #654: 294 900 → 296 000 (замер 295 047). Холодный первый кадр
 * обязан знать, что 2.5D запрошен, пока его ленивый граф ещё не загружен, и
 * удерживать заставку до вычисления цвета бумаги; эту границу по определению
