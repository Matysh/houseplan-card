 ...
 *
 * 2026-09-10, #500: потолок перецентрирован 299 600 → 300 200. Идентичность
 * config/layout и единая гейт-последовательность adoption вынесены в
 * `config-adoption.ts` (+~600 Б gzip: владелец идентичности, делегаты хоста,
 * post-write профиль для четырёх путей, которые раньше гейт пропускали);
 * дубли в `_loadFromServer`/`_reloadConfigOnly`/summary recovery сняты.
 * Измеренный факт 299 577 Б оставляет 623 Б сверху и 1 377 Б до нижней
 * границы полосы; общий бюджет 301 066 Б и долг #367 не меняются.
 */
export const INITIAL_VIEW_GZIP_CEILING = 300_200;
