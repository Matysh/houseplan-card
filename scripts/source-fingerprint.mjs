 ...
 * стоила пересборки, пересъёмки и переприёмки. В #334 из-за этого правило
 * приёмки пришлось вынести в `scripts/` и вызывать обёрткой вместо того, чтобы
 * положить его туда, где ему место.
 *
 ...
 * подготовкой сцены, то есть `run.mjs`, `harness.mjs`, `matrix.mjs` и
 * фикстурами. Если в `policy.mjs` когда-нибудь появится что-то, влияющее на
 * кадр, файл обязан вернуться в корпус — и тест ниже это заметит не сам,
 * поэтому вопрос «а как эта правка попадёт в кадр» задаётся при каждой правке.
 */
