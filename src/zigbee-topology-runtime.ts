...
export function zigbeeTopologyRuntimeSnapshot(hass: any): ZigbeeTopologyRuntimeSnapshot {
...
export function subscribeZigbeeTopology(hass: any, listener: () => void): () => void {
  const cache = cacheOf(hass);
...
function transactionOf(value: any): string | null {
  const transaction = value?.transaction ?? value?.data?.transaction;
...
export function refreshZ2mTopology(
  hass: any, baseTopic: string, timeoutMs = 150_000,
): Promise<void> {
  const cache = cacheOf(hass);
  if (!cache) return Promise.resolve();
  ...
  return run(cache, cacheKey, async () => {
    ...
    const response = new Promise<any>((resolve) => { responseResolve = resolve; });
    const unsubscribers: Array<() => void> = [];
    try {
      const unsubInfo = await subscribe.call(hass.connection, (message: any) => {
        if (message?.retain === true && parseMessage(message)) infoResolve?.();
      }, { type: 'mqtt/subscribe', topic: `${topic}/bridge/info` });
      if (typeof unsubInfo === 'function') unsubscribers.push(unsubInfo);
      const unsubResponse = await subscribe.call(hass.connection, (message: any) => {
        if (message?.retain === true) return;
      ...
      if (value?.status && value.status !== 'ok') throw fail('provider');
