...
.dev .hval {
  ...
  `}let Hi=null,Bi=null,ji=!1;const Wi=new Set;function Gi(){if(Hi)return Hi;try{const t=JSON.parse(localStorage.getItem("houseplan_card_cfg_v1")||"null");if(t&&t.config&&Array.isArray(t.config.spaces))return{config:t.config,rev:t.rev||0,layout:t.layout||{}}}catch{}return null}function Vi(t){return Hi?Promise.resolve(Hi):Bi||(Bi=async function(t){const[e,i]=await Promise.all([t.callWS({type:"houseplan/config/get"}),t.callWS({type:"houseplan/layout/get"})]);if(Hi={config:e?.config??null,rev:e?.rev??0,layout:i?.layout??{}},!ji&&t.connection?.subscribeEvents){ji=!0;try{await t.connection.subscribeEvents(()=>{Hi=null,Wi.forEach(t=>t())},"houseplan_config_updated")}catch{ji=!1}}return Hi}(t).finally(()=>{Bi=null}),Bi)}class Ki extends lt{constructor(){super(...arguments),this._spaces=null,this._spacesLoading=!1}setConfig(t){this._config=t}async _loadSpaces(){if(!this._spaces&&!this._spacesLoading&&this.hass){this._spacesLoading=!0;try{const t=await this.hass.callWS({type:"houseplan/config/get"});this._spaces=(t?.config?.spaces||[]).map(t=>({value:t.id,label:t.title||t.id}))}catch{this._spaces=[]}finally{this._spacesLoading=!1}}}get _lang(){return Ai(this.hass,this._config?.language)}get _schema(){const t=this._spaces||[];return[t.length?{name:"space",selector:{select:{mode:"dropdown",options:t}}}:{name:"space",selector:{text:{}}},{name:"title",selector:{text:{}}},{name:"show_button",selector:{boolean:{}}},{name:"button_label",selector:{text:{}}},{name:"button_target",selector:{text:{}}},{name:"aspect_ratio",selector:{text:{}}},{name:"icon_size",selector:{number:{min:1,max:6,step:.1,mode:"box"}}}]}render(){if(!this.hass||!this._config)return G;this._loadSpaces();const t=this._lang,e={space:Ri(t,"editor.space"),title:Ri(t,"editor.title"),show_button:Ri(t,"editor.show_button"),button_label:Ri(t,"editor.button_label"),button_target:Ri(t,"editor.button_target"),aspect_ratio:Ri(t,"editor.aspect_ratio"),icon_size:Ri(t,"editor.icon_size")};return B`<ha-form
    ...
    `}_renderDevice(t,e,i=!0){const s=this._pos(t),o=(s.x-e.x)/e.w*100,n=(s.y-e.y)/e.h*100,r=this._stateClass(t),a=this._liveTemp(t),l=this._liveHum(t),c=i&&!t.virtual?wi(this.hass,t.entities):null,h=t.marker,p=h?.display||"badge",d="ripple"===p||"icon_ripple"===p,u=t.primary?this.hass.states[t.primary]:void 0,_="value"===p?null!=a?a+"°":null!=l?l+"%":u&&!isNaN(parseFloat(u.state))?parseFloat(u.state)+(u.attributes?.unit_of_measurement?" "+u.attributes.unit_of_measurement:""):null:null,g=t.primary?t.primary.split(".")[0]:null,m=this._config?.live_states?ei(t.icon,g,u?.attributes?.device_class,u?.state,!!h?.icon):t.icon,f=(h?.controls||[]).filter(ri),b=this._config?.live_states?f.length?f.map(t=>ii(this.hass.states[t])).find(t=>t)||null:"light"===g?ii(u):null:null,v=this._config?.live_states&&function(t,e,i){return"on"===i&&("siren"===t||"binary_sensor"===t&&!!e&&mi.has(e))}(g,u?.attributes?.device_class,u?.state),y=d&&!!t.primary&&ke(this.hass.states[t.primary]?.state),w=Number(h?.size)>0?Number(h.size):1,x=Number(h?.angle)||0,$=Number(h?.ripple_size)>0?Number(h.ripple_size):3,k=[`left:${o}%`,`top:${n}%`];return 1!==w&&k.push(`--dev-scale:${w}`),d&&(k.push(`--ripple-scale:${$}`),h?.ripple_color?k.push(`--ripple-color:${h.ripple_color}`):b&&k.push(`--ripple-color:${b}`)),b&&k.push(`--light-color:${b}`),B`<div
        ...
        <div class="body">
          ${(()=>{const e=this._cardEntities(t);return e.length?B`<div class="entlist">
              ${e.map(({eid:t,kind:e})=>{const i=this.hass.states[t],s=this.hass.entities[t]?.name||i?.attributes?.friendly_name||t,o=i?this.hass.formatEntityState?.(i)??i.state:"",n="on"===i?.state||["open","unlocked","playing","cleaning"].includes(i?.state);return B`<div class="entrow ${n?"on":""}">
                  <ha-icon icon=${ei(mt(s,"",this._iconRules),t.split(".")[0],i?.attributes?.device_class,i?.state,!1)}></ha-icon>
