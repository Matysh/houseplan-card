      ...
      <ha-card>
        ...
        <div class="stage ${this._markup?"markup tool-"+this._tool+("split"!==this._tool||this._splitSel?"":" pickstage")+("openwall"===this._tool&&this._openWallHover?" wallhot":""):""} ${"decor"===this._mode?"dtool-"+this._decorTool:""} ${e.bg?"":"noplan"} mode-${this._mode}${p?" daynight":""}${this._booting?" hpboot":""}${this._bootSoft?" hpsettle":""}"
          ...
          <svg viewBox="${c.x} ${c.y} ${c.w} ${c.h}" preserveAspectRatio="xMidYMid meet">
            ${(()=>{if(e.bg){const t=this._paperRect(e);return j`<rect class="hp-paper" x="${t.x}" y="${t.y}" width="${t.w}" height="${t.h}" pointer-events="none"></rect>`}return $e(e.rooms).map(t=>"poly"in t?j`<polygon class="hp-paper" points="${t.poly}" pointer-events="none"></polygon>`:j`<rect class="hp-paper" x="${t.rect.x}" y="${t.rect.y}" width="${t.rect.w}" height="${t.rect.h}" rx="${t.rect.rx}" pointer-events="none"></rect>`)})()}
