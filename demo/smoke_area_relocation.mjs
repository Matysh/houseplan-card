...
const res = await page.evaluate(async () => {
  ...
  staticConfig.settings.marker_area_snapshot.d_light1 = {
    binding: 'device:d_light1', area: 'living_room',
  };
  staticConfig.settings.new_device_ids = [];
  const staticLayout = { ...c._layout, d_light1: { s: 'f1', x: 0.22, y: 0.22 } };
  const staticCalls = [];
  ...
  const staticEl = staticCard.renderRoot.querySelector('.dev[data-id="d_light1"]');
  ...
  return {
    ...
    attentionShown: c._serverCfg.settings.new_device_ids?.includes('d_light1')
    ...
    staticReadOnlyProjection: staticCard._areaRelocationIds.has('d_light1')
